...
#define NTEXTBUFFERS 128
  ...
  inline void pdfomus_flushout() {
    while (pdfomus_inptr != pdfomus_outptr) {
      ...
      if (pdfomus_outptr >= &pdfomus_textbuffers[NTEXTBUFFERS - 1]) pdfomus_outptr = &pdfomus_textbuffers[0]; else ++pdfomus_outptr;
    }
  }
...
#define CLOCKDELAY 500
  ...
  struct pdfomus_scopedsync {
    pdfomus_scopedsync() {fomus_flush(); pdfomus_sync = true;}
    ~pdfomus_scopedsync() {pdfomus_sync = false;}
  };
}
...
extern "C" {
  ...
  typedef struct _fomus {
    ...
    int xxxxx; // when compiling in mingw32, need to put an int between x_obj and fom, probably some kind of alignment issue?
    FOMUS fom;
    t_clock* clock;
  } t_fomus;
  ...
  void pdfomus_voice(t_fomus *x, t_symbol *s, int argc, t_atom *i) { // float or list
    pdfomus_flushout();
    if (!x->fom) return;
  ...
  void pdfomus_mark(t_fomus *x, t_symbol *s, int argc, t_atom *i) { // list
	...
	break;
      case A_SYMBOL:
	if (fi) {fomus_sval(x->fom, fomus_par_markid, fomus_act_set, atom_getsymbol(i)->s_name); fi = false;}
	else fomus_sval(x->fom, fomus_par_markval, fomus_act_add, atom_getsymbol(i)->s_name);      
	break;
      default: ;
      }
    }
  ...
  void* pdfomus_init_new() {
    ...
    inlet_new(&x->x_obj, &x->x_obj.ob_pd, &s_float, gensym("voice")); // a list has to go in leftmost inlet
  ...
  void pdfomus_output(const char* str) {
    if (pdfomus_sync) {
      ...
      std::string* ptr = (pdfomus_inptr >= &pdfomus_textbuffers[NTEXTBUFFERS - 1] ? &pdfomus_textbuffers[0] : pdfomus_inptr + 1);
      if (ptr == pdfomus_outptr) return; // don't run into outptr, discard output
  ...
  FOMUSMOD_EXPORT void fomus_setup() {
    ...
    post("%s", "Copyright (c) 2009, 2010, 2011 David Psenicka, All Rights Reserved");
    // pd stuff
    fomus_class = class_new(gensym("fomus"), (t_newmethod)pdfomus_init_new, (t_method)pdfomus_init_free, sizeof(t_fomus), CLASS_DEFAULT, (t_atomtype)0); // no arguments
    // methods
    class_addmethod(fomus_class, (t_method)pdfomus_new, gensym("new"), (t_atomtype)0);
    class_addmethod(fomus_class, (t_method)pdfomus_clear, gensym("clear"), (t_atomtype)0);
    class_addmethod(fomus_class, (t_method)pdfomus_load, gensym("load"), A_DEFSYMBOL, (t_atomtype)0);
    class_addmethod(fomus_class, (t_method)pdfomus_send, gensym("send"), A_GIMME, (t_atomtype)0); // call fomus_parse
    class_addmethod(fomus_class, (t_method)pdfomus_run, gensym("run"), A_DEFSYMBOL, (t_atomtype)0);
    class_addmethod(fomus_class, (t_method)pdfomus_flushout, gensym("flush"), (t_atomtype)0);
    //
    class_addmethod(fomus_class, (t_method)pdfomus_part, gensym("part"), A_SYMBOL, (t_atomtype)0);
    class_addmethod(fomus_class, (t_method)pdfomus_time, gensym("time"), A_DEFFLOAT, (t_atomtype)0);
    class_addmethod(fomus_class, (t_method)pdfomus_grtime, gensym("grtime"), A_DEFFLOAT, (t_atomtype)0);
    class_addmethod(fomus_class, (t_method)pdfomus_dur, gensym("dur"), A_DEFFLOAT, (t_atomtype)0);
    class_addmethod(fomus_class, (t_method)pdfomus_voice, gensym("voice"), A_GIMME, (t_atomtype)0); // only a float goes in inlet
    class_addmethod(fomus_class, (t_method)pdfomus_pitch, gensym("pitch"), A_DEFFLOAT, (t_atomtype)0);
    class_addmethod(fomus_class, (t_method)pdfomus_dyn, gensym("dyn"), A_DEFFLOAT, (t_atomtype)0);
    class_addmethod(fomus_class, (t_method)pdfomus_mark, gensym("mark"), A_GIMME, (t_atomtype)0);
    //
    class_addmethod(fomus_class, (t_method)pdfomus_enternote, gensym("note*"), (t_atomtype)0);
    class_addmethod(fomus_class, (t_method)pdfomus_enterrest, gensym("rest*"), (t_atomtype)0);
    class_addmethod(fomus_class, (t_method)pdfomus_entermark, gensym("mark*"), (t_atomtype)0);
