...
namespace fomus {
  ...
  void fomstdout(const char* out) {fputs(out, stdout);}
  void fomstderr(const char* out) {fputs(out, stderr);}
  
  boost::iostreams::stream<myout> fout(fomstdout, stdout, false), ferr(fomstderr, stderr, true);
