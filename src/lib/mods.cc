...
#warning                                                                       \
    "make sure info functions and some modinout functions don't get called after processing has started"
...
namespace fomus {
  ...
  int foundmod(const char* filename, lt_ptr dat) { // data is &vector<string>
    try {
      ...
      if (std::binary_search(((foundmoddata*) dat)->bl.begin(),
                             ((foundmoddata*) dat)->bl.end(), mn) ||
          !((foundmoddata*) dat)->lded.insert(mn).second) {
        return 0;
      }
      lt_dladvise adv;
      lt_dlhandle ha = NULL;
      if (!lt_dladvise_init(&adv) && !lt_dladvise_ext(&adv) &&
          !lt_dladvise_local(&adv))
        ha = lt_dlopenadvise(filename, adv);
      ...
      try {
        ...
        try {
          esy = (modfun_initerr) getsym(ha, mn,
                                        "module_initerr"); // throws moderr
          const char* err = esy(); // call MODFUN_ERR (see if INIT produced err)
          if (err) {
            CERR << "error `" << err << "' initializing module `" << mn << '\''
                 << std::endl;
            throw moderr();
          }
          ...
          case module_modoutput: {
            dlmodstufferr st(
                boost::filesystem::path(filename)
                    ...
                    .FS_DIRECTORY_STRING()
                    .c_str(),
                mn, (modfun_newdata) getsym(ha, mn, "module_newdata"), //
          ...
          case module_modaux:
            mb.reset(new dlmodaux(
                dlmodstuffinit(
                    boost::filesystem::path(filename)
                        .FS_BRANCH_PATH()
                        .FS_DIRECTORY_STRING()
                        .c_str(),
                    mn, (modfun_longname) getsym(ha, mn, "module_longname"),
                    (modfun_author) getsym(ha, mn, "module_author"),
                    (modfun_free) getsym(ha, mn, "module_free"),
                    (modfun_doc) getsym(ha, mn, "module_doc"), esy, getity,
                    (modfun_getsetting) getsym(ha, mn, "module_get_setting"),
                    (modfun_ready) getsym(ha, mn, "module_ready")),
                (engauxfun_interfaceid) getsym(
                    ha, mn, "aux_ifaceid"), // provides the interfaceid
                (auxfun_fillinterface) getsym(ha, mn, "aux_fill_iface")));
            break;
          default:
            mb.reset(new dlmodmod(
                dlmodstuff(
                    boost::filesystem::path(filename)
                        .FS_BRANCH_PATH()
                        .FS_DIRECTORY_STRING()
                    ...
                    (modfun_getsetting) getsym(ha, mn, "module_get_setting"),
                    (modfun_ready) getsym(ha, mn, "module_ready")),
                (modfun_interfaceid) getsym(
                    ha, mn,
                    "module_engine_iface"), // what engine type to hook up with
          ...
          if (sy != NULL) {
            ...
            if (esy != NULL) {
              const char* err =
                  esy(); // call MODFUN_ERR (see if FREE produced err)
              if (err != NULL) {
                CERR << "error `" << err << "' freeing module `" << mn << '\''
                     << std::endl;
              }
            }
          }
...
#else
        lt_dlclose(
            ha); // closing error isn't terribly useful after some other error
#endif
      }
  ...
  void findconflicts(int strt = 0) {
    bool err = false;
    {
      ...
      std::map<const std::string, const varbase*, isiless>::const_iterator ne(
          names.end());
      int c = 0;
      for (varsvect::iterator i(vars.begin()); i != vars.end(); ++i, ++c) {
        ...
        INSIT:
          names.insert(
              std::map<const std::string, const varbase*, isiless>::value_type(
                  n, i->get()));
          continue;
        }
        std::map<const std::string, const varbase*, isiless>::iterator f(
            names.find(n));
        if (f != ne) {
          CERR << "conflicting setting `" << n << "' in modules `"
               << f->second->getmodsname() << "' and `" << v.getmodsname()
               << "'\n";
          err = true;
        } else
          goto INSIT;
  ...
  void findinvalids(int strt = 0) {
    ...
    for (varsvect_it i(vars.begin() + strt); i != vars.end(); ++i) {
      varbase& v = **i;
      if (!v.isvalid(0)) {
        ferr << " in setting `" << v.getname() << "', module `"
             << v.getmodsname() << "'\n";
        err = true;
      }
    }
  ...
  const char* repl_modfun_initerr() {
    return 0;
  }
  void* repl_modfun_newdata(FOMUS f) {
    return 0;
  }
  void repl_modfun_freedata(void* data) {}
  const char* repl_modfun_longname() {
    return "(unnamed)";
  }
  const char* repl_modfun_author() {
    return "(callback)";
  }
  const char* repl_modfun_doc() {
    return "";
  }
  const char* repl_modfun_err(void* data) {
    return 0;
  }
  int repl_modfun_getsetting(int n, struct module_setting* set, int id) {
    return 0;
  }
  void repl_modfun_ready() {}
  const char* repl_modinoutfun_getloadid() {
    return 0;
  }
  int repl_modoutfun_ispre() {
    return 0;
  }
  int repl_modfun_sameinst(module_obj a, module_obj b) {
    return 1;
  }
  int repl_modfun_interfaceid() {
    return ENGINE_INTERFACEID;
  }
  const char* repl_modfun_engine(void* m) {
    return "dumb";
  }
  int repl_modfun_priority() {
    return 0;
  }
  }
  ...
  template <typename T>
  const T checkcb(const T cb, const char* name, const char* slot) {
    if (cb)
      return cb;
    CERR << "missing callback function `" << slot << "\' for module `" << name
         << '\'' << std::endl;
    throw errbase();
  }
  template <typename T>
  const T checkcb(const T cb, const T repl) {
    return (cb ? cb : repl);
  }
  ...
  void initmodules() {
    dlin = true;
    {
      ...
      if (p != NULL)
        pa = std::string(p) + LT_PATHSEP_CHAR + pa; // CMD_MACRO(MODULE_PATH);
      p = getenv("FOMUS_MODULES_BLACKLIST");
      foundmoddata xdata;
      if (p != NULL) {
        boost::split(xdata.bl, p, boost::lambda::_1 == LT_PATHSEP_CHAR);
        std::for_each(
            xdata.bl.begin(), xdata.bl.end(),
            boost::lambda::bind(&boost::trim<std::string>, boost::lambda::_1,
                                boost::lambda::constant_ref(std::locale())));
      }
    ...
    std::for_each(mods.begin(), mods.end(),
                  boost::lambda::bind(addvars, boost::lambda::_1));
    findconflicts();
    findinvalids();
    std::for_each(mods.begin(), mods.end(),
                  boost::lambda::bind(&modbase::isready, boost::lambda::_1));
  }
  ...
  void addvars(const modbase& mb) {
    for (int i = 0;; ++i) {
      ...
      if (!mb.getsetting(i, &set.set, vars.size()))
        break;
  ...
  // MODULE API
  bool module_valid_intaux(const struct module_value& val, const fomus_int min,
                           const enum module_bound minbnd, const fomus_int max,
                           const enum module_bound maxbnd,
                           const char* printerr) {
  ...
  inline bool
  module_valid_intn(const struct module_value& val, const fomus_int min,
                    const enum module_bound minbnd, const fomus_int max,
                    const enum module_bound maxbnd, const valid_listint fun,
                    const int n, const char* printerr) {
    if (!module_valid_intaux(val, min, minbnd, max, maxbnd, printerr))
      return false;
  ...
  module_valid_intn(const struct module_value& val, const fomus_int min,
                    const enum module_bound minbnd, const fomus_int max,
                    const enum module_bound maxbnd, const valid_mapint fun,
                    const char* s, const int n, const char* printerr) {
    if (!module_valid_intaux(val, min, minbnd, max, maxbnd, printerr))
      return false;
  ...
  bool module_valid_rataux(const struct module_value& val, const fomus_rat& min,
                           const enum module_bound minbnd, const fomus_rat& max,
                           const enum module_bound maxbnd,
                           const char* printerr) {
    ...
    rat v(val.type == module_rat ? rat(val.val.r.num, val.val.r.den)
                                 : val.val.i);
    rat mi(min.num, min.den);
    rat ma(max.num, max.den);
    if ((minbnd == module_incl && v < mi) ||
        (minbnd == module_excl && v <= mi) ||
        (maxbnd == module_incl && v > ma) ||
        (maxbnd == module_excl && v >= ma)) {
      printexp(printerr);
      return false;
    }
    return true;
  ...
  inline bool
  module_valid_ratn(const struct module_value& val, const fomus_rat& min,
                    const enum module_bound minbnd, const fomus_rat& max,
                    const enum module_bound maxbnd, const valid_listrat fun,
                    const int n, const char* printerr) {
    if (!module_valid_rataux(val, min, minbnd, max, maxbnd, printerr))
      return false;
  ...
  module_valid_ratn(const struct module_value& val, const fomus_rat& min,
                    const enum module_bound minbnd, const fomus_rat& max,
                    const enum module_bound maxbnd, const valid_maprat fun,
                    const char* s, const int n, const char* printerr) {
    if (!module_valid_rataux(val, min, minbnd, max, maxbnd, printerr))
      return false;
  ...
  bool module_valid_numaux(const struct module_value& val,
                           const struct module_value& min,
                           const enum module_bound minbnd,
                           const struct module_value& max,
                           const enum module_bound maxbnd,
                           const char* printerr) {
    numb v;
    switch (val.type) {
    ...
    case module_float:
      ...
      break;
    case module_rat:
      v = rat(val.val.r.num, val.val.r.den);
      break;
    default:
      printexp(printerr);
      return false;
    ...
    case module_float:
      ...
      break;
    case module_rat:
      mi = rat(min.val.r.num, min.val.r.den);
      break;
    default:
      return false;
    }
    ...
    case module_float:
      ...
      break;
    case module_rat:
      ma = rat(max.val.r.num, max.val.r.den);
      break;
    default:
      return false;
    }
    if ((minbnd == module_incl && v < mi.modval()) ||
        (minbnd == module_excl && v <= mi.modval()) ||
        (maxbnd == module_incl && v > ma.modval()) ||
        (maxbnd == module_excl && v >= ma.modval())) {
      printexp(printerr);
      return false;
    }
    return true;
  }
  inline bool module_valid_numn(const struct module_value& val,
                                const struct module_value& min,
                                const enum module_bound minbnd,
                                const struct module_value& max,
                                const enum module_bound maxbnd,
                                const valid_listnum fun, const int n,
                                const char* printerr) {
    if (!module_valid_numaux(val, min, minbnd, max, maxbnd, printerr))
      return false;
    if (fun && !fun(n, &val)) {
      printexp(printerr);
      return false;
  ...
  inline bool module_valid_numn(const struct module_value& val,
                                ...
                                const struct module_value& max,
                                const enum module_bound maxbnd,
                                const valid_mapnum fun, const char* s,
                                const int n, const char* printerr) {
    if (!module_valid_numaux(val, min, minbnd, max, maxbnd, printerr))
      return false;
    if (fun && !fun(n, s, &val)) {
  ...
  bool module_valid_stringaux(const struct module_value& val, const int minlen,
                              const int maxlen, const char* printerr) {
    if (val.type == module_string && val.val.s != NULL) {
      size_t sz = strlen(val.val.s);
      if ((minlen < 0 || sz >= (size_t) minlen) &&
          (maxlen < 0 || sz <= (size_t) maxlen))
        return true;
    }
    printexp(printerr);
    return false;
  }
  inline bool module_valid_stringn(const struct module_value& val,
                                   const int minlen, const int maxlen,
                                   const valid_liststring fun, const int n,
                                   const char* printerr) {
    if (!module_valid_stringaux(val, minlen, maxlen, printerr))
      return false;
  ...
  inline bool module_valid_stringn(const struct module_value& val,
                                   const int minlen, const int maxlen,
                                   const valid_mapstring fun, const char* s,
                                   const int n, const char* printerr) {
    if (!module_valid_stringaux(val, minlen, maxlen, printerr))
      return false;
  ...
  inline bool listofvals_isinvalid(int& n, const struct module_value& val,
                                   valid_listval fun, const char* printerr) {
    if (!fun(++n, val)) {
      printexp(printerr);
      return false;
    }
  ...
  inline bool maptoints_isinvalid(int& n, const char*& s,
                                  const struct module_value& val,
                                  const fomus_int min, enum module_bound minbnd,
                                  const fomus_int max, enum module_bound maxbnd,
                                  valid_mapint fun, const char* printerr) {
    if (++n % 2 == 0) {
      s = val.val.s;
      return !module_valid_stringaux(val, 1, -1, printerr);
    } else {
      return !module_valid_intn(val, min, minbnd, max, maxbnd, fun, s, n / 2,
                                printerr);
    }
  }

  inline bool
  maptorats_isinvalid(int& n, const char*& s, const struct module_value& val,
                      const struct fomus_rat min, enum module_bound minbnd,
                      const struct fomus_rat max, enum module_bound maxbnd,
                      valid_maprat fun, const char* printerr) {
    if (++n % 2 == 0) {
      s = val.val.s;
      return !module_valid_stringaux(val, 1, -1, printerr);
    } else {
      return !module_valid_ratn(val, min, minbnd, max, maxbnd, fun, s, n / 2,
                                printerr);
    }
  }

  inline bool
  maptonums_isinvalid(int& n, const char*& s, const struct module_value& val,
                      const struct module_value& min, enum module_bound minbnd,
                      const struct module_value& max, enum module_bound maxbnd,
                      valid_mapnum fun, const char* printerr) {
    if (++n % 2 == 0) {
      s = val.val.s;
      return !module_valid_stringaux(val, 1, -1, printerr);
    } else {
      return !module_valid_numn(val, min, minbnd, max, maxbnd, fun, s, n / 2,
                                printerr);
    }
  }

  inline bool maptostrings_isinvalid(int& n, const char*& s,
                                     const struct module_value& val, int minlen,
                                     int maxlen, valid_mapstring fun,
                                     const char* printerr) {
    if (++n % 2 == 0) {
      s = val.val.s;
      return !module_valid_stringaux(val, 1, -1, printerr);
    } else {
      return !module_valid_stringn(val, minlen, maxlen, fun, s, n / 2,
                                   printerr);
    }
  }

  inline bool maptovals_isinvalid(int& n, const char*& s,
                                  const struct module_value& val,
                                  valid_mapval fun, const char* printerr) {
    if (++n % 2 == 0) {
      s = val.val.s;
      return !module_valid_stringaux(val, 1, -1, printerr);
    } else {
  ...
  void modbase::fireup(fomusdata* fd, void* data) const {
    ...
    if (i == modsbyname.end()) {
      CERR << "engine`" << e << "' doesn't exist in module `" << getcname()
           << '\'' << std::endl;
      throw errbase();
    }
    if (i->second->getifaceid() != getifaceid()) {
      CERR << "invalid engine type in module `" << getcname() << '\''
           << std::endl;
      throw errbase();
    }
...
void module_register(const char* name,
  ...
  case module_modoutput: {
    dlmodstufferr st("(callback)", name,
                     ...
                     checkcb(callbacks->doc_fun, repl_modfun_doc), esy, getity,
    ...
    case module_modengine:
      mb.reset(new dlmodeng(
          st,
          checkcb(callbacks->engaux_interfaceid_fun, name,
                  "engaux_interfaceid_fun"),
          checkcb(callbacks->run_fun, name, "run_fun"),
          checkcb(callbacks->getinterface_fun, name, "getinterface_fun")));
      break;
    case module_modinput:
      mb.reset(new dlmodin(
          st, checkcb(callbacks->getext_fun, name, "getext_fun"),
  ...
  case module_modaux:
    mb.reset(new dlmodaux(
        ...
        checkcb(callbacks->engaux_interfaceid_fun, name,
                "engaux_interfaceid_fun"),
        checkcb(callbacks->aux_fillinterface_fun, name,
                "aux_fillinterface_fun")));
    break;
  default:
    mb.reset(new dlmodmod(
        dlmodstuff("(callback)", name,
                   checkcb(callbacks->newdata_fun, repl_modfun_newdata),
                   checkcb(callbacks->freedata_fun, repl_modfun_freedata),
                   checkcb(callbacks->longname_fun, repl_modfun_longname),
                   checkcb(callbacks->author_fun, repl_modfun_author),
        ...
        checkcb(callbacks->mod_fillinterface_fun, name,
                "mod_fillinterface_fun"),
        checkcb(callbacks->itertype_fun, name, "itertype_fun"),
...
struct module_list module_new_list(int n) { // api fun
  ...
  std::for_each(arr.vals, arr.vals + n,
                boost::lambda::bind(initvalue, boost::lambda::_1));
  return arr;
  EXIT_API_MODLIST0;
}
void module_free_list(struct module_list arr) { // api fun
  ENTER_API;
  if (arr.vals) {
    std::for_each(arr.vals, arr.vals + arr.n,
                  bind(freevalue, boost::lambda::_1));
...
fomus_int module_getval_int(struct module_value val) {
  ...
  case module_int:
    return val.val.i;
  case module_float:
    return lround(val.val.f);
  case module_rat:
    return rattoint(rat(val.val.r.num, val.val.r.den));
  default:
...
fomus_float module_getval_float(struct module_value val) {
  ...
  case module_float:
    return val.val.f;
  case module_rat:
    return boost::rational_cast<fomus_float>(rat(val.val.r.num, val.val.r.den));
  default:
    CERR << "invalid value type" << std::endl;
    throw errbase();
  }
...
int module_valid_int(struct module_value val, fomus_int min,
...
int module_valid_rat(struct module_value val, fomus_rat min,
                     enum module_bound minbnd, fomus_rat max,
                     enum module_bound maxbnd, valid_rat fun,
                     const char* printerr) {
  ENTER_API;
  if (!module_valid_rataux(val, min, minbnd, max, maxbnd, printerr))
    return false;
...
int module_valid_num(struct module_value val, struct module_value min,
                     enum module_bound minbnd, struct module_value max,
                     enum module_bound maxbnd, valid_num fun,
                     const char* printerr) {
  ENTER_API;
  if (!module_valid_numaux(val, min, minbnd, max, maxbnd, printerr))
    return false;
...
int module_valid_string(struct module_value val, int minlen, int maxlen,
                        valid_string fun, const char* printerr) {
  ENTER_API;
  if (!module_valid_stringaux(val, minlen, maxlen, printerr))
    return false;
...
int module_valid_listofints(struct module_value val, int minlen, int maxlen,
                            fomus_int min, enum module_bound minbnd,
                            fomus_int max, enum module_bound maxbnd,
                            valid_listint fun, const char* printerr) {
  ENTER_API;
  if (!module_valid_listaux(val, minlen, maxlen, printerr))
    return false;
  int n = 0;
  if (hassome(val.val.l.vals, val.val.l.vals + val.val.l.n,
              !boost::lambda::bind(module_valid_intn, boost::lambda::_1, min,
                                   minbnd, max, maxbnd, fun,
                                   boost::lambda::var(n)++, printerr)))
    return false;
  return true;
  EXIT_API_0;
}

int module_valid_listofrats(struct module_value val, int minlen, int maxlen,
                            fomus_rat min, enum module_bound minbnd,
                            fomus_rat max, enum module_bound maxbnd,
  ...
  int n = 0;
  if (hassome(val.val.l.vals, val.val.l.vals + val.val.l.n,
              !boost::lambda::bind(module_valid_ratn, boost::lambda::_1,
                                   boost::lambda::constant_ref(min), minbnd,
                                   boost::lambda::constant_ref(max), maxbnd,
                                   fun, boost::lambda::var(n)++, printerr)))
    return false;
  return true;
  EXIT_API_0;
}

int module_valid_listofnums(struct module_value val, int minlen, int maxlen,
  ...
  int n = 0;
  if (hassome(val.val.l.vals, val.val.l.vals + val.val.l.n,
              !boost::lambda::bind(module_valid_numn, boost::lambda::_1,
                                   boost::lambda::constant_ref(min), minbnd,
                                   boost::lambda::constant_ref(max), maxbnd,
                                   fun, boost::lambda::var(n)++, printerr)))
    return false;
...
int module_valid_listofstrings(struct module_value val, int minlen0,
                               int maxlen0, int minlen, int maxlen,
                               valid_liststring fun, const char* printerr) {
  ENTER_API;
  if (!module_valid_listaux(val, minlen0, maxlen0, printerr))
    return false;
  int n = 0;
  if (hassome(val.val.l.vals, val.val.l.vals + val.val.l.n,
              !boost::lambda::bind(module_valid_stringn, boost::lambda::_1,
                                   minlen, maxlen, fun, boost::lambda::var(n)++,
                                   printerr)))
    return false;
  return true;
  EXIT_API_0;
...
int module_valid_listofvals(struct module_value val, int minlen, int maxlen,
                            valid_listval fun, const char* printerr) {
  ENTER_API;
  if (!module_valid_listaux(val, minlen, maxlen, printerr))
    return false;
  int n = -1;
  if (fun &&
      hassome(val.val.l.vals, val.val.l.vals + val.val.l.n,
              !boost::lambda::bind(listofvals_isinvalid, boost::lambda::var(n),
                                   boost::lambda::_1, fun, printerr)))
    return false;
  return true;
  EXIT_API_0;
}

int module_valid_maptoints(struct module_value val, int minlen, int maxlen,
                           fomus_int min, enum module_bound minbnd,
                           fomus_int max, enum module_bound maxbnd,
                           valid_mapint fun, const char* printerr) {
  ENTER_API;
  if (!module_valid_listaux(val, minlen * 2, maxlen * 2, printerr))
    return false;
  int n = -1;
  const char* s;
  if (hassome(val.val.l.vals, val.val.l.vals + val.val.l.n,
              boost::lambda::bind(maptoints_isinvalid, boost::lambda::var(n),
                                  boost::lambda::var(s), boost::lambda::_1, min,
                                  minbnd, max, maxbnd, fun, printerr)))
    return false;
  if (n % 2 == 0) {
...
int module_valid_maptorats(struct module_value val, int minlen, int maxlen,
                           fomus_rat min, enum module_bound minbnd,
                           fomus_rat max, enum module_bound maxbnd,
                           valid_maprat fun, const char* printerr) {
  ENTER_API;
  ...
  const char* s;
  if (hassome(val.val.l.vals, val.val.l.vals + val.val.l.n,
              boost::lambda::bind(maptorats_isinvalid, boost::lambda::var(n),
                                  boost::lambda::var(s), boost::lambda::_1,
                                  boost::lambda::constant_ref(min), minbnd,
                                  boost::lambda::constant_ref(max), maxbnd, fun,
                                  printerr)))
    return false;
  if (n % 2 == 0) {
    CERR << "missing map value";
...
int module_valid_maptonums(struct module_value val, int minlen, int maxlen,
                           struct module_value min, enum module_bound minbnd,
                           struct module_value max, enum module_bound maxbnd,
                           valid_mapnum fun, const char* printerr) {
  ENTER_API;
  ...
  const char* s;
  if (hassome(val.val.l.vals, val.val.l.vals + val.val.l.n,
              boost::lambda::bind(maptonums_isinvalid, boost::lambda::var(n),
                                  boost::lambda::var(s), boost::lambda::_1,
                                  boost::lambda::constant_ref(min), minbnd,
                                  boost::lambda::constant_ref(max), maxbnd, fun,
                                  printerr)))
...
int module_valid_maptostrings(struct module_value val, int minlen0, int maxlen0,
                              int minlen, int maxlen, valid_mapstring fun,
                              const char* printerr) {
  ENTER_API;
  if (!module_valid_listaux(val, minlen0 * 2, maxlen0 * 2, printerr))
    return false;
  int n = -1;
  const char* s;
  if (hassome(val.val.l.vals, val.val.l.vals + val.val.l.n,
              boost::lambda::bind(maptostrings_isinvalid, boost::lambda::var(n),
                                  boost::lambda::var(s), boost::lambda::_1,
                                  minlen, maxlen, fun, printerr)))
    return false;
  if (n % 2 == 0) {
    CERR << "missing map value";
    return false;
...
int module_valid_maptovals(struct module_value val, int minlen, int maxlen,
                           valid_mapval fun, const char* printerr) {
  ENTER_API;
  if (!module_valid_listaux(val, minlen * 2, maxlen * 2, printerr))
    return false;
  ...
  const char* s;
  if (hassome(val.val.l.vals, val.val.l.vals + val.val.l.n,
              boost::lambda::bind(maptovals_isinvalid, boost::lambda::var(n),
                                  boost::lambda::var(s), boost::lambda::_1, fun,
                                  printerr)))
    return false;
  if (n % 2 == 0) {
    CERR << "missing map value";
...
int module_setting_allowed(const enum module_setting_loc allowedin,
...
module_objlist module_get_percinsts(FOMUS f, int all) { // TODO: use vectors<>
  ENTER_API;
  return all ? ((fomusdata*) f)->get_allpercinstslist()
             : ((fomusdata*) f)->get_percinstslist();
  EXIT_API_MODOBJLIST0;
}
module_objlist module_get_insts(FOMUS f, int all) {
  ENTER_API;
  return all ? ((fomusdata*) f)->get_allinstslist()
             : ((fomusdata*) f)->get_instslist();
  EXIT_API_MODOBJLIST0;
}
...
const struct info_setlist
info_get_settings(module_obj o) { // modinouts don't run in parallel
  ENTER_API;
  try {
...
const struct info_marklist info_get_marks() {
  ...
  for_each2(
      marklist.marks, marklist.marks + marklist.n, markdefs.begin(),
...
// for modules working with entire parts--this isn't synchronized with
// `nextnote', these are just iterators
...
int module_less(module_obj obj1, module_obj obj2) {
  ...
  try {
    ...
    CERR << "cannot determine order of " << ((modobjbase*) obj1)->gettype()
         << " object and " << ((modobjbase*) obj2)->gettype() << " object";
    modprinterr();
    throw errbase();
  }
  EXIT_API_0;
...
int module_markcantouch(int type, module_noteobj note) {
  ...
  assert(((modobjbase*) note)->isvalid());
  return ((modobjbase*) note)->getismarkrest() ||
         markdefs[type].getcantouch(((modobjbase*) note)->getnoteevbase());
  EXIT_API_0;
}
int module_markcanspanone(int type, module_noteobj note) {
  ...
  assert(((modobjbase*) note)->isvalid());
  return ((modobjbase*) note)->getismarkrest() ||
         markdefs[type].getcanspanone(((modobjbase*) note)->getnoteevbase());
  EXIT_API_0;
}
...
int module_markcanspanrests(int type, module_noteobj note) {
  ...
  assert(((modobjbase*) note)->isvalid());
  return ((modobjbase*) note)->getismarkrest() ||
         markdefs[type].getcanspanrests(((modobjbase*) note)->getnoteevbase());
  EXIT_API_0;
}
int module_markcanendonrests(int type, module_noteobj note) {
  ...
  assert(((modobjbase*) note)->isvalid());
  return ((modobjbase*) note)->getismarkrest() ||
         markdefs[type].getcanendonrests(((modobjbase*) note)->getnoteevbase());
  EXIT_API_0;
}
...
const char* module_pitchnametostr(struct module_value pitch,
                                  const int forceacc) {
  ENTER_API;
  try {
...
module_percinstobj
module_percinst(module_noteobj note) { // return NULL if no perc inst
  ENTER_API;
  try {
    assert(((modobjbase*) note)->isvalid());
...
const char*
module_percinststr(module_noteobj note) { // return NULL if no perc inst
  ENTER_API;
  try {
    assert(((modobjbase*) note)->isvalid());
...
void meas_assign(module_measobj meas, fomus_rat time, fomus_rat dur,
                 int rmable) {
  ENTER_API;
  try {
    assert(((modobjbase*) meas)->isvalid());
    ((modobjbase*) meas)
        ->assignmeas(
            time, dur,
            rmable); // meas must be a meausure when assignmeas is called
  } catch (const wrongtype& e) {
    wrongtypeerr(e);
    throw errbase();
  }
...
void tquant_assign_time(
    module_noteobj note, struct fomus_rat time,
    struct fomus_rat
        dur) { // fomus protects grace notes from having durations overwritten
  ENTER_API;
  try {
    assert(((modobjbase*) note)->isvalid());
...
void tquant_assign_gracetime(
    module_noteobj note, struct fomus_rat time, struct module_value grtime,
    struct module_value
        dur) { // fomus protects grace notes from having durations overwritten
  ENTER_API;
  try {
...
void tquant_delete(module_noteobj note) { // user might want notes deleted if
                                          // their durations quantize to 0
  ENTER_API;
  try {
    assert(((modobjbase*) note)->isvalid());
...
void special_assign_makeinv(
    module_noteobj note) { // user might want notes deleted if their durations
                           // quantize to 0
  ENTER_API;
  try {
    assert(((modobjbase*) note)->isvalid());
...
void special_assign_newnote(module_noteobj note, fomus_rat pitch,
                            fomus_rat acc1, fomus_rat acc2,
                            special_markslist marks) {
  ENTER_API;
  try {
    assert(((modobjbase*) note)->isvalid());
...
void divide_assign_gracesplit(module_measobj meas,
...
void parts_assigngroup(module_partobj begin, module_partobj end,
...
void marks_assign_add(module_noteobj note, int type, const char* arg1,
                      module_value arg2) {
  ENTER_API;
  try {
    assert(((modobjbase*) note)->isvalid());
...
void marks_assign_remove(module_noteobj note, int type, const char* arg1,
                         module_value arg2) {
  ENTER_API;
  try {
    assert(((modobjbase*) note)->isvalid());
...
void metaparts_assign(module_noteobj note, module_partobj part, int voice,
                      fomus_rat pitch) {
  ENTER_API;
  try {
    assert(((modobjbase*) note)->isvalid());
...
void tpose_assign_keysig(module_measobj meas,
...
void markevs_assign_add(module_partobj part, int voice, struct module_value off,
                        int type, const char* arg1, struct module_value arg2) {
  ENTER_API;
  try {
    assert(((modobjbase*) part)->isvalid());
...
fomus_rat module_strtonote(const char* str, module_noteparts* parts) {
  ...
  return val0.isnull() ? makerat(std::numeric_limits<fint>::max(), 1)
                       : numtofrat(val0);
  EXIT_API_RAT0;
}
...
struct fomus_rat module_makerat(fomus_int n, fomus_int d) {
  ENTER_API;
  struct fomus_rat r = {n, d};
  return r;
  EXIT_API_RAT0;
}
struct module_value module_vgracetiedendtime(module_obj obj) {
  ENTER_API;
  return module_vgraceendtime(obj);
  EXIT_API_MODVAL0;
}
struct fomus_rat module_gracetiedendtime(module_obj obj) {
  ENTER_API;
