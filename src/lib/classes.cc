...
namespace fomus {
  ...
  void marksbase::cachessort() {
    boost::ptr_vector<markobj>::iterator x1(std::lower_bound(
        RMUT(marks).begin(), RMUT(marks).end(), m_voicebegin,
        boost::lambda::bind(&markobj::getspantype, boost::lambda::_1) <
            boost::lambda::_2));
    assert(x1 >= RMUT(marks).begin() && x1 <= RMUT(marks).end());
    boost::ptr_vector<markobj>::const_iterator x2(std::lower_bound(
        x1, RMUT(marks).end(), m_voiceend,
        boost::lambda::bind(&markobj::getspantype, boost::lambda::_1) <
            boost::lambda::_2));
  ...
  const std::vector<std::pair<rat, rat>>& measure::getkeysigvect() const {
    ...
    if (sigs.userempty()) {
      READLOCK;
      return ((const var_keysigs&) get_varbase(KEYSIG_ID))
          .getkeysigvect(RMUT(newkey).empty() ? get_sval(COMMKEYSIG_ID)
                                              : RMUT(newkey));
    } else {
  ...
  bool measure::getwritaccaux(const noteevbase& ev, SHLOCKPAR) const {
    assert(ev.isrlocked());
    if (!ev.getisnote() || ev.getistiedleft_nomut())
      return false;
    if (RMUT(((const noteev&) ev).cautacc))
      return true;
    assert(accrules.find(ev.get_sval_nomut(ACCRULE_ID)) !=
           accrules.end()); // shouldn't ever happen
    accruleenum ru = accrules.find(ev.get_sval_nomut(ACCRULE_ID))->second;
    if (ru == accrule_notenats)
      return true;
    ...
    std::pair<rat, rat> accs;
    {
      ...
      DBG("The KEYSIG accidental for note " << no0 << " is " << accs.first
                                            << std::endl);
      if (ru == accrule_meas) {
        const noteev* invnote = 0;
        for (eventmap_constit e(CMUT(events).begin());
             e != CMUT(events).end() && e->second != &ev; ++e) {
          if (e->second->getisnote() &&
              numtoint(e->second->getwrittennote()) == no0 &&
              !e->second->getistiedleft()) { // same written note
            std::pair<rat, rat> ba(((noteev*) e->second)->getbothaccs());
            if (!invnote || ba != invnote->getbothaccs())
              accs = ba;
            if (e->second->isinvisible())
              invnote = ((noteev*) e->second);
            else
              invnote = 0;
          }
        }
    ...
    return (RMUT(((const noteev&) ev).acc1) != accs.first ||
            RMUT(((const noteev&) ev).acc2) != accs.second);
  }

#ifndef NDEBUG
  void part::resetstage() {
    std::for_each(getmeass().begin(), getmeass().end(),
                  boost::lambda::bind(&measure::resetstage,
                                      boost::lambda::bind(&measmap_val::second,
                                                          boost::lambda::_1)));
  }
#endif
#ifndef NDEBUGOUT
  void part::showstage() {
    std::for_each(getmeass().begin(), getmeass().end(),
                  boost::lambda::bind(&measure::showstage,
                                      boost::lambda::bind(&measmap_val::second,
                                                          boost::lambda::_1)));
  }
#endif
  ...
  void noteevbase::getclefsinit() {
    ...
    if (cc.val.l.n > 0) {
      ...
      for (module_value *i = cc.val.l.vals, *ie = cc.val.l.vals + cc.val.l.n;
           i < ie; ++i) {
      ...
      clefscache.reset(); // ok for clefscache to be 0, this means go to parent
                          // measure for clefs
    {
      WRITELOCK;
    ...
    if (l.val.l.n > 0) {
      ...
      WRITELOCK;
      for (int i = 1; i <= ns; ++i)
        WMUT(staves).push_back(i);
    }
  }

  void noteevbase::dopointl(const offgroff& o1, std::list<noteevbase*>& tmp,
                            UPLOCKPAR) {
    ...
    if (o1.groff < RMUT(off).groff) {
      WMUT(dur) =
          RMUT(off).groff - o1.groff; // off and groff are correct in this case
      tmp.push_back(this);
      return;
    }
    WMUT(dur) = get_rval_nomut(DEFAULTGRACEDUR_ID);
  }
  void noteevbase::dopointr(const offgroff& o2, std::list<noteevbase*>& tmp,
                            UPLOCKPAR) {
    assert(RMUT(off).groff.isntnull());
  ...
  void part::fixmeasures() {
    if (RMUT(newmeass).empty()) {
      CERR << "no measures";
      integerr("measure");
    }
    if (RMUT(newmeass).begin()->second->gettime() != (fint) 0) {
      CERR << "invalid time/duration";
      integerr("measure");
    }
    for (measmap_constit m(RMUT(newmeass).begin()),
         me(boost::prior(RMUT(newmeass).end()));
         m != me; ++m) {
      measmap_constit nm(boost::next(m));
      const measure& mea(*m->second);
      assert(mea.gettime().israt());
      assert(mea.getdur().israt());
      if (mea.gettime() < (fint) 0 || mea.getdur() <= (fint) 0 ||
          !mea.fixmeascheck()) {
        CERR << "invalid time/duration";
        integerr("measure");
      }
      if (mea.getendtime() != nm->second->gettime()) {
        CERR << "unaligned measures";
        integerr("measure");
      }
    }
    ...
    for (measmap_it m(CMUT(meass).begin()); m != CMUT(meass).end(); ++m) {
      ...
      for (eventmap_it n(mea.getevents().begin()), ne(mea.getevents().end());
           n != ne;) { // noteevent
        const numb& o(n->second->gettime_nomut());
        if (o < (fomus_int) 0 || n->second->getendtime_nomut() >
                                     en) { // en is very last measure endoffset
          CERR << "invalid note time/duration";
          integerr("measure");
        }
#ifndef NDEBUG
...
#else
        boost::prior(newmeass.upper_bound(o))
            ->second->insertnew(mea.getevents().release(n++).release());
#endif
      }
      assert(mea.getevents().empty());
    }
    XMUT(meass).clear();
    for (measmap::iterator i(RMUT(newmeass).begin());
         i != RMUT(newmeass).end();) {
  ...
  void noteevbase::fixtimequant(numb& mx) {
    if (!RMUT(off).off.israt() ||
        !(RMUT(off).groff.israt() || RMUT(off).groff.isnull()) ||
        !RMUT(dur).israt()) {
      CERR << "non-rational time/duration";
      integerr("time quantization");
    }
    ...
    if (RMUT(dur) == (fint) 0 &&
        RMUT(off).groff.isnull()) { // becomes a grace note
  ...
  void part::reinsert(std::auto_ptr<noteevbase>& e, const char* what) {
    ...
    if (i == CMUT(meass).begin() ||
        e->getendtime_nomut() >
            boost::prior(CMUT(meass).end())->second->getendtime()) {
      DBG("i == meass.begin() = " << (i == CMUT(meass).begin()) << std::endl);
      DBG("e->getendtime() = "
          << e->getendtime()
          << "  boost::prior(meass.end())->second->getendtime() = "
          << boost::prior(CMUT(meass).end())->second->getendtime()
          << std::endl);
      CERR << "invalid time/duration";
      integerr(what);
    ...
    assert(m.gettime() <= e->gettime_nomut() &&
           m.getendtime() >= e->gettime_nomut());
  ...
  void measure::fixtimequant(numb& mx, const bool inv) {
    DBG("fixtimequant: part=" << prt->getid() << " measure=" << gettime()
                              << std::endl);
    boost::ptr_list<noteevbase> tmp;
    for (eventmap_it i(CMUT(events).begin()); i != CMUT(events).end();) {
      if (inv && i->second->getvoice_nomut() < 1000)
        ++i;
      else
        tmp.push_back(XMUT(events).release(i++).release());
    }
    noteevbase* lne = 0;
    for (boost::ptr_list<noteevbase>::iterator e(tmp.begin());
         e != tmp.end();) {
  ...
  numb part::fixtimequant() { // return number of empty measures that can be
                              // trimmed off the end
    numb mx((fint) -1);
    for (measmap_it m(CMUT(meass).begin()); m != CMUT(meass).end(); ++m)
      m->second->fixtimequant(mx, false);
    assert(!CMUT(meass).empty());
    numb o(CMUT(meass).rbegin()->second->getendtime());
    for (measmap::const_reverse_iterator i(CMUT(meass).rbegin());
         i != CMUT(meass).rend(); ++i) {
      if (i->second->canremove() && mx <= i->first.off)
        o = i->first.off;
      else
        break;
    }
  ...
  marksbase::marksbase(MUTPARAM_ marksbase& x)
    ...
    while (!RMUT(x.marks).empty() &&
      ...
      CMUT(marks).push_back(WMUT(x.marks).pop_back().release());
    for (boost::ptr_vector<markobj>::iterator i(RMUT(x.marks).begin());
         i != RMUT(x.marks).end(); ++i) {
      assert(i->getmove() != move_right);
      if (i->getmove() == move_leftright)
        CMUT(marks).push_back(new markobj(*i));
    }
    WMUT(x.marks).sort(marksetlt());
    x.cachessort();
  ...
  void marksbase::unsplitmarks(marksbase& x) {
    WMUT(x.marks).sort(markslr()); // sort by left, leftright, right
    while (!RMUT(x.marks).empty() && RMUT(x.marks).back().getmove() < move_left)
      WMUT(marks).push_back(WMUT(x.marks).pop_back().release());
    WMUT(marks).sort(marksetlt()); /*DISABLEMUTCHECK;*/
    cachessort();
  }

  void restev::checkrestvoices(
      boost::ptr_list<noteevbase>& tmp) { // duplicate 1 rest for ea. voice
    UPREADLOCK;
    assert(RMUT(voices).size() > 0);
    for (std::vector<int>::iterator i(boost::next(RMUT(voices).begin()));
         i != RMUT(voices).end(); ++i) {
      tmp.push_back(new restev(*this, *i));
    }
    if (RMUT(voices).size() > 1) {
      TOWRITELOCK;
      WMUT(voices).resize(1);
    }
  }

  bool isspecial(const marksbase& x, const marksbase& y) {
    for (boost::ptr_vector<markobj>::const_iterator i(x.getmarkslst().begin());
         i != x.getmarkslst().end(); ++i) {
      for (boost::ptr_vector<markobj>::const_iterator j(
               y.getmarkslst().begin());
           j != y.getmarkslst().end(); ++j) {
        if (markisspecialpair(i->getmarkid(), j->getmarkid()))
          return true;
      }
    }
    return false;
  ...
  inline bool checkpruneaux(noteevbase& x, const noteevbase& y) {
    ...
    if (isspecial(x, y))
      return false;
    return x.get1voice() == y.get1voice() &&
           (x.getisrest() || y.getisrest() || x.getnote() == y.getnote()) &&
           ((x.isgrace() && y.isgrace())
                ? y.getfulltime_nomut() < x.getfullendtime() &&
                      x.getfulltime_nomut() < y.getfullendtime()
  ...
  void measure::fixties() {
    ...
    for (eventmap_it n(CMUT(events).begin()); n != CMUT(events).end();) {
      ...
      boost::ptr_map<const rat, boost::nullable<divsplitstr>>& spl =
          ev->getsplits();
      boost::ptr_map<const rat, boost::nullable<divsplitstr>>::const_iterator i(
          spl.begin());
      if (i != spl.end()) { // ev doesn't need to be locked here
        rat et(numtorat(ev->isgrace(MUTDBG) ? ev->getgraceendtimenochk(MUTDBG)
                                            : ev->getendtime_nomut(MUTDBG)));
        if (i->first != numtorat(ev->isgrace(MUTDBG)
                                     ? ev->getgracetime_nomut(MUTDBG)
                                     : ev->gettime_nomut(MUTDBG))) {
          CERR << "bad split";
          integerr("note division");
        }
        while (true) {
          assert(i->first == numtorat(ev->isgrace(MUTDBG)
                                          ? ev->getgracetime_nomut(MUTDBG)
                                          : ev->gettime_nomut(MUTDBG)));
          if (i->second)
            ev->filltups(i->second->tups _MUTDBG);
          DBG((ev->gettupsvect(MUTDBG).empty() ? false
                                               : ev->gettupsvect(MUTDBG)[0].beg)
              << '/'
              << (ev->gettupsvect(MUTDBG).empty()
                      ? false
                      : ev->gettupsvect(MUTDBG)[0].end)
              << ' ');
          if (++i == spl.end())
            break;
          if (i->first >= et) {
          ...
          DISABLEMUTCHECK;
          tmp.push_back(ev = ev->getsplitat0(
                            i->first)); // getsplitat0 doesn't split the tuplets
        }
        spl.clear();
      }
    }
    DBG(std::endl);
    for (boost::ptr_list<noteevbase>::iterator i(tmp.begin());
         i != tmp.end();) {
    ...
    for (boost::ptr_list<noteevbase>::iterator i(tmp.begin()); i != tmp.end();
         ++i) {
      DBG((i->gettupsvect(MUTDBG).empty() ? false
                                          : i->gettupsvect(MUTDBG)[0].beg)
          << '/'
          << (i->gettupsvect(MUTDBG).empty() ? false
                                             : i->gettupsvect(MUTDBG)[0].end)
          << ' ');
    }
#endif
    DBG(std::endl);
    for (boost::ptr_list<noteevbase>::iterator i(tmp.begin()); i != tmp.end();)
      insertnew(tmp.release(i++).release());
  }
    ...
    for (eventmap_it n(CMUT(events).begin()); n != CMUT(events).end();) {
      if (n->second->getclips().get()) {
        std::auto_ptr<std::set<clippair>> clips(
            n->second->getclips()); // steal it first--clips represent
                                    // begins/ends of NEW notes
        ...
        if (!clips->empty()) {
          ...
          if (no->isgrace(MUTDBG)) {
            if (ie->o2 != no->getgraceendtimenochk(MUTDBG))
              no->adjustdur(ie->o2 - no->getgracetime_nomut(MUTDBG));
          } else {
            if (ie->o2 != no->getendtime_nomut(MUTDBG))
              no->adjustdur(ie->o2 - no->gettime() _MUTDBG);
          }
          std::set<clippair>::const_iterator i(clips->begin());
          DBG("i->o1 = " << numb(i->o1) << std::endl);
          while (true) {
            if (i->o1 < (no->isgrace(MUTDBG) ? no->getgracetime_nomut(MUTDBG)
                                             : no->gettime_nomut(MUTDBG)) ||
                i->o1 >= i->o2) {
              CERR << "bad prune times";
              integerr("prune");
            } // is in range?
            if (no->isgrace(MUTDBG)) {
              if (i->o1 > no->getgracetime_nomut(MUTDBG))
                no->adjustgracetime(i->o1 _MUTDBG);
            } else {
              if (i->o1 > no->gettime_nomut(MUTDBG))
                no->adjusttime(i->o1 _MUTDBG);
            }
            if (i == ie)
              break;
  ...
  void measure::getkeysig_init() {
    ...
    for (std::vector<std::pair<rat, rat>>::const_iterator i(x.begin());
         i != x.end(); ++i, ++j) {
  ...
  void
  marksbase::spreadreplmarks(const boost::ptr_set<markobj, marksetlt>& mrks) {
    assert(!RMUT(newm));
    for (boost::ptr_set<markobj, marksetlt>::const_iterator i(mrks.begin());
         i != mrks.end(); ++i)
      WMUT(marks).push_back(new markobj(*i));
    WMUT(marks).sort(marksetlt());
    cachessort();
  ...
  struct haspart : public std::unary_function<const module_value&, bool> {
    ...
    bool operator()(const module_value& x) const {
      assert(x.type == module_string);
      return boost::algorithm::iequals(id, x.val.s);
    }
  };
  void
  part::postinput(std::vector<makemeas>& makemeass, const numb& trun1,
                  const numb& trun2) { // notes need to go to measures for .fms
                                       // output (and any other preproc output)
    assert(!CMUT(meass).empty());
    for (std::vector<makemeas>::iterator i(makemeass.begin());
         i != makemeass.end(); ++i) {
      if (i->off < trun1 || (trun2 > (fint) 0 && i->off >= trun2))
        continue;
      numb of(i->off - trun1);
      std::auto_ptr<measure> m(new measure(of, i->dur, i->measdef, *def));
      module_value pl(m->get_lval(PARTS_ID));
      assert(pl.type == module_list);
      if (pl.val.l.n <= 0 || hassome(pl.val.l.vals, pl.val.l.vals + pl.val.l.n,
                                     haspart(getid()))) {
        DBG("INSERTING MEASURE at " << of << " dur " << i->dur << " INTO PART "
                                    << getid() << std::endl);
        XMUT(meass).erase(of);
    ...
    def->cachesinit();
    std::for_each(getmeass().begin(), getmeass().end(),
                  boost::lambda::bind(&measure::getkeysig_init,
                                      boost::lambda::bind(&measmap_val::second,
                                                          boost::lambda::_1)));
    postinput2(trun1, trun2);
  }

  void part::postinput2(
      const numb& trun1,
      const numb& trun2) { // is also called after distributing mparts
    assert(isvalid());
    ...
    {
      WRITELOCK;
      for (boost::ptr_list<noteevbase>::iterator i(RMUT(tmpevs).begin());
           i != RMUT(tmpevs).end();) {
        if ((i->gettime() < trun1) ||
            (trun2 > (fint) 0 && i->gettime() >= trun2)) {
    ...
    std::for_each(getmeass().begin(), getmeass().end(),
                  boost::lambda::bind(&measure::initclefscaches,
                                      boost::lambda::bind(&measmap_val::second,
                                                          boost::lambda::_1)));
  }
  ...
  void part::postinput3() { // is also called after distributing mparts
...
#endif
    {
      for (boost::ptr_list<noteevbase>::iterator i(RMUT(tmpevs).begin());
           i != RMUT(tmpevs).end();) {
        std::auto_ptr<fomus::noteevbase> ne(
            WMUT(tmpevs).release(i++).release());
  ...
  void measure::removerest(noteevbase& n) {
    ...
    bool fi = true;
    for (eventmap_it i(CMUT(events).lower_bound(t));
         i != CMUT(events).end() && i->second->getfulltime_nomut() <= t;) {
      if (i->second->get1voice() == n.get1voice()) {
        if (fi) {
          n.replacetups(*i->second);
          fi = false;
        }
  ...
  void measure::reinsert() {
    ...
    for (std::vector<eventmap_it>::iterator i(tmp.begin()); i != tmp.end();
         ++i) {
  ...
  void noteevbase::domerge() {
    assert(isntlocked());
    {
      ...
      if (RMUT(mergeto)) {
        ...
        meas->insertnew(new restev(gettime(), getgracetime(), getdur(), v,
                                   get1staff(), pos, RMUT(marksbase::marks)));
      }
    }
    assert(isntlocked());
    post_apisetvalue();
  ...
  void part::mergefrom(part& x, const numb& shift) {
    ...
    for (boost::ptr_list<noteevbase>::iterator i(RMUT(x.tmpevs).begin());
         i != RMUT(x.tmpevs).end(); ++i)
      CMUT(tmpevs).push_back(i->fomclone(shift));
    for (boost::ptr_vector<modobjbase>::iterator i(CMUT(x.markevs).begin());
         i != CMUT(x.markevs).end(); ++i)
      CMUT(markevs).push_back(((markev&) *i).fomclone(shift));
  }

  bool measure::getisbeginchord(eventmap_it i, const offgroff& ti, const int v)
      const { // beginning in arbitrary eventmap ordering only!
    while (i != CMUT(events).begin()) {
      --i;
      bool bl;
      if (i->second->beginchordaux(bl, ti, v))
        return bl;
    };
    return true;
  }
  bool measure::getisendchord(eventmap_it i, const offgroff& ti, const int v)
      const { // end in arbitrary eventmap ordering only!
    ++i;
    for (; i != CMUT(events).end(); ++i) {
      bool bl;
      if (i->second->endchordaux(bl, ti, v))
        return bl;
    };
    return true;
  }
  bool measure::getischordlow(const eventmap_it& i0, const offgroff& ti,
                              const int v,
                              const rat& n) const { // is it the lowest note?
    eventmap_it i(i0);
    while (i != CMUT(events).begin()) {
      --i;
      bool bl;
      if (i->second->lowchordaux1(bl, ti, v, n)) {
        if (bl)
          break;
        else
          return false;
    ...
    for (eventmap_it i(boost::next(i0)); i != CMUT(events).end(); ++i) {
      bool bl;
      if (i->second->lowchordaux2(bl, ti, v, n))
        return bl;
    };
    return true;
  }
  bool measure::getischordhigh(const eventmap_it& i0, const offgroff& ti,
                               const int v,
                               const rat& n) const { // is it the highest note?
    eventmap_it i(i0);
    while (i != CMUT(events).begin()) {
      --i;
      bool bl;
      if (i->second->highchordaux1(bl, ti, v, n)) {
        if (bl)
          break;
        else
          return false;
    ...
    for (eventmap_it i(boost::next(i0)); i != CMUT(events).end(); ++i) {
      bool bl;
      if (i->second->highchordaux2(bl, ti, v, n))
        return bl;
    };
    return true;
  }

  bool measure::getoctavebegin(eventmap_it ev, const offgroff& ti,
                               const int st) const {
    while (true) {
      if (ev == CMUT(events).begin())
        return prt->getoctavebegin(self, ti, st);
      --ev;
      if (!(ev->first < ti))
        continue;
      bool bl;
      if (ev->second->getoctaux(bl, st))
        return bl;
    }
  }
  bool part::getoctavebegin(measmap_it me, const offgroff& ti,
                            const int st) const {
    do {
      if (me == CMUT(meass).begin())
        return true;
      --me;
    } while (me->second->getevents().empty());
    return me->second->getoctavebegin(
        boost::prior(me->second->getevents().end()), ti, st);
  }
  bool measure::getoctaveend(eventmap_it ev, const offgroff& ti,
                             const int st) const {
    while (true) {
      ++ev;
      if (ev == CMUT(events).end())
        return prt->getoctaveend(self, ti, st);
      if (!(ev->first > ti))
        continue;
      bool bl;
      if (ev->second->getoctaux(bl, st))
  ...
  bool part::getoctaveend(measmap_it me, const offgroff& ti,
                          const int st) const {
    do {
      ++me;
      if (me == CMUT(meass).end())
        return true;
    } while (me->second->getevents().empty());
    return me->second->getoctavebegin(me->second->getevents().begin(), ti, st);
  }

  int measure::getvoiceinstaff(const noteevbase& no, const offgroff& ti,
                               const int vo,
                               const int st) const { // is it the highest note?
    bool vgr = false;
    ...
    while (i != CMUT(events).begin()) {
      --i;
      bool bl;
      if (i->second->voiceinstaffaux1(bl, ti, vo, st, bf, vgr)) {
        if (bl)
          break;
        else
          continue;
    ...
    for (eventmap_it i(boost::next(no.getself())); i != CMUT(events).end();
         ++i) {
      bool bl;
      if (i->second->voiceinstaffaux2(bl, ti, vo, st, bf, vgr)) {
        if (bl)
          break;
        else
          continue;
  ...
  const module_value& event::get_lval0(const int id, const bool nomut) const {
    ...
    if (i != sets.end())
      return i->second->getmodval();
    if (!meas)
      return fom_get_lval_up(id);
    const module_value* ret;
    if (meas->get_lval0(id, ret))
      return *ret;
    if (nomut) {
      if (RMUT(clf) && RMUT(clf)->get_lval0(id, ret))
        return *ret;
      if (RMUT(stf) && RMUT(stf)->get_lval0(id, ret))
        return *ret;
      return meas->get_lval_up(id, *this);
    } else {
      {
        READLOCK;
        if (RMUT(clf) && UNLOCKP(RMUT(clf))->get_lval0(id, ret))
          return *ret;
      }
      {
        READLOCK;
        if (RMUT(stf) && UNLOCKP(RMUT(stf))->get_lval0(id, ret))
          return *ret;
      }
      {
        READLOCK;
  ...
  const std::string& event::get_sval0(const int id, const bool nomut) const {
    ...
    if (meas->get_sval0(id, ret))
      return *ret;
    if (nomut) {
      if (RMUT(clf) && RMUT(clf)->get_sval0(id, ret))
        return *ret;
      if (RMUT(stf) && RMUT(stf)->get_sval0(id, ret))
        return *ret;
      return meas->get_sval_up(id, *this);
    } else {
      {
        READLOCK;
        if (RMUT(clf) && UNLOCKP(RMUT(clf))->get_sval0(id, ret))
          return *ret;
      }
      {
        READLOCK;
        if (RMUT(stf) && UNLOCKP(RMUT(stf))->get_sval0(id, ret))
          return *ret;
      }
      {
        READLOCK;
  ...
  ffloat event::get_fval0(const int id, const bool nomut) const {
    ...
    ffloat ret;
    if (meas->get_fval0(id, ret))
      return ret;
    if (nomut) {
      if (RMUT(clf) && RMUT(clf)->get_fval0(id, ret))
        return ret;
      if (RMUT(stf) && RMUT(stf)->get_fval0(id, ret))
        return ret;
      return meas->get_fval_up(id, *this);
    } else {
      {
        READLOCK;
        if (RMUT(clf) && UNLOCKP(RMUT(clf))->get_fval0(id, ret))
          return ret;
      }
      {
        READLOCK;
        if (RMUT(stf) && UNLOCKP(RMUT(stf))->get_fval0(id, ret))
          return ret;
      }
      {
        READLOCK;
  ...
  rat event::get_rval0(const int id, const bool nomut) const {
    ...
    rat ret;
    if (meas->get_rval0(id, ret))
      return ret;
    if (nomut) {
      if (RMUT(clf) && RMUT(clf)->get_rval0(id, ret))
        return ret;
      if (RMUT(stf) && RMUT(stf)->get_rval0(id, ret))
        return ret;
      return meas->get_rval_up(id, *this);
    } else {
      {
        READLOCK;
        if (RMUT(clf) && UNLOCKP(RMUT(clf))->get_rval0(id, ret))
          return ret;
      }
      {
        READLOCK;
        if (RMUT(stf) && UNLOCKP(RMUT(stf))->get_rval0(id, ret))
          return ret;
      }
      {
        READLOCK;
  ...
  fint event::get_ival0(const int id, const bool nomut) const {
    ...
    fint ret;
    if (meas->get_ival0(id, ret))
      return ret; // measure first
    if (nomut) {
      if (RMUT(clf) && RMUT(clf)->get_ival0(id, ret))
        return ret;
      if (RMUT(stf) && RMUT(stf)->get_ival0(id, ret))
        return ret;
      return meas->get_ival_up(id, *this); // go up to part
    } else {
      {
        READLOCK;
        if (RMUT(clf) && UNLOCKP(RMUT(clf))->get_ival0(id, ret))
          return ret;
      }
      {
        READLOCK;
        if (RMUT(stf) && UNLOCKP(RMUT(stf))->get_ival0(id, ret))
          return ret;
      }
      {
        READLOCK;
  ...
  const varbase& event::get_varbase0(const int id, const bool nomut) const {
    ...
    const varbase* ret;
    if (meas->get_varbase0(id, ret))
      return *ret; // measure first
    if (nomut) {
      if (RMUT(clf) && RMUT(clf)->get_varbase0(id, ret))
        return *ret;
      if (RMUT(stf) && RMUT(stf)->get_varbase0(id, ret))
        return *ret;
      return meas->get_varbase_up(id, *this); // go up to part
    } else {
      {
        READLOCK;
        if (RMUT(clf) && UNLOCKP(RMUT(clf))->get_varbase0(id, ret))
          return *ret;
      }
      {
        READLOCK;
        if (RMUT(stf) && UNLOCKP(RMUT(stf))->get_varbase0(id, ret))
          return *ret;
      }
      {
        READLOCK;
  ...
  int measure::getconnbeamsleft(eventmap_constit i, const offgroff& ti,
                                const int vo, const int bl) const {
    while (i != CMUT(events).begin()) {
      --i;
      int nb;
      if (i->first < ti && i->second->connbeamsleftaux(nb, vo))
        return (nb < bl ? nb : bl);
    }
    return bl;
  }

  int measure::getconnbeamsright(eventmap_constit i, const offgroff& ti,
                                 const int vo, const int br) const {
    ++i;
    while (i != CMUT(events).end()) {
      int nb;
      if (i->first > ti && i->second->connbeamsrightaux(nb, vo))
        return (nb < br ? nb : br);
      ++i;
    }
    return br;
  ...
  fomus_rat measure::timesig() const {
    ...
    if (c)
      b = rat(b.numerator() * 3, b.denominator() * 2);
    ...
    assert(tss.islist());
    for (const module_value *i(tss.val.l.vals),
         *ie(tss.val.l.vals + tss.val.l.n);
         i < ie; ++i) {
  ...
  void measure::assigndetmark0(numb off0, const int voice, const int type,
                               const char* arg1,
                               const struct module_value& arg2) {
    DBG("**off0=" << off0 << " measoff=" << RMUT(off).off << std::endl);
    if (off0 < RMUT(off).off)
      off0 = RMUT(off).off;
    numb eo(getendoff());
    if (off0 > eo)
      off0 = eo;
    ...
    for (boost::ptr_multimap<const numb, noteevbase>::iterator i(
             RMUT(newevents).lower_bound(off0));
         i != RMUT(newevents).end() && i->first <= off0; ++i) {
      DISABLEMUTCHECK;
    ...
    TOWRITELOCK;
    if (off0 >= eo) {
      DBG("**sticking in grace note at offset " << eo << ", dur 0, voice "
                                                << voice << std::endl);
      WMUT(newevents).insert(
          eo, x = new restev(
                  eo, (fomus_int) 2000, get_rval(DEFAULTGRACEDUR_ID), voice,
                  filepos("(internal)"))); // grace note... don't use max/2 here
                                           // to avoid later math errors
      assert(x->isvalid());
    } else { // normal note
      DBG("**sticking in non-grace note at offset " << off0 << ", dur 0, voice "
                                                    << voice << std::endl);
      WMUT(newevents).insert(
          off0, x = new restev(
                    off0, std::numeric_limits<fint>::max() / 2 /*module_none*/,
                    (fomus_int) 0, voice, filepos("(internal)")));
      assert(x->isvalid());
  ...
  void measure::dopostmarkevs() {
    for (boost::ptr_multimap<const numb, noteevbase>::iterator i(
      ...
      if (!e->getisgrace_nomut(MUTDBG)) {
        boost::ptr_multimap<const numb, noteevbase>::const_iterator j(
            RMUT(newevents).upper_bound(e->gettime_nomut(MUTDBG)));
        e->setdur((j == RMUT(newevents).end() ? getendoff() : j->first) -
                  e->gettime_nomut(MUTDBG) _MUTDBG);
      }
      e->recachemarks0();
      e->getclefsinit();
    }
  ...
  void restev::checkstaves2() {
    {
      ...
      DBG("checkstaves2, voice = " << get1voice() << "  staff = " << get1staff()
                                   << std::endl);
  ...
  void measure::getrestmarkstaffclef(restev& ev, UPLOCKPAR) {
    ...
    if (v >= 3000) {
      TOWRITELOCK;
      ((stavesbase&) ev).assign(prt->getnstaves());
    } else if (v >= 2000) {
      TOWRITELOCK;
      ((stavesbase&) ev).assign(1);
    } else {
      v %= 1000;
      assert(v > 0 && v <= 128);
      ...
      {
        UNLOCKUPREAD;
        for (eventmap_it i(CMUT(events).begin());
             i != CMUT(events).end() && (*i)->first < et; ++i) {
          if (i->second->matchesvoiceetime(v, ti, st, cl))
            goto GOTIT;
        }
        return;
      }
  ...
  inline bool noteevbase::lastgrendoffaux(const offgroff& off, numb& ret,
                                          const int v) const {
    READLOCK;
    if (get1voice() != v)
      return false;
  ...
  numb measure::getlastgrendoff(const numb& off, const int v) const {
    ...
    for (eventmap_constit i(CMUT(events).lower_bound(off));
         i != CMUT(events).end(); ++i) {
      if (i->second->lastgrendoffaux(off, ret, v))
        break;
    }
    return (ret <= -std::numeric_limits<ffloat>::max()) ? (fint) 0 : ret;
  }

  inline bool noteevbase::firstgrendoffaux(const offgroff& off, numb& ret,
                                           const int v) const {
    READLOCK;
    if (get1voice() != v)
      return false;
  ...
  numb measure::getfirstgroff(const numb& off, const int v) const {
    ...
    for (eventmap_constit i(CMUT(events).lower_bound(off));
         i != CMUT(events).end(); ++i) {
      if (i->second->firstgrendoffaux(off, ret, v))
        break;
    }
    return ret;
  }
  ...
  void measure::checkoct(eventmap_it e, const offgroff& ti, const int st,
                         const int os) {
  ...
  void measure::postspecial() {
    for (eventmap::iterator i(CMUT(events).begin()); i != CMUT(events).end();) {
      if (i->second->getisnote() && i->second->isassdel())
        XMUT(events).erase(i++);
      else
        ++i;
    }
  ...
  void marksbase::spreadgetmarks(boost::ptr_set<markobj, marksetlt>& mrks) {
    ...
    while (!RMUT(marks).empty() && !RMUT(marks).back().getdontspread()) {
      DBG("spread id=" << RMUT(marks).back().getmarkid() << " dontspread="
                       << RMUT(marks).back().getdontspread() << std::endl);
  ...
  bool noteevbase::isntlocked() const {
    if (threadfd.get()->get_ival(NTHREADS_ID) > 1)
      return true;
    if (!lockcheck.get())
      return true;
  ...
  void marksbase::remtremmarks() {
    movetoold(); // actually copies to new
    boost::ptr_set<markobj>::iterator i(RMUT(nmarks).lower_bound(
        markobj(mark_trem, 0, numb(-std::numeric_limits<ffloat>::max()))));
    if (i != RMUT(nmarks).end() && i->getmarkid() == mark_trem)
      WMUT(nmarks).erase(i);
    i = RMUT(nmarks).lower_bound(
        markobj(mark_trem2, 0, numb(-std::numeric_limits<ffloat>::max())));
    if (i != RMUT(nmarks).end() && i->getmarkid() == mark_trem2)
      WMUT(nmarks).erase(i);
    recachemarks();
  }
  void marksbase::switchtrems(const module_markids from,
                              const module_markids to) {
    movetoold(); // actually copies to new
    boost::ptr_set<markobj>::iterator i(RMUT(nmarks).lower_bound(
        markobj(from, 0, numb(-std::numeric_limits<ffloat>::max()))));
  ...
  void noteev::trem_fix(const numb& chop, WRLOCKPAR) {
    ...
    // destroytiedr();
    if (chop.isntnull()) {
      noteevbase* s = getsplitatt(gettime_nomut() +
                                  chop); // use getsplitat (no t) to retain ties
      {
        DISABLEMUTCHECK;
        s->remtremmarks();
      }
  ...
  void noteev::checkacc1(const fomusdata* fom) {
    {
      UPREADLOCK;
      if (RMUT(acc1) < (fint) -128 || RMUT(acc1) > (fint) 128 ||
          RMUT(acc2) < (fint) -128 || RMUT(acc2) > (fint) 128) {
        const listelvect& v(get_varbase_nomut(ACC_ID).getvectval());
        if (v.size() == 1) {
          module_noteparts parts;
          parts.acc1 = parts.acc2 = module_makerat(0, 1);
          numb val0(doparseacc(fom, listel_getstring(v.front()).c_str(), true,
                               &parts));
          if (val0.isntnull()) {
            TOWRITELOCK;
        ...
        CHECKERR:
          ...
          integerr("accidentals"); // found during accidental integrity check"
                                   // << std::endl;
        }
      }
      ...
      if (RMUT(tiedr)) {
        const rat& a1(RMUT(acc1));
        const rat& a2(RMUT(acc2));
  ...
  void part::preprocess(const numb& te, const std::string& ts) {
    ...
    if (te > (fint) 0) {
      ...
      setmap ss;
      ss.insert(
          setmap_val(DETACH_ID, boost::shared_ptr<varbase>(new var_detach(1))));
      insertnewmarkev(new markev((fint) 0, (fint) 0, (fint) 0, std::set<int>(),
                                 filepos("(internal)"), mm, point_none, ss));
    }
  }
  ...
  void noteev::dumpall() const {
    DBG(this);
    DBG("    off=" << CMUT(off).off << "  groff=" << CMUT(off).groff
                   << "  dur=" << CMUT(dur) << "  note=");
    if (CMUT(percname))
      DBG(CMUT(percname));
    else {
    ...
    if (!CMUT(voices).empty()) {
      for (std::vector<int>::const_iterator i(CMUT(voices).begin()),
           ie(boost::prior(CMUT(voices).end()));
           i != CMUT(voices).end(); ++i) {
        DBG(*i);
        if (i != ie)
          DBG(' ');
    ...
    DBG(")  tups=(");
    if (!CMUT(tups).empty()) {
      for (std::vector<tupstruct>::const_iterator i(CMUT(tups).begin()),
           ie(boost::prior(CMUT(tups).end()));
           i != CMUT(tups).end(); ++i) {
        DBG("b=" << i->beg << ",e=" << i->end << ",u=" << i->tup.num << '/'
                 << i->tup.den);
        if (i != ie)
          DBG(' ');
      }
    ...
    if (!CMUT(staves).empty()) {
      for (std::vector<int>::const_iterator i(CMUT(staves).begin()),
           ie(boost::prior(CMUT(staves).end()));
           i != CMUT(staves).end(); ++i) {
        DBG(*i);
        if (i != ie)
          DBG(' ');
  ...
  void measure::dumpall() const {
    DBG("  PART=" << prt->getid() << "  MEASURE=" << CMUT(off).off
                  << "  DURATION=" << CMUT(dur) << std::endl);
    DBG("           KEYSIG= " << keysigcache.mode << ' ' << keysigcache.dianote
                              << ' ' << keysigcache.acc << ' ' << keysigcache.n
                              << std::endl);
    DBG("           DIVS= ");
    for (std::vector<fomus_rat>::const_iterator i(CMUT(divs).begin());
         i != CMUT(divs).end(); ++i)
      DBG(numb(*i) << " ");
    DBG(std::endl);
    for (eventmap_constit i(CMUT(events).begin()); i != CMUT(events).end(); ++i)
      i->second->dumpall();
  }
#endif
  ...
  void part::dumpall() const {
    ...
    for (std::set<int>::const_iterator i(CMUT(egroups).begin());
         i != CMUT(egroups).end(); ++i) {
      DBG(*i << ' ');
    }
    DBG(std::endl);
  ...
  void durbase::unsplittups(const durbase& rt) {
    if (RMUT(tups).size() != RMUT(rt.tups).size())
      goto UNSPLITERR;
    {
      std::vector<tupstruct>::const_iterator j(RMUT(rt.tups).begin());
      for (std::vector<tupstruct>::iterator i(RMUT(tups).begin());
           i != RMUT(tups).end(); ++i, ++j) {
        if (i->end || j->beg)
          goto UNSPLITERR;
        i->end = j->end;
