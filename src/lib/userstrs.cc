...
namespace fomus {
  ...
  const std::string clefstostrs[] =
    {"subbass-8down",
     ...
     "treble-8up",
     "percussion"};
  const int clefmidpitches[] =
    {35, 38, 41, 41, 45, 47, 48, 50, 52, 53, 53, 55, 57, 59, 59, 60, 62, 64, 65, 65, 67, 69, 71, 72, 76, 79, 83, 60};
...
#define NSETLOCS ((int)(sizeof(setlocstostrs) / sizeof(const std::string)))
  ...
  void initstrenummaps() {
    for (int i = 0; i < clef_nclefs; ++i) strstoclefs.insert(strtoclefmap_val(clefstostrs[i], i));
    std::ostringstream out;
    int ie = clef_nclefs - 1;
    for (int i = 0; i < ie; ++i) out << clefstostrs[i] << '|';
    out << clefstostrs[ie];
    clefstypestr = out.str();
    assert(NMODS == module_modspecial + 1);
    for (int i = 0; i < NMODS; ++i) strtomodtypes.insert(modtypemap_val(modtypestostrs[i], (enum module_type)i));
    assert(NSETLOCS == module_locnote + 1);
    for (int i = 0; i < NSETLOCS; ++i) strtosetlocs.insert(setlocmap_val(setlocstostrs[i], (module_setting_loc)i));
  }
...
const char* module_cleftostr(int en) {
  ...
  return cleftostr(en).c_str(); // assuming this is in range, since this is an enum
  EXIT_API_0;
}
