...
namespace parts {
  ...
  inline bool operator<(const parthold& x, const parthold& y) {return x.ind < y.ind;}
  ...
  public:
    ...
    void inner(module_value* &e1, const parts_grouptype g, module_partobj& fip0, module_partobj& rep0);
  };

  void partsdata::inner(module_value* &e, const parts_grouptype g, module_partobj& fip0, module_partobj& rep0) { // ord begins at int_min--so assigned parts just go to bottom
	...
	if (beg != strtogroupb.end()) {
	  ...
	  if (en != strtogroupe.end()) {
	    if (fip) {
	      if (en->second == g) {
		if (fip != rep || module_setting_ival(fip, groupsingid)) parts_assigngroup(fip, rep, g);
		rep0 = rep;
		if (!fip0) fip0 = fip;
	      } else barf();
	    }
	  ...
	  std::multimap<const std::string, parthold, isiless>::iterator i(pts.lower_bound(e->val.s));
	  std::set<parthold> pp;
	  while (i != pts.end() && boost::algorithm::iequals(e->val.s, i->first)) {
	    pp.insert(i->second);
	    pts.erase(i++);
	  }
  ...
  void partsdata::run_fun(FOMUS fom) {
	...
	pts.insert(std::multimap<const std::string, parthold, isiless>::value_type(module_id(module_inst(p)), parthold(p, ind++)));
      }
	...
	for (module_value *i = lst.vals, *ie = lst.vals + lst.n; i < ie; i += 2) {
	  if (ens == GET_S(*i)) {
  ...
  const char* err_partfun(void* moddata) {return ((partsdata*)moddata)->err_fun();}
  void run_partfun(FOMUS f, void* moddata) {((partsdata*)moddata)->run_fun(f);} // return true when done

  const char* ensemtype = "string";

  // const char* valid_def_type = "";
  // int valid_def(struct module_value val) {module_valid_listofstrings(val, -1, -1, -1, -1, 0, valid_def_type);}
...
const char* module_longname() {return "Parts";}
const char* module_author() {return "(fomus)";}
const char* module_doc() {return "Groups staves and arranges the order of parts in the score.";}
void* module_newdata(FOMUS f) {return new partsdata;}
void module_freedata(void* dat) {delete (partsdata*)dat;}
int module_priority() {return 0;}
enum module_type module_type() {return module_modparts;}
  ...
  case 0:
    {
      ...
      set->descdoc = "A mapping from ids to part layout definitions."
	"  Layout definitions inform FOMUS on how to order and group the parts in a score."
	"  Each one consists of instrument IDs specified in the order that they should appear in the score."
	"  \"[\" and \"]\" strings should surround instruments that are grouped together (i.e., with a bracket) and can be nested to any depth."
	"  \"{\" and \"}\" are used to group instruments with a brace (usually this isn't needed--parts with multiple staves automatically get a brace)."
	"  \"<\"  and \">\" are used for groups that aren't connected with a vertical bar line at each measure boundary."
	"  A \"*\" before a top-level group (i.e., before a family of instruments such as woodwinds or strings) indicates a location where system-wide markings such as Tempo should appear."
	"  Layout definitions should contain any instruments that you might use--instruments that appear in the definition but aren't in a score are ignored."
	"  A good place to alter this setting is in your `.fomus' configuration file."
	"  To define a layout specifically for a score, use `layout-def' instead.";
  ...
  case 1:
    {
      ...
      set->descdoc = "A string identifying a part layout."
	"  Set this to affect how parts are ordered and grouped in the score."
	"  The value of this setting must be the ID of one of the part layouts defined in `layout-defs'."
	//"  FOMUS has two predefined layouts that you can use: `small-ensemble' and `orchestra'."
	;
  ...
  case 2:
    {
      ...
      set->descdoc = "A single part layout definition.  "
	//"If this is not a zero-length list, this definition is used instead of any predefined ones in setting `layout-defs'"
	"Set this to define a layout and override the `layout' setting."
	"  Part layout definitions inform FOMUS on how to order and group the parts in a score."
	"  It consists of a list of instrument IDs specified in the order that they should appear in the score."
	"  \"[\" and \"]\" strings should surround instruments that are grouped together (i.e., with a bracket) and can be nested to any depth."
	"  \"{\" and \"}\" are used to group instruments with a brace (usually this isn't needed--parts with multiple staves automatically get a brace)."
	"  \"<\"  and \">\" are used for groups that aren't connected with a vertical bar line at each measure boundary."
	"  A \"*\" before a top-level group (i.e., before a family of instruments such as woodwinds or strings) indicates a location where system-wide markings such as Tempo should appear."
  ...
  case 3:
    {
      ...
      set->descdoc = "Indicates whether or not an instrument or part can be the only one in a group with a bracket around it.  "
	"Set this to `yes' in an instrument or part to cause a bracket to always appear, even around a single part (that is, if the parts layout indicates that there should be a bracket).";
      //set->typedoc = ensemtype; 
