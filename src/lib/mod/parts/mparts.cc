...
namespace mparts {
  ...
  struct cirscope {
    ...
    cirscope(const module_partobj p, std::set<module_partobj>& cir):cir(cir), i(cir.insert(p).first) {}
    ~cirscope() {cir.erase(i);}
  };
  void dumpstuff(const module_partobj p, const module_noteobj n, std::set<module_partobj>& cir, const int v0, const fomus_rat& no) {
    ...
    if (cir.find(p) != cir.end()) return;
    cirscope xxx(p, cir);
    metaparts_partmaps m(metaparts_getpartmaps(p));
    for (const metaparts_partmapobj* i(m.partmaps), *ie(m.partmaps + m.n); i < ie; ++i) {
	...
	if (!f || f == v0) {
	  if (ism) {
	    dumpstuff(np, n, cir, t, (no >= (fomus_int)0 ? no + tpos : module_makerat(-1, 1)));
	  } else { // got to a real part
	    metaparts_assign(n, np, t, (no >= (fomus_int)0 ? no + tpos : module_makerat(-1, 1)));
	  }
	}
      }
    }
  ...
  void mparts_run_fun(FOMUS fom, void* moddata) {
	...
	while (n0 && module_part(n0) == p) {
	  std::set<module_partobj> cir;
	  dumpstuff(p, n0, cir, module_voice(n0), (module_isrest(n0) ? module_makerat(-1, 1) : module_pitch(n0)));
	  //metaparts_assign_done(n);
	  n0 = module_peeknextnote(n0);
	}
	module_objlist mm0(module_getmarkevlist(p));
	for (const module_markobj *i(mm0.objs), *ie(mm0.objs + mm0.n); i != ie; ++i) {
  ...
  const char* mparts_err_fun(void* moddata) {return 0;}
  ...
  const char* tofromvoicetype = "integer1..128 | (integer1..128 integer1..128 ...)";
  int valid_tofromvoice(struct module_value val) {return module_valid_listofints(val, -1, -1, 1, module_incl, 128, module_incl, 0, tofromvoicetype);}
  ...
  int get_setting(int n, module_setting* set, int id) {
	...
	set->descdoc = "Specifies the voice or voices that note events are copied from in a metapart.  Use in conjunction with `to-voice' to specify "
	  "a complete distribution rule (i.e., note events must be moved from one voice or group of voices to another).  Can be a single voice or a list of voices."
	  "  If both `from-voice' and `to-voice' are lists, then the two lists must be the same size so that the "
	...
	set->type = module_list_nums;
	set->descdoc = "Specifies the voice or voices that note events are copied to in a metapart.  Use in conjunction with `from-voice' to specify "
	  "a complete distribution rule (i.e., note events must be moved from one voice or group of voices to another).  Can be a single voice or a list of voices."
	  "  If both `from-voice' and `to-voice' are lists, then the two lists must be the same size so that the "
	  "\"from\" and \"to\" voices match.";
	set->typedoc = tofromvoicetype; 
	...
	set->type = module_rat;
	set->descdoc = "Specifies a transposition in semitones to apply when distributing note events from a metapart.";
