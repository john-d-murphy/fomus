...
namespace ranges {
  ...
  void dumb_run(FOMUS fom, void* moddata) {
    ...
    do {
      if (!pa) pa = module_part(n);
      fomus_rat p(module_pitch(n));
      if (p < module_setting_rval(n, minpid)) {++ci; if (p < mi) mi = p;}
      if (p > module_setting_rval(n, maxpid)) {++ca; if (p > ma) ma = p;}
      module_skipassign(n);
      n = module_nextnote();
    } while (n);
    if (ca > 0) {
      fout << "*** " << ca << " pitch";
      if (ca > 1) fout << "es";
      fout << " out of range (highest is " << module_pitchtostr(module_makeval(ma)) << ") in part `" << module_id(pa) << '\'' << std::endl;
    }
    if (ci > 0) {
      fout << "*** " << ci << " pitch";
      if (ci > 1) fout << "es";
      fout <<" out of range (lowest is " << module_pitchtostr(module_makeval(mi)) << ") in part `" << module_id(pa) << '\'' << std::endl;
    }
  }
  const char* dumb_err(void* moddata) {return 0;}
  
  const char* minptype = "note | rational0..128";
  int valid_minp (const struct module_value val) {return module_valid_rat(val, module_makerat(0, 1), module_incl, module_makerat(128, 1), module_incl, 0, minptype);} // also maxp
...
const char* module_longname() {return "Range Check";}
const char* module_author() {return "(fomus)";}
const char* module_doc() {return "Checks all notes and warns if any are out of range.";}
void* module_newdata(FOMUS f) {return 0;}
void module_freedata(void* dat) {}
int module_priority() {return 0;}
enum module_type module_type() {return module_modcheck;}
...
int module_get_setting(int n, module_setting* set, int id) {
  ...
  case 0:
	...
	"  This is used to check if pitches fall within the correct range for an instrument.  "
