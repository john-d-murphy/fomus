...
namespace tquant {
  ...
  int rlmodid, maxdivid, delzeroid, howaveid, enginemodid /*, mintupdurid, maxtupdurid*/; //, smallestdurid;
  ...
  struct setptrcmp:public std::binary_function<const setptr&, const setptr&, bool> {
  ...
  struct module_valueq:public module_value {
    bool q;
    module_valueq(const module_value& v, const bool q):module_value(v), q(q) {}
    bool hasbeenquant() const {return q;}
    void setquant() {
      assert(!q);
      q = true;
    }
    module_valueq getqed() const {return module_valueq(*this, true);}
  };
  inline bool operator<(const module_valueq& x, const module_valueq& y) {return (const module_value&)x < (const module_value&)y;}
#ifndef NDEBUGOUT
  std::ostream& operator<<(std::ostream& o, const module_valueq& x) {
    ...
    case module_int: o << ((const module_value&)x).val.i; return o;
    case module_float: o << ((const module_value&)x).val.f; return o;
  ...
  inline fomus_float closestdist(const module_value& v, timepointsset_constit& a, const timepointsset_constit& aend) {
    assert(a != aend);
  ...
  inline fomus_rat closest(const module_value& v, const timepointsset& a) {
    ...
    if (a2 == a.begin()) return GET_R(*a2);
    timepointsset_constit a1(boost::prior(a2));
    if (a2 == a.end()) return GET_R(*a1);
    return GET_R(*(std::fabs(GET_F(*a1 - v)) < std::fabs(GET_F(*a2 - v)) ? a1 : a2));
  }

  inline fomus_float ave_mse(const timepointssetq& pts, const timepointsset& qpts) {
    assert(!qpts.empty());
    fomus_float sm = 0;
    ...
    for (timepointssetq_constit e(pts.begin()); e != pts.end(); ++e) {
      if (e->q) {
	DBG("q(" << *e << ") ");
	fomus_float x = closestdist(*e, q, qpts.end()); // q might be incremented
	sm += x * x;
      }
    }
    return sm; //return std::sqrt(sm); don't bother with extra calculation
  }
  inline fomus_float ave_sum(const timepointssetq& pts, const timepointsset& qpts) {
    assert(!qpts.empty());
    fomus_float sm = 0;
    timepointsset_it q(qpts.begin()); //, q0(qpts.begin());
    for (timepointssetq_constit e(pts.begin()); e != pts.end(); ++e) {
  ...
  inline fomus_float getscore(const quanthowave how, const timepointssetq& pts, const timepointsset& qpts) { // qpts are the search points
    DBG("SCORE:" << std::endl);
#ifndef NDEBUGOUT
    DBG("orig: ");
    for (timepointssetq_constit i(pts.begin()), ie(pts.end()); i != ie; ++i) DBG(*i << ' '); DBG(std::endl);
    DBG("quant: ");
    for (timepointsset_constit i(qpts.begin()), ie(qpts.end()); i != ie; ++i) DBG(*i << ' '); DBG(std::endl);
#endif    
    switch (how) {
    case how_mse: {fomus_float x = ave_mse(pts, qpts); DBG('=' << x << std::endl); return x;}
    case how_sum: return ave_sum(pts, qpts);
    default: assert(false);
    }
  }
  ...
  struct solut {
    ...
    bool o1notset() const {return o1.den == -1;}
    bool o2notset() const {return o2.den == -1;}
    bool isready() const {return o1.den != -1 && o2.den != -1;}
    void assign(/*const int delzeroid*/);
    bool hasbegtup(const int lvl) const {return begtup.find(lvl) != begtup.end();}
    bool hasendtup(const int lvl) const {return endtup.find(lvl) != endtup.end();}
    bool hasanybegtup() const {return !begtup.empty();}
    bool hasanyendtup() const {return !endtup.empty();}
  };

  solut::solut(const module_noteobj note, const int vs):note(note), vs(vs) /*, begtup(false), endtup(false)*/ {
    o1.den = -1;
    o2.den = -1;
  ...
  void solut::assign(/*const int delzeroid*/) {
	...
	tquant_delete(note);
      ...
      DBG("TQUANT ASSIGN c: ti:" << o1 << " grti:" << module_vgracetime(note) << " grdu:" << module_vgracedur(note) << std::endl);
      tquant_assign_gracetime(note, o1, module_vgracetime(note), module_vgracedur(note));
  ...
  AGAIN:
    assert(vals.empty());
    fomus_rat mbeg(module_time(meas)); // measure off/endoffs expected to be rationals
    fomus_rat mend(module_endtime(meas));
    for (std::deque<solut>::const_iterator i(inout.begin()); i != inout.end(); ++i) { //... previous notes that overlap into this measure
      if (i->vs != lvsnum) continue;
	...
	vals.insert(module_valueq(o, false)); // insert the offset...
      ...
      DBG("grabbing from prev meas: o=" << o << " eo=" << eo <<
	  " o1=" << (i->o1notset() ? module_makerat(-1, 1) : i->o1) << " o2=" << (i->o2notset() ? module_makerat(-1, 1) : i->o2) << std::endl);
    ...
    lowestlim = module_ratinv(module_setting_rval(meas, maxdivid)); // beatdivision returns rat
#ifndef NDEBUG
    strtohowavemap_constit hh(strtohowave.find(module_setting_sval(meas, howaveid)));
  ...
  struct quantbase {
...
#endif    
    virtual void expand(quantdata& data, const divsearch_ornode_ptr ornode) const = 0;
    ...
    virtual bool isnotes() const {return false;}
#endif    
  };
  ...
  struct quantnotes:public quantbase {
    ...
    quantnotes(const quantdata& data, const timepointssetq_constit& n1, const timepointssetq_constit& n2, const divrules_div rule,
	       const fomus_rat& o1, const fomus_rat& o2, const std::vector<divrules_range>& vect); // EXPAND
    quantnotes(const divrules_iface& rliface, const fomus_rat& o1, const fomus_rat& o2):rliface(rliface), o1(o1), o2(o2), rule(0), qtzd(true) {} // ASSEMBLE
    quantnotes(const quantdata& data, struct divsearch_andnode& andnode);
    ~quantnotes() {if (rule) rliface.free_node(rliface.moddata, rule);}
#ifndef NDEBUG
    bool isnotes() const {return true;}
#endif    
  ...
  quantnotes::quantnotes(const quantdata& data, const timepointssetq_constit& n1, const timepointssetq_constit& n2, const divrules_div rule,
			 const fomus_rat& o1, const fomus_rat& o2, const std::vector<divrules_range>& vect):rliface(data.rliface), o1(o1), o2(o2), rule(rule) {
    qvals.insert(module_makeval(o1));
    qvals.insert(module_makeval(o2));
    if (o2 - o1 > data.lowestlim && hassome(n1, n2, boost::lambda::_1 > o1 && boost::lambda::_1 < o2)) {
      vals.insert(n1, n2);
      qtzd = false;
    } else {
      for (timepointssetq_constit i(n1); i != n2; ++i) vals.insert(i->getqed());
      qtzd = true;
    }
    for (std::vector<divrules_range>::const_iterator i(vect.begin()); i != vect.end(); ++i) {
      if (i->time2 >= o1 && i->time1 <= o2) excl.push_back(*i);
    }
  }
  
  quantnotes::quantnotes(const quantdata& data, struct divsearch_andnode& andnode):rliface(data.rliface),
										   o1(module_makerat(std::numeric_limits<fomus_int>::max(), 1)),
										   o2(module_makerat(std::numeric_limits<fomus_int>::min() + 1, 1)), 
										   rule(0) {
    assert(andnode.n > 0);
    for (divsearch_node* d = andnode.nodes, *de = andnode.nodes + andnode.n; d != de; ++d) {
      const quantnotes& dd = *((quantnotes*)*d);
      if (dd.o1 < o1) o1 = dd.o1;
      if (dd.o2 > o2) o2 = dd.o2;
      vals.insert(dd.vals.begin(), dd.vals.end());
      qvals.insert(dd.qvals.begin(), dd.qvals.end());
    }
  }
  
  inline void quantnotes::expand(quantdata& data, const divsearch_ornode_ptr onode) const {
    assert(rule);
    divrules_rangelist exc = {excl.size(), &excl[0]};
    divrules_ornode exp(rliface.expand(rliface.moddata, rule, exc)); // only need to free the nodes
    assert(exp.n > 0);
	...
	data.api.push_back(anode, new quantnotes(data, pts1, pts2, *d, t1, t2, excl));
	t1 = t2;
	pts1 = pts2;
      }
    }
  ...
  struct quantmeas:public quantbase { // measure node--should be "expanded" in order
    const module_measobj meas; // dfsearch should open these in order
    quantmeas(const module_measobj meas):meas(meas) {}
    void expand(quantdata& data, const divsearch_ornode_ptr onode) const;
    bool isleaf(const quantdata& data) const {return false;}
    divsearch_score score(const quantdata& data) const {
      divsearch_score r;
      r.f = 0;
      return r;
  ...
  inline void quantmeas::expand(quantdata& data, const divsearch_ornode_ptr onode) const {
	...
	data.api.push_back(andn, new quantnotes(data, vals.begin(), vals.end(), *d, mt, met, data.exclvect));
      }
    }
  }
  ...
  inline void quantdata::assignnotes(const quantnotes& node) { // node is a solution node
    assert(lmeas);
#ifndef NDEBUG
    assert(node.qtzd);
    for (timepointssetq_constit i(node.vals.begin()); i != node.vals.end(); ++i) {
      assert(i->q);
    }
#endif
    std::set<userpt> pts;
    fomus_rat mbeg(module_time(lmeas)); // measure off/endoffs expected to be rationals
    fomus_rat mend(module_endtime(lmeas));
    assert(pass2 || exclvect.empty());
    for (std::deque<solut>::iterator i(inout.begin()); i != inout.end(); ++i) { // assign timepoints to notes
      if (i->vs != lvsnum) continue;
      DBG("Note TIMES = " << module_vtime(i->note) << " -- " << module_vendtime(i->note) << std::endl);
	...
	if (o >= mbeg && o <= mend) {
	  ...
	  if (!pass2) {
	    for (std::set<int>::const_iterator j(i->begtup.begin()), je(i->begtup.end()); j != je; ++j) {
	...
	if (eo >= mbeg && eo <= mend) {
	  ...
	  if (!pass2) {
	    for (std::set<int>::const_iterator j(i->endtup.begin()), je(i->endtup.end()); j != je; ++j) {
  ...
  // notes & rests haven't been divided up yet, even across measures--still the user's raw input
  inline divsearch_node quantdata::getroot() {
    ...
    if (m) {
      ...
      assert(rlmod);
      if (rliface.moddata) rliface.free_moddata(rliface.moddata);
      rliface.data.meas = m;
  ...
  union divsearch_score score(void* moddata, divsearch_node node) {return ((quantbase*)node)->score(*(quantdata*)moddata);}
  void expand(void* moddata, divsearch_ornode_ptr ornode, divsearch_node node) {((quantbase*)node)->expand(*(quantdata*)moddata, ornode);}
  divsearch_node assemble(void* moddata, struct divsearch_andnode andnode) {
    assert(andnode.n > 0);
    assert((*(quantbase**)andnode.nodes)->isnotes());
    quantnotes* q = new quantnotes(((quantdata*)moddata)->rliface,
				   (*(quantnotes**)(quantbase**)andnode.nodes)->o1,
				   (*(((quantnotes**)(quantbase**)andnode.nodes) + andnode.n - 1))->o2);
    std::for_each(((quantnotes**)(quantbase**)andnode.nodes), ((quantnotes**)(quantbase**)andnode.nodes) + andnode.n,
		  boost::lambda::bind(&quantnotes::assem, boost::lambda::var(*q), *boost::lambda::_1));
    return q;
  } // TODO!
  int score_lt(void* moddata, union divsearch_score x, union divsearch_score y) {return x.f > y.f;}
  void free_node(void* moddata, divsearch_node node) {delete (quantbase*)node;}
  divsearch_node get_root(void* moddata) {return ((quantdata*)moddata)->getroot();}
  int is_leaf(void* moddata, divsearch_node node) {return ((quantbase*)node)->isleaf(*(quantdata*)moddata);}
  void solution(void* moddata, divsearch_node node) {((quantdata*)moddata)->assignnotes(*(quantnotes*)node);}
  const char* qerr(void* moddata) {return 0;}
  ...
  int valid_quanterr_aux(const char* val) {return strtohowave.find(val) != strtohowave.end();}
  int valid_quanterr(const struct module_value val) {return module_valid_string(val, -1, -1, valid_quanterr_aux, quanterrtype);}
  const char* beatdivtype = "rational>=1";
  int valid_beatdiv(const struct module_value val) {return module_valid_rat(val, module_makerat(1, 1), module_incl, module_makerat(0, 1), module_nobound, 0, beatdivtype);}
  ...
  int valid_smallestdur(const struct module_value val) {return module_valid_rat(val, module_makerat(0, 1), module_excl, module_makerat(0, 1), module_nobound, 0, smallestdurtype);}
...
const char* module_longname() {return "Quantize Times/Durations";}
const char* module_author() {return "(fomus)";}
const char* module_doc() {return "Quantizes times and durations to metrical and tuplet divisions providing the closest fit.";}
void* module_newdata(FOMUS f) {return new quantdata;}
void module_freedata(void* dat) {delete (quantdata*)dat;}
int module_priority() {return 0;}
enum module_type module_type() {return module_modtquant;}
...
int module_itertype() {return module_bypart;} // notes aren't divided yet, so they reach across measures
...
int module_get_setting(int n, module_setting* set, int id) {
  ...
  case 0:
    {
      ...
      set->descdoc = "The module used by FOMUS's quantizing algorithm to search for valid metrical divisions and tuplets."
  ...
  case 1:
    {
      ...
      set->type = module_rat;
      set->descdoc = "The maximum number of divisions allowed in one beat.  "
	"Set this to the smallest division that times and durations should be quantized to.  "
	"If a beat is equivalent to a quarter note, a `beatdiv' of 4 allows notes to be quantized down to the size of a sixteenth note.  "
  ...
  case 2:
    {
      ...
      set->descdoc = "Determines whether or not events quantized to a duration of zero should become grace notes.  "
	"Set this to `yes' if you want notes of very small duration to turn into grace notes or `no' if you want them to disappear.";
      //set->typedoc = beatdivtype; 
  ...
  case 3:
    {
      ...
      set->type = module_string;
      set->descdoc = "How quantization error is calculated.  FOMUS's quantize module works by minimizing the amount of error and searches for the combinations of "
	"divisions and tuplets that provide the \"best fit.\"  "
	"Choices are `ave' for average error and `mse' for mean-squared error (larger errors are less likely at the expense of a slightly higher average error).  "
	"Choose the one that represents best how you want quantization error to be calculated.";
  ...
  case 4:
	...
	"  Set this to the name of an engine module to change the search algorithm used for time quantization.";
