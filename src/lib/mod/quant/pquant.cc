...
namespace pquant {
  ...
  void dumb_run(FOMUS fom, void* moddata) { // return true when done
    while (true) {
      ...
      if (!n) break;
      pquant_assign(n, roundto(module_vpitch(n), module_makerat(1, module_setting_ival(n, qtsid) ? 2 : 1)));
    }
  }
...
const char* module_longname() {return "Quantize Pitches";}
const char* module_author() {return "(fomus)";}
const char* module_doc() {return "Quantizes pitches to semitones or quartertones.";}
void* module_newdata(FOMUS f) {return 0;}
void module_freedata(void* dat) {}
int module_priority() {return 10;}
enum module_type module_type() {return module_modpquant;}
const char* module_initerr() {return 0;}
int module_itertype() {return module_bymeas | module_norests | module_noperc;}
void module_init() {}
  ...
  case 0:
    {
      ...
      set->descdoc = "Whether or not pitches are quantized to quartertone values.  "
