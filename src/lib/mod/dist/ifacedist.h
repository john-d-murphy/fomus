...
//   };
  ...
  typedef fomus_float (*dist_dist_fun)(void* moddata, module_noteobj note1, module_noteobj note2);
  typedef void (*dist_free_moddata_fun)(void* moddata);
  typedef int (*dist_is_outofrange_fun)(void* moddata, module_noteobj note1, module_noteobj note2);

  // using module provides:
  struct dist_data {
    int octdist_setid, beatdist_setid; // for cart dist--setting ids so dist.cc can look them up by note!
