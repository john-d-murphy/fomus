...
namespace staves {
  ...
  int upllsid, downllsid,
	 distmodid, octdistid, beatdistid,
	 properclefid, cleforderid, pitchorderid, clefchangeid, staffchangeid, voicemaxid,
	 /*octupid, octdownid,*/ exponid, enginemodid, rangeid, heapsizeid /*, defclefid*/,
    preferredid, clefprefid, highlowclefid, highlowstaffid, staffid, clefid, balanceid /*, vertbalanceid*/;
  
#ifndef NDEBUG  
  inline fomus_float testgtzero(const fomus_float sc) {assert(sc >= 0); return sc;}
#else
#define testgtzero(xxx) (xxx)
  ...
  struct stavesnode _NONCOPYABLE {
    ...
    fomus_float properclefpenalty, cleforderpenalty, pitchorderpenalty, clefchangepenalty, staffchangepenalty, voicemaxpenalty, clefprefpenalty,
      highlowclefpenalty, highlowstaffpenalty, balancepenalty /*, vertbalancepenalty*/; //, defclefpenalty;
#ifndef NDEBUG
    int valid;
#endif
    int defcl;
    ...
    fomus_int wno;
    stavesnode(const module_noteobj note, const int staff, const int clef, const module_partobj prt, const fomus_float totclefpref):
      note(note), clef(clef), staff(staff), voice(module_voice(note)), midpitch(todiatonic(module_clefmidpitch(clef))),
#ifndef NDEBUG      
      valid(12345),
#endif      
      defcl(module_strtoclef(module_staffclef(prt, staff))),
      no(module_pitch(note)), ti(module_time(note)), /*teti(module_tiedendtime(note)),*/ wno(module_writtennote(note)) {
      module_clefobj cl = staves_clef(prt, staff, clef);
      uplls = midpitch + module_setting_ival(cl, upllsid) * 2 + 5; // add these to mid written pitch to get max/min writ pitch
  ...
  struct scorenode {
    const stavesnode* node; // pointer to keep boost::pool_allocator from bitching
    fomus_float dist;
    bool staffch, clefch;
    scorenode(const stavesnode* node, const fomus_float dist, const int st, const int cl):node(node), dist(dist), staffch(st), clefch(cl) {
      assert(node->isvalid());
    }
  };

  // is clef the default for that staff?
  inline fomus_float defaultclef(const stavesnode& n2) { // o1 and o2 = *written* notes
    assert(n2.isvalid());
  ...
  inline fomus_float properclef(const stavesnode& n2) { // o1 and o2 = *written* notes
    assert(iswhite(n2.wno));
    fomus_int d = todiatonic(n2.wno);
    assert(n2.properclefpenalty >= 0);
    if (d < n2.downlls) return testgtzero(((n2.downlls - d) / 2) * n2.properclefpenalty); // one penalty for each legerline
    if (d > n2.uplls) return testgtzero(((d - n2.uplls) / 2) * n2.properclefpenalty);
    return 0;
  }
  // PENALTIES:
  // clef change (+ all previous ones in range--so + penalty for them being close together)
  inline fomus_float clefchange(const bool ch, const stavesnode& n2) { // n2 used for calculating distance
    assert(n2.isvalid());
    return ch ? n2.clefchangepenalty : 0;
  }
  // staff change (...ditto...)
  inline fomus_float staffchange(const bool ch, const stavesnode& n2) { // n2 used for calculating distance
    assert(n2.isvalid());
    return ch ? n2.staffchangepenalty : 0;
  }
  // are higher pitches in higher clefs?
  inline fomus_float highlowpitchclef(const stavesnode& n1, const stavesnode& n2) {
    return ((n1.no < n2.no && n1.midpitch > n2.midpitch)
	    || (n1.no > n2.no && n1.midpitch < n2.midpitch)
	    || (n1.no == n2.no && n1.midpitch != n2.midpitch && n1.staff == n2.staff))
      ? n2.highlowclefpenalty : 0;
  }
  inline fomus_float highlowpitchstaff(const stavesnode& n1, const stavesnode& n2) {
    return ((n1.no < n2.no && n1.staff < n2.staff)
	    || (n1.no > n2.no && n1.staff > n2.staff)
	    || (n1.no == n2.no && n1.staff != n2.staff))
      ? n2.highlowstaffpenalty : 0;
  }
  inline fomus_float balance(const stavesnode& n1, const stavesnode& n2) {
    return (/*n1.ti == n2.ti &&*/ n1.voice != n2.voice && n1.staff == n2.staff ? n2.balancepenalty : 0);
  }
  // inline fomus_float vertbalance(const stavesnode& n1, const stavesnode& n2) {
  //   return (n1.ti == n2.ti && n1.voice != n2.voice && n1.staff == n2.staff ? n2.vertbalancepenalty : 0);
  //   //return (n1.staff == n2.staff && ((n1.voice < n2.voice && n1.no < n2.no) || (n1.voice > n2.voice && n1.no > n2.no)) ? n2.vertbalancepenalty : 0);
  // }
  ...
  struct stavesdata _NONCOPYABLE {
    ...
    ~stavesdata() {if (diface.moddata) diface.free_moddata(diface.moddata);}
    bool isoutofrange(const search_node n1, const search_node n2) const {return diface.is_outofrange(diface.moddata, ((stavesnode*)n1)->note, ((stavesnode*)n2)->note);}
    int getlistofclefs(const module_partobj p) {
      ...
      for (int i = 1; i <= st; ++i) staves.push_back(i);
      for (std::vector<staff>::iterator i(staves.begin()); i != staves.end(); ++i) {
      	for (std::vector<int>::const_iterator j(clefs.begin()); j != clefs.end(); ++j) {
      	  i->tcp += module_setting_fval(staves_clef(prt, i->st, *j), clefprefid);
      	}
      	assert(!clefs.empty());
    ...
    inline fomus_float orderbalance(const std::vector<scorenode>& arr, const stavesnode& n2) const { // only looks at overlapping notes!
	...
	if (module_tiedendtime(i->node->note) > n2.ti) { // was >
	  boost::ptr_map<const int, std::set<int> >::iterator c(cnt.find(i->node->staff));
	  if (c == cnt.end()) {
	    cnt.insert(i->node->staff, x = new std::set<int>);
	    x->insert(i->node->voice);
	  } else c->second->insert(i->node->voice);
	  assert(n2.cleforderpenalty >= 0);
	  assert(n2.pitchorderpenalty >= 0);
	  if (i->node->staff < n2.staff) { // i is in higher staff
	    if (i->node->midpitch <= n2.midpitch) sc += n2.cleforderpenalty;
	    if (module_pitch(i->node->note) < n2.no) sc += n2.pitchorderpenalty;
      ...
      for (boost::ptr_map<int, std::set<int> >::const_iterator i(cnt.begin()); i != cnt.end(); ++i) {
	int s = i->second->size();
	if (s > 2) {
...
//     inline fomus_float vertbalance(const std::vector<scorenode>& arr, const stavesnode& n2) const {
//       std::multiset<std::pair<int, int> > x;
//       for (std::vector<scorenode>::const_reverse_iterator i(arr.rbegin()); i != arr.rend() && i->node->ti >= n2.ti; ++i) {
// #ifndef NDEBUG
// 	assert(i->node->ti <= n2.ti);
// 	// if (i != arr.rbegin()) assert(i->node->ti <= boost::prior(i)->node->ti);
// #endif	
// 	//if (i->node->ti == n2.ti) 
// 	x.insert(std::set<std::pair<int, int> >::value_type(i->node->staff, i->node->voice));
//       }
//       fomus_float r = 0;
//       if (!x.empty()) {
// 	bool mv = false;
// 	int nmv = 0, nv = 0;
// 	for (std::set<std::pair<int, int> >::const_iterator i(boost::next(x.begin())); i != x.end(); ++i) {
// 	  const std::pair<int, int>& p(*boost::prior(i));
// 	  if (i->first == p.first) { // same staff
// 	    if (i->second == p.second) mv = true;
// 	    else {
...
//     }
	...
	if (l == lsts.end()) lsts.insert(std::map<int, int>::value_type((*i)->staff, (*i)->clef)); else l->second = (*i)->clef;
	if (vl == vlsts.end()) vlsts.insert(std::map<int, int>::value_type((*i)->voice, (*i)->staff)); else vl->second = (*i)->staff;
	assert(arr.empty() || arr.back().node->isvalid());
      }
      search_score sc;
      sc.f = testgtzero(properclef(n2)) +
	testgtzero(orderbalance(arr, n2)) +
	testgtzero(defaultclef(n2)) /*+ testgtzero(vertbalance(arr, n2))*/; // multiply by arr size so matches with accumulated calculations in loop
      if (!arr.empty()) {
	fomus_float mx = 0, ll = 0;
	for (std::vector<scorenode>::const_iterator i(arr.begin()); i != arr.end(); ++i) { // arr contains all but n2
	  assert(i->dist > 0);
	  ll += i->dist;
	  mx += testgtzero(clefchange(i->clefch, n2)) + testgtzero(staffchange(i->staffch, n2))
	    + (testgtzero(highlowpitchclef(*i->node, n2)) + testgtzero(highlowpitchstaff(*i->node, n2)) + balance(*i->node, n2) /*+ vertbalance(*i->node, n2)*/) * i->dist;
	}
	sc.f += (mx  / ll);
      }
      DBG("STAFF SCORE: ");
#ifndef NDEBUGOUT      
      for (std::vector<scorenode>::const_iterator i(arr.begin()); i != arr.end(); ++i) {
	DBG(module_pitch(i->node->note) << ',' << i->node->staff << ',' << i->node->clef << "  ");
      }
#endif      
	...
	: ((((stavesnode*)prevnode)->note != getn)
      ...
      if (!module_hasstaffclef(n, st, cl)) return 0;
      int ust = module_staff(n);
      if (ust != 0 && ust != st) return 0;
      int ucl = module_clef(n);
      if (ucl != -1 && ucl != cl) return 0;
      const module_value z(module_setting_val(n, staffid));
      assert(z.type == module_list);
      if (z.val.l.n > 0) {
	for (const module_value* i = z.val.l.vals, *ie = z.val.l.vals + z.val.l.n; i < ie; ++i) {
	  if (*i == (fomus_int)st) goto ITSOK1;
	}
	return 0;
      }
    ITSOK1:
      ...
      if (zz.val.l.n > 0) {
	for (const module_value* i = zz.val.l.vals, *ie = zz.val.l.vals + zz.val.l.n; i < ie; ++i) {
	  assert(i->type == module_string);
	  if (module_strtoclef(i->val.s) == cl) goto ITSOK2;
	}
	return 0;
      }
    ITSOK2:
    ...
    void assignnext(const int choice) {
      DBG("ASSIGNING STAFF = " << module_time(module_peeknextnote(ass)) << " --> " << staves[choice / clefs.size()].st << ' ' << clefs[choice % clefs.size()] << std::endl);
      staves_assign(ass = module_peeknextnote(ass), staves[choice / clefs.size()].st, clefs[choice % clefs.size()]);
      assert(ass);      
    };
  };
  ...
  void search_assign(void* moddata, int choice) {((stavesdata*)moddata)->assignnext(choice);} // makes a solution assignment & reports it so that other phases of the program can continue
  union search_score search_get_score(void* moddata, struct search_nodes nodes) {return ((stavesdata*)moddata)->getscore(nodes);}
  search_node search_new_node(void* moddata, search_node prevnode, int choice) {return ((stavesdata*)moddata)->newnode(prevnode, choice);} // return NULL if no valid next node or END if at end of search--`prevnode' might be BEGIN (get the first node)
  void search_free_node(void* moddata, search_node node) {delete (stavesnode*)node;}
  const char* search_err(void* moddata) {return 0;}
  int search_score_lt(void* moddata, union search_score x, union search_score y) {return x.f > y.f;}
  ...
  int search_is_outofrange(void* moddata, search_node node1, search_node node2) {return ((stavesdata*)moddata)->isoutofrange(node1, node2);} // node2 has the proper diface

  const char* staffupllstype = "integer>=0";
  int valid_staffuplls(const struct module_value val) {return module_valid_int(val, 0, module_incl, 0, module_nobound, 0, staffupllstype);}
  const char* staffdistmultandclefpreftype = "real>0";
  int valid_staffdistmultandclefpref(const struct module_value val) {
    return module_valid_num(val, module_makeval((fomus_int)0), module_excl, module_makeval((fomus_int)0), module_nobound, 0, staffdistmultandclefpreftype);
  }
...
void module_fill_iface(void* moddata, void* iface) { // module
  ...
  module_partobj p = module_peeknextpart(0);
  ((search_iface*)iface)->nchoices = ((stavesdata*)moddata)->getlistofclefs(p);
...
const char* module_longname() {return "Staves";}
const char* module_author() {return "(fomus)";}
const char* module_doc() {return "Divides notes among staves and creates clef changes where appropriate.";}
void* module_newdata(FOMUS f) {return new stavesdata;}
void module_freedata(void* dat) {delete (stavesdata*)dat;}
const char* module_err(void* dat) {return 0;}
int module_priority() {return 0;}
enum module_type module_type() {return module_modstaves;}
const char* module_initerr() {return ierr;}
int module_itertype() {return module_bypart | module_firsttied | module_norests;}
void module_init() {}
...
int module_get_setting(int n, module_setting* set, int id) {
  ...
  case 0:
    {
      ...
      set->type = module_int;
      set->descdoc = "Number of ledger lines generally allowed above the staff before a clef, staff or octave sign occurs.  "
	"Increasing this increases the number of ledger lines generally used and decreases the number of octave signs above the staff.";
  ...
  case 1:
    {
      ...
      set->type = module_int;
      set->descdoc = "Number of ledger lines generally allowed below the staff before a clef, staff or octave sign occurs.  "
	"Increasing this increases the number of ledger lines used and decreases the number of octave signs below the staff.";
  ...
  case 2:
	...
	" (at twice this distance the score is decreased to 1/4 it's full value, etc.).";
      set->typedoc = staffdistmultandclefpreftype; // uses the same string/valid fun
  ...
  case 3:
	...
	"  Set this to the name of an engine module to change the search algorithm used for finding staff and clef changes.";
  ...
  case 4:
	...
	"  Set this to change the algorithm used for calculating distance when making decisions regarding staff and clef changes."
	;
  ...
  case 5:
	...
	"  A larger distance translates to a smaller weight in decisions regarding staves/clefs and vice versa.";
  ...
  case 7:
    {
      set->name = "staves-instaff-score"; // docscat{staves} // fits in the staff w/ given clef
      set->type = module_number;
      set->descdoc = "Score applied when notes fit inside the staff.  "
	"A high score causes notes to appear inside their staves (i.e., without the use of copious ledger lines) and also affects the selection of proper clef signatures.";
      set->typedoc = scoretype; 
  ...
  case 8:
    {
      set->name = "staves-clef-vertorder-score"; // docscat{staves} // for simultaneous notes
      set->type = module_number;
      set->descdoc = "Score applied when clefs appear in the correct vertical order on more than one staff.  "
	"This persuades FOMUS to avoid situations where an upper staff is actually lower in register than a lower staff (and vice versa).";
      set->typedoc = scoretype; 
  ...
  case 9:
    {
      set->name = "staves-pitch-vertorder-score"; // docscat{staves} // for simultaneous notes
      set->type = module_number;
      set->descdoc = "Score for putting simultaneous pitches in the correct vertical order (i.e., higher notes go on higher staves and vice versa).  "
	"Increasing this value makes it more likely for pitches to appear in the correct vertical order across staves.";
      set->typedoc = scoretype; 
  ...
  case 10:
	...
	"  Increasing this makes it less likely for clef changes to occur rapidly in succession.";
      set->typedoc = scoretype; 
  ...
  case 11:
	...
	"  Increasing this makes it less likely for staff changes to occur rapidly in succession.";
      set->typedoc = scoretype; 
  ...
  case 12:
    {
      set->name = "staves-voicemax-score"; // docscat{staves} // more than 2 voices in a staff penalty
      set->type = module_number;
      set->descdoc = "Score for placing no more than two voices in a staff.  "
	"Three or more voices could become crowded since note stems are forced to run into each other."
  ...
  case 15:
	...
	"  Preferred clefs are specified in staff objects with the `clef-preference' setting.";
      set->typedoc = scoretype; 
  ...
  case 16:
	...
	"  1 is the default value for all clefs, which gives them equal probability of being chosen.  "
	"Anything less than or greater than 1 decreases or increases the probability of choosing that clef."
	"  Set this in an instrument definition to affect how clefs are chosen for that instrument.";
  ...
  case 17:
    {
      set->name = "staves-appropriate-clef-score"; // docscat{staves} // higher pitches get highre clefs, vice versa
      set->type = module_number;
      set->descdoc = "Score applied when higher pitches are in \"higher\" clefs and vice versa."
	"  This helps insure that awkward clef changes don't occur when a pitch interval is leaping in the opposite direction.";
      set->typedoc = scoretype; 
  ...
  case 18:
    {
      set->name = "staves-appropriate-staff-score"; // docscat{staves} // higher pitches get highre clefs, vice versa
      set->type = module_number;
      set->descdoc = "Score applied when relatively higher pitches in a melodic line are in higher staves and vice versa."
	"  This helps insure that awkward staff changes don't occur when a pitch interval is leaping in the opposite direction.";
      set->typedoc = scoretype; 
  ...
  case 19:
    {
      set->name = "staves-balance-score"; // docscat{staves} // more than 2 voices in a staff penalty
      set->type = module_number;
      set->descdoc = "Score for balancing voices across staves.  "
	"Increasing this helps spread prevent crowding of separate voices onto the same staff.";
      set->typedoc = scoretype; 
  ...
  // case 20:
  //   {
  //     set->name = "staves-vertbalance-score"; // docscat{staves} // more than 2 voices in a staff penalty
  //     set->type = module_number;
  //     set->descdoc = "Score for balancing simultaneous notes across staves.  "
  // 	"Increasing this helps spread prevent crowding of separate chords onto the same staff.";
  //     set->typedoc = scoretype; 
...
void module_ready() {
  ...
  if (staffid < 0) {ierr = "missing required setting `staff'"; return;}
  clefid = module_settingid("clef");
  if (clefid < 0) {ierr = "missing required setting `clef'"; return;}
}
