...
namespace rstaves {
  ...
  void blastem(std::deque<module_noteobj>& notes, const int cs, const int cl) {
    while (!notes.empty()) {
      ...
      if (module_isrest(n)) {
	DBG("out: (t=" << module_time(n) << ") st=" << cs << " cl=" << cl << std::endl);
  ...
  void rstaves_run_fun(FOMUS fom, void* moddata) {
	...
	for (const fomus_rat *r = dvs.rats, *re = dvs.rats + dvs.n; r < re; ++r) {
	  divs.push_back(o);
	  o = o + *r;
	}
      }
	...
	cs = cs1 = c;
	cl = l;
      } else if (mb && (module_tuplet(n, 0) == (fomus_int)0 || module_tupletbegin(n, 0))) {
	if (cs) {
	  blastem(notes, cs, cl);
	  cs = 0;
	}
    ...
    blastem(notes, cs1, cl < 0 ? module_strtoclef(module_staffclef(module_peeknextpart(0), cs1)) : cl);
  }

  const char* rstaves_err_fun(void* moddata) {return 0;}
