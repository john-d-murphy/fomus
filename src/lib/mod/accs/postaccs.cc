...
namespace postaccs {
  ...
  inline void getaccsaux(const fomus_rat& frnote, const fomus_rat& facc,
  ...
  struct noteholder {
    ...
    fomus_rat acc1, acc2;
    fomus_rat p;
    noteholder(const module_noteobj n) : n(n), p(module_pitch(n)) {
      acc1.num = std::numeric_limits<fomus_int>::max();
    }
    void assign() const {
      if (acc1.num == std::numeric_limits<fomus_int>::max())
        module_skipassign(n);
      else
        accs_assign(n, acc1, acc2);
    }
    void set(const fomus_rat& a) {
      acc1 = module_makerat(a.num / a.den, 1);
      acc2 = a - acc1;
    }
    void setcopy(const noteholder& x) {
      acc1 = x.acc1;
      acc2 = x.acc2;
    }
    void getaccs(const fomus_rat& frnote, const fomus_rat& facc) {
      getaccsaux(frnote, facc, p, acc1, acc2);
    }
  };
  inline bool operator<(const noteholder& x, const noteholder& y) {
    return x.p < y.p;
  }
  inline bool operator==(const noteholder& x, const noteholder& y) {
    return x.p == y.p;
  ...
  void run_fun(FOMUS f, void* moddata) { // neither of these parameters used
    ...
    while (true) {
      ...
      if (module_isendchord(n)) {
        if (t && !nos1.empty()) {
          fomus_int o = todiatonic(module_writtennote(
              nos1[0]->n)); // assuming everything valid at this point
          for (std::vector<noteholder*>::iterator i(nos2.begin());
               i != nos2.end(); ++i) {
            fomus_rat d(nos1[0]->p - (*i)->p);
            if (d > (fomus_int) 0 &&
                d < (fomus_int) 3) { // valid trill = whole step or less, must
                                     // be 1 diatonic step
              (*i)->set((*i)->p - tochromatic(o + 1));
            }
          }
        }
        if (mo && !nos1.empty() && !nos2.empty() &&
            nos1.size() == nos2.size()) { // if same notes in a tremolo
          std::multiset<noteholder*> p1, p2;
          for (std::vector<noteholder*>::iterator i(nos1.begin());
               i != nos1.end(); ++i)
            p1.insert(*i);
          for (std::vector<noteholder*>::iterator i(nos2.begin());
               i != nos2.end(); ++i)
          ...
          if (std::equal(p1.begin(), p1.end(), p2.begin())) {
            ...
            for (std::multiset<noteholder*>::const_iterator x1(p1.begin());
                 x1 != p1.end(); ++x1, ++x2)
              (*x2)->setcopy(**x1);
          }
        }
        if (h && !nos0.empty()) { // written harm, in unison w/ base note
          ...
          for (std::set<noteholder*>::iterator i(boost::next(nos0.begin()));
               i != nos0.end(); ++i)
            (*i)->getaccs(n, wn);
        }
        std::for_each(
...
const char* module_longname() {
  return "Accidentals Post-processing";
}
const char* module_author() {
  return "(fomus)";
}
const char* module_doc() {
  return "Handles accidentals in special situations such as trills and "
...
int module_itertype() {
  return module_bymeas | module_byvoice | module_firsttied | module_norests;
