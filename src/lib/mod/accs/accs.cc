...
#define NQT 3
  ...
  // setting ids
  int heapsizeid, qtsid, daccsid,
    stepwiseid, diaintid, dianoteid, simqtid,
    exponid,
    octdistid, beatdistid, rangeid, distmodid,
    enginemodid, accid, wrongdirid; //, inkeyid;
  
  struct accsnode _NONCOPYABLE {
    module_noteobj note;
    bool daccs, qts; // is this note qt-enabled?
    fomus_float stepwisepenalty, diaintpenalty, dianotepenalty, simqtpenalty, /*inkeypenalty,*/ wrongdirpenalty;
    fomus_float expon;
    int acc;
    fomus_rat qtacc;
    //struct module_keysigref keyaccs;
    //bool oacc;
    accsnode(const module_noteobj note, /*const deque<accsnode*>::type::const_iterator& it,*/ const int acc, const fomus_rat& qtacc, /*const dist_iface& diface,*/
	     const bool daccs, const bool qts):note(note), /*it(it),*/ daccs(daccs), qts(qts), acc(acc), qtacc(qtacc) /*,*/ /*, diface(diface)*/ 
...
//       else {
// 	if (a < (fomus_int)0) {
// 	  struct module_keysigref xx(module_measkeysigacc(m, tochromatic(todiatonic(nn) - 1)));
// 	  oacc = (xx.acc1 + xx.acc2 > (fomus_int)0);
// 	} else {
// 	  struct module_keysigref xx(module_measkeysigacc(m, tochromatic(todiatonic(nn) + 1)));
// 	  oacc = (xx.acc1 + xx.acc2 < (fomus_int)0);
// 	}
//       }
  ...
  struct scorenode {
    ...
    fomus_float dist;
    scorenode(const accsnode* node, const fomus_float dist):node(node), dist(dist) {}
  };

  inline fomus_float stepwise(const accsnode& n1, const accsnode& n2, const fomus_rat& o1, const fomus_rat& o2) { // o1 and o2 = notes
    ...
    return ((o1 - a1 == o2 - a2) &&
	    ((o2 > o1 && (a2 < (fomus_int)0 || a1 < (fomus_int)0)) ||
	     (o2 < o1 && (a2 > (fomus_int)0 || a1 > (fomus_int)0)))) ? n2.stepwisepenalty : 0;
  }
  inline fomus_float dianote(const accsnode& n2, const fomus_rat& o2) { // o2 is note
    //int a = n2.acc;
    return ((n2.acc != 0 && iswhite(o2 - n2.qtacc)) || n2.acc < -1 || n2.acc > 1) ? n2.dianotepenalty : 0; // acc = semitone-acc
  }
...
//       // 	    || (n2.keyaccs.acc2 > (fomus_int)0  && n2.qtacc < (fomus_int)0)
//       // 	    || (n2.keyaccs.acc2 < (fomus_int)0 && n2.qtacc > (fomus_int)0))
//       n2.oacc ? n2.inkeypenalty : 0;
//   }
  // increase in sharpness/flatness must match the allowed values for that IC
  inline fomus_float diaint(const accsnode& n1, const accsnode& n2, const fomus_rat& o1, const fomus_rat& o2) {
    assert(diff(o1 - /*n1.acc -*/ n1.qtacc, o2 - /*n2.acc -*/ n2.qtacc).den == 1);
    int it(diff(o1 - /*n1.acc -*/ n1.qtacc, o2 - /*n2.acc -*/ n2.qtacc).num % 12); // the base written interval mod 12
    int id(o1 > o2 ? n1.acc - n2.acc : n2.acc - n1.acc); // the accidental difference (not including qt)
  ...
  inline fomus_float simqt(const accsnode& n1, const accsnode& n2) {
    ...
    return ((n1.acc < (fomus_int)0 && q2 > (fomus_int)0) || (n1.acc > (fomus_int)0 && q2 < (fomus_int)0)
	    || (q1 < (fomus_int)0 && q2 > (fomus_int)0) || (q1 > (fomus_int)0 && q2 < (fomus_int)0)
	    || (q1 < (fomus_int)0 && n2.acc > (fomus_int)0) || (q1 > (fomus_int)0 && n2.acc < (fomus_int)0)) ? n2.simqtpenalty : 0;
  }
  inline fomus_float wrongdir(const accsnode& n1, const accsnode& n2, const fomus_rat& o1, const fomus_rat& o2) {
    fomus_rat a1(o1 - ((fomus_int)n1.acc + n1.qtacc)), a2(o2 - ((fomus_int)n2.acc + n2.qtacc));
    return ((o1 > o2 && a1 < a2) || (o1 < o2 && a1 > a2)) ? n2.wrongdirpenalty : 0;
  }
  ...
  struct accsdata _NONCOPYABLE {
    ...
    ~accsdata() {if (diface.moddata) diface.free_moddata(diface.moddata);}
    bool isoutofrange(const search_node n1, const search_node n2) const {return diface.is_outofrange(diface.moddata, ((accsnode*)n1)->note, ((accsnode*)n2)->note);}
    // no rests, only notes
    union search_score getscore(const search_nodes& nodes) const { // nodes ordered by offset, last one is the node being scored
	...
	if (d <= diface.data.rangemax) arr.push_back(scorenode(*i, pow(n2.expon, d)));
      ...
      sc.f = dianote(n2, o2) /*+ inkey(n2, o2)*/; //vertmax(arr, n2) * arr.size(); // multiply by arr size so matches with accumulated calculations in loop
      if (!arr.empty()) {
	...
	for (std::vector<scorenode>::const_iterator i(arr.begin()); i != arr.end(); ++i) {
	  ...
	  DBG("  dist " << module_time(i->node->note) << " to " << module_time(n2.note) << " = " << i->dist << std::endl);
	  assert(i->dist > 0);
	  ll += i->dist;
	  mx += (stepwise(*i->node, n2, o1, o2) + diaint(*i->node, n2, o1, o2) + simqt(*i->node, n2) + wrongdir(*i->node, n2, o1, o2)) * i->dist; 
	}
	sc.f += mx / ll;
      }
      DBG("ACC SCORE: ");
	...
	: ((((accsnode*)prevnode)->note != getn)
      ...
      if (!daccs && choice >= (3 * NQT)) return 0;
      bool qts = module_setting_ival(n, qtsid);
      fomus_int theacc1 = choicetoacc[choice / NQT];
      fomus_rat theacc2 = choicetoqtacc[choice % NQT];
      if ((!qts && (choice % NQT) > 0) || (!iswhite(module_pitch(n) - (fomus_int)theacc1 - theacc2))) return 0;
      fomus_rat a1(module_acc1(n));
      if (a1.num != std::numeric_limits<fomus_int>::max()) {
	if (a1 != theacc1) return 0;
	fomus_rat a2(module_acc2(n));
	if (a2.num  != std::numeric_limits<fomus_int>::max() && a2 != theacc2) return 0; 
      }
      module_value x(module_setting_val(n, accid)); // what the user wants
      assert(x.type == module_list);
      if (x.val.l.n > 0) {
	bool gotval = false;
	for (const module_value* i = x.val.l.vals, *ie = x.val.l.vals + x.val.l.n; i < ie; ++i) {
	  module_noteparts parts;
	  parts.acc1 = parts.acc2 = module_makerat(0, 1);
	  fomus_rat ret(module_strtoacc(i->val.s, &parts));
	  if (parts.acc1 == theacc1 && parts.acc2 == theacc2) goto ITSOK;
	  if (ret.num != std::numeric_limits<fomus_int>::max() && (qts || (choice % NQT) <= 0) && (iswhite(module_pitch(n) - parts.acc1 - parts.acc2))) gotval = true;
	}
	if (gotval) return 0; // there's a valid one in there, don't make choice for user--if there isn't a valid one, go ahead and make choice for user
      }
    ITSOK:
      DBG("  considering acc: " << module_pitch(n) << ' ' << theacc1 << ' ' << theacc2 << std::endl);
      return new accsnode(n, /*boost::prior(vect.end()),*/ theacc1, theacc2, /*getdiface(n),*/ daccs, qts);
    }
    void assignnext(const int choice) {
      DBG("ASSIGNING ACC = " << module_pitch(module_peeknextnote(ass)) << " --> " << choicetoacc[choice / NQT] << ' ' << choicetoqtacc[choice % NQT] << std::endl);
      accs_assign(ass = module_peeknextnote(ass), module_makerat(choicetoacc[choice / NQT], 1), choicetoqtacc[choice % NQT]);
      assert(ass);
    };
  };
  ...
  void search_assign(void* moddata, int choice) {((accsdata*)moddata)->assignnext(choice);} // makes a solution assignment & reports it so that other phases of the program can continue
  union search_score search_get_score(void* moddata, struct search_nodes nodes) {return ((accsdata*)moddata)->getscore(nodes);}
  search_node search_new_node(void* moddata, search_node prevnode, int choice) {return ((accsdata*)moddata)->newnode(prevnode, choice);} // return NULL if no valid next node or END if at end of search--`prevnode' might be BEGIN (get the first node)
  void search_free_node(void* moddata, search_node node) {delete (accsnode*)node;}
  const char* search_err(void* moddata) {return 0;}
  int search_score_lt(void* moddata, union search_score x, union search_score y) {DBG("  comparing " << x.f << ">" << y.f << std::endl); return x.f > y.f;}
  union search_score search_score_add(void* moddata, union search_score x, union search_score y) {union search_score r; r.f = x.f + y.f; return r;}
  int search_is_outofrange(void* moddata, search_node node1, search_node node2) {return ((accsdata*)moddata)->isoutofrange(node1, node2);} // node2 has the proper diface
  ...
  int valid_accsdistmult(const struct module_value val) {
    return module_valid_num(val, module_makeval((fomus_int)0), module_excl, module_makeval((fomus_int)0), module_nobound, 0, accsdistmulttype);
  }
...
void module_fill_iface(void* moddata, void* iface) { // module
  ...
  ((search_iface*)iface)->heapsize = module_setting_ival(module_peeknextpart(0), heapsizeid);
}
const char* module_err(void* data) {return 0;}
const char* module_longname() {return "Accidentals";}
const char* module_author() {return "(fomus)";}
const char* module_doc() {return "Determines accidental spellings.";}
void* module_newdata(FOMUS f) {return new accsdata;}
...
int module_itertype() {return module_bypart | module_byvoice | module_firsttied | module_norests;}
...
int module_get_setting(int n, struct module_setting* set, int id) {
  ...
  case 1:
    {
      ...
      set->descdoc = "Score when successive stepwise notes contain sharps in upwards motion or flats in downwards motion.  "
	"Increasing this value increases the change of stepwise ascending sharps and descending flats.";
  ...
  case 2:
    {
      ...
      set->type = module_number;
      set->descdoc = "Score when intervals between notes are diatonic (these are presumably easier to read).  "
	"Increasing this score increases the chance of diatonic intervals occurring.";
      set->typedoc = scoretype; 
  ...
  case 3:
    {
      ...
      set->type = module_number;
      set->descdoc = "Score when notes are typical diatonic spellings (i.e., no F-flats, B-sharps, C-doublesharps, etc.).  "
	"Increasing this value increases the chance of diatonic spellings occurring.";
      set->typedoc = scoretype; 
  ...
  case 4:
    {
      ...
      set->type = module_number;
      set->descdoc = "Score when quartertone accidentals on nearby notes are similar to each other (i.e., modifying the written pitch in the same direction).  "
	"Increasing this value increases the chances of similar quartertone adjustments on adjacent notes.";
      set->typedoc = scoretype; 
  ...
  case 6:
	...
	"  Set this to the name of an engine module to change the search algorithm used for finding accidental spellings.";
  ...
  case 7:
	...
	"  Set this to change the algorithm used for calculating distance when making decisions regarding accidentals."
	;
  ...
  case 8:
    {
      ...
      set->descdoc = "Whether or not double accidentals (i.e., double flats or sharps) are allowed.  "
  ...
  case 9:
	...
	"  A larger distance translates to a smaller weight in decisions regarding accidentals and vice versa.";
      set->typedoc = scoretype; 
  ...
  case 11:
    {
      ...
      set->type = module_number;
      set->descdoc = "The maximum distance allowed for accidental spelling calculations."
  ...
  case 12:
    {
      ...
      set->type = module_number;
      set->descdoc = "Score for using higher written notes to spell higher pitches and vice versa.  "
	"Setting this to a high value insures that notes higher up on the staff are actually higher pitches.";
      set->typedoc = scoretype; 
...
void module_ready() {
  ...
  if (qtsid < 0) {ierr = "missing required setting `quartertones'"; return;}
  accid = module_settingid("acc");
  if (accid < 0) {ierr = "missing required setting `acc'"; return;}
}
