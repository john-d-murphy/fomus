...
namespace cautaccs {
  ...
  int octdiffid, octsignsid, rangeid, allowedid, enginemodid, octdistid, beatdistid, distmodid;
  ...
  inline bool octchangeeq(const module_noteobj p, const module_noteobj n, const int pn, const int nt, const octs_enum dis) {
    int pno = pn - module_octsign(p) * 7;
    int nto = nt - module_octsign(n) * 7;
    return (pno == nto || (dis == octs_1 && diff(pn, nt) == 7) || (dis == octs_all && (pn % 7 == nt % 7))
	    || pn == nto || (dis == octs_1 && diff(pn, nt) == 7) || (dis == octs_all && (pn % 7 == nt % 7))
	    || pno == nt || (dis == octs_1 && diff(pn, nt) == 7) || (dis == octs_all && (pn % 7 == nt % 7)));
  }
  ...
  struct scoped_distiface:public dist_iface {
    ~scoped_distiface() {free_moddata(moddata);}
  };
  
  // by part
  void run_fun(FOMUS f, void* moddata) { // neither of these parameters used
    // fomus_float rng, range; // range is the MAXIMUM, sent to distance function
    ...
    while (true) {
	  ...
	  || (module_writtenacc1(n) != module_makerat(std::numeric_limits<fomus_int>::max(), 1)
	      && module_writtenacc2(n) != module_makerat(std::numeric_limits<fomus_int>::max(), 1))) {
	module_skipassign(n);
	continue; // accidental that appears in score!
      }
      while (p != n && diface.is_outofrange(diface.moddata, p, n)) { // set p to first note in range
	//module_skipassign(p);
	p = module_peeknextnote(p);
      }
      int nt = module_writtennote(n);
      //fomus_float rng = module_setting_fval(n, rangeid);
      octs_enum dis = octsstrtoenum.find(module_setting_sval(n, octdiffid))->second; // should have checked for this already
	...
	if (diface.dist(diface.moddata, p0, n) <= diface.data.rangemax) {
	  ...
	  if (pn == nt || (dis == octs_1 && diff(pn, nt) == 12) || (dis == octs_all && (pn % 12 == nt % 12)) || (octsigns && octchangeeq(p0, n, pn, nt, dis))) {
	    pacc = module_fullacc(p0);
	  }
	}
	p0 = module_peeknextnote(p0);
      }
      if (pacc != nacc) cautaccs_assign(n); else module_skipassign(n);
    }
  }
...
//     z.type = module_int;
//     z.val.i = 0;
    return module_valid_num(val, module_makeval((fomus_int)0), module_excl, module_makeval((fomus_int)0), module_nobound, 0, rangetype);
  } // also maxp
  const char* octstype = "none|one|all";
  int valid_octs_aux(const char* val) {return octsstrtoenum.find(val) != octsstrtoenum.end();}
  int valid_octs(const struct module_value val) {return module_valid_string(val, -1, -1, valid_octs_aux, octstype);}
...
const char* module_longname() {return "Cautionary Accidentals";}
const char* module_author() {return "(fomus)";}
const char* module_doc() {return "Adds cautionary accidentals.";}
void* module_newdata(FOMUS f) {return 0;}
void module_freedata(void* dat) {}
int module_priority() {return 0;}
enum module_type module_type() {return module_modcautaccs;}
...
int module_itertype() {return module_bypart | module_byvoice | module_firsttied | module_norests;} // notes aren't divided yet, so they reach across measures
...
int module_get_setting(int n, module_setting* set, int id) {
  ...
  case 0:
    {
      ...
      set->descdoc = "The maximum distance two events are allowed to be from each other to create a cautionary accidental"
	" (how distance is calculated depends on the the value of `cautaccs-distmod')."
	"  Increase this value to create more cautionary accidentals that are farther apart."
	//"  Changing `cautaccs-distmod' also affects where cautionary accidentals occur."
	;
  ...
  case 1:
    {
      ...
      set->descdoc = "Whether or not cautionary accidentals are affected by notes in other octaves.  "
	"Set this in the score, a part, a measure definition, or in note events to affect which notes receive cautionary accidentals.  "
	"`none' means only notes in the same octave can create a cautionary accidental.  "
	"`one' means notes one octave apart can create one while `all' means notes in any octave can cause one.";
  ...
  case 2:
    {
      ...
      set->descdoc = "Whether or not cautionary accidentals are affected by octave signs."
	"Set this in the score, a part, a measure definition, or in note events to affect the impact of octave signs on cautionary accidentals.  "
	"  `yes' means cautionary accidentals can occur in places where notes appear on the same staff line but are actually different pitches due to octave signs."
	"  `no' means cautionary accidentals occur only when the notes are actually the same pitch (or whatever is specified in `cautaccs-octs').";
  ...
  case 3:
    {
      ...
      set->type = module_bool;
      set->descdoc = "Whether or not cautionary accidentals should appear in the score.  "
  ...
  case 4:
	...
	"  A larger distance translates to a smaller weight in decisions regarding cautionary accidentals and vice versa.";
  ...
  case 6:
	...
	"  Set this to change the algorithm used for calculating distance when making decisions regarding cautionary accidentals."
	;
