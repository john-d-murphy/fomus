...
namespace marksaux {
  ...
  template <class I1, class I2, class L,
	    class C1, class C2>
  inline void inplace_set_difference(C1& cont1, C2& cont2) { // doesn't modify the first one
  ...
  struct mark {
    ...
    std::string str;
    module_value val;
    mark(const module_markobj m):ty(module_markid(m)), val(module_marknum(m)) {
      const char* s = module_markstring(m);
      if (s) str = s;
    }
    mark(const int m, const std::string& str):ty(m), str(str), val(module_makenullval()) {}
    ...
    mark(const int m, const std::string& str, const module_value& val):ty(m), str(str), val(val) {}
    mark(const int m, const std::string& str, const fomus_rat& r):ty(m), str(str), val(module_makeval(r)) {}
    mark(const int m, const std::string& str, const fomus_int r):ty(m), str(str), val(module_makeval(r)) {}
    void assass(const module_noteobj n) const {
      DBG("assigning mark " << ty << ',' << (!str.empty() ? str : "") << ',' << val << " at " << module_time(n) << std::endl);
      marks_assign_add(n, ty, str.c_str(), val);
    }
    void assrem(const module_noteobj n) const {
      DBG("removing mark " << ty << ',' << (!str.empty() ? str : "") << ',' << val << " at " << module_time(n) << std::endl);
  ...
  struct markless:std::binary_function<const mark&, const mark&, bool> {
    inline bool operator()(const mark& x, const mark& y) const {
      ...
      if (!(x.str.empty() || y.str.empty())) return x.str < y.str; // not case insensitive here
      if (x.str.empty() != y.str.empty()) return y.str.empty();
      return false;
    }
  };  
