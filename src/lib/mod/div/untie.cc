...
/* this is the main entry point for the module.  the `dumb' engine simply passes control over to this function.  note
   ...
   a pointer to a data structure created by `module_newdata()' below.  modules instances should only read/write to their
   own data structures and not use global variables since multiple threads may be running concurrently. */
...
/* this is called during scheduling and tells FOMUS if two separate module instances can be considered equivalent.  this
   ...
   specified a different module for determining the distance between notes (in either of these cases, the algorithm
...
/* the module type determines the purpose of the algorithm, when it gets scheduled, and which assign functions it can
   use. */
...
/* the module must fill a data structure (defined in the engine's header file) with information the engine needs to
   ...
   created by `module_newdata()', the address of the function called to pass control over to the module, and the address
