...
namespace grdiv {
  ...
  struct node {
    ...
    node(const module_noteobj n, const fomus_rat& o, const fomus_rat& eo):n(n), o(o), eo(eo) {}
    void inspt(const fomus_rat& t) {if (t > o && t < eo) pts.insert(t);}
  ...
  void node::assign(const module_measobj m) {
    if (!pts.empty()) {
      pts.insert(o);
      for (std::set<fomus_rat>::const_iterator i(pts.begin()); i != pts.end(); ++i) {
	divide_gracesplit x = {n, *i};
	divide_assign_gracesplit(m, x);
	DBG("assigning grace split at off=" << *i << ", o=" << o << ", eo=" << eo << std::endl);
      }
    }
    module_skipassign(n);
  }
  ...
  void grdiv_run_fun(FOMUS fom, void* moddata) {
	...
	lt = t;
      ...
      for (boost::ptr_deque<node>::iterator i(nds.begin()); i != nds.end(); ++i) {
	i->inspt(o);
	i->inspt(eo);
	nno->inspt(i->o);
	nno->inspt(i->eo);
  ...
  const char* grdiv_err_fun(void* moddata) {return 0;}
