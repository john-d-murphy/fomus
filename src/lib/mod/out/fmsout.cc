...
namespace fmsout {
  ...
  int whichsettingsid, equalsstrid, filemaxwidthid, tabwidthid, bypartsid, nonrepeventsid, nbeatsid; //, orderid;
  ...
  typedef std::map<const std::string, info_setwhere, isiless>::value_type whichmap_val;
  std::map<const std::string, equalsstr_enum, isiless> equalsmap;
  typedef std::map<const std::string, equalsstr_enum, isiless>::value_type equalsstr_val;
  ...
  struct outdata {
    ...
    //bool userdefpart;
    outdata(FOMUS fom):
      whset(whichmap.find(module_setting_sval(fom, whichsettingsid))->second),
    ...
    void printobj(std::ostream& f, const std::string& typ, const info_objinfo& set);
    void modout_write(FOMUS f, const char* filename);
    void printit(std::ostream& f, const std::string& x);
    void printnotes(std::ostream& out, const module_partobj p, module_measobj& m0, module_noteobj& n, const std::vector<module_markobj>& mevs);
  };
  
  const char* writesettingstype = "none|score|config|all";
  int valid_writesetsstring(const char* val) {return whichmap.find(val) != whichmap.end();}
  int valid_writesets(const struct module_value val) {return module_valid_string(val, -1, -1, valid_writesetsstring, writesettingstype);}
  const char* writeequalstype = "equals|colon";
  int valid_writeequalsstring(const char* val) {return equalsmap.find(val) != equalsmap.end();}
  int valid_writeequals(const struct module_value val) {return module_valid_string(val, -1, -1, valid_writeequalsstring, writeequalstype);}
  const char* writemaxwidthtype = "integer>=79";
  int valid_maxwidth(const struct module_value val) {return module_valid_int(val, 79, module_incl, -1, module_nobound, 0, writemaxwidthtype);}
  const char* writetabwidthtype = "integer0..20";
  int valid_tabwidth(const struct module_value val) {return module_valid_int(val, 0, module_incl, 20, module_incl, 0, writemaxwidthtype);}
  const char* nonreptype = "(eventtype eventtype ...), eventtype = t|ti|tim|time|t*|ti*|tim*|time*|d|du|dur|duration|d*|du*|dur*|duration*|...";
  //const char* ordertype = "(eventtype eventtype ...), eventtype = t|ti|tim|time|d|du|dur|duration|...";
  int valid_nonrep_aux(int n, const char* val) {return evtypemap.find(val) != evtypemap.end() || evtyperetmap.find(val) != evtyperetmap.end();}
  int valid_nonrep(struct module_value val) {return module_valid_listofstrings(val, -1, -1, -1, -1, valid_nonrep_aux, nonreptype);}
  ...
  struct settingslist { // info interface is better in this case--sorts it
    ...
    settingslist(FOMUS f, const info_setwhere wh) {
      ...
      info_sortpair so[] = {{info_modname, info_ascending}, {info_setname, info_ascending}};
  ...
  void outdata::printset(std::ostream& f, const info_setting& set) {
    ...
    os << set.name;
    switch (eqstr) {
    case equalsstr_equals: os << " = "; break;
    case equalsstr_colon: os << ": ";
    }
    os << set.valstr; // << '\n';
    printit(f, os.str());
  }
  void outdata::printobj(std::ostream& f, const std::string& typ, const info_objinfo& set) {
    std::ostringstream os;
    os << typ << ' ' << set.valstr;
    printit(f, os.str());
  }
  ...
  struct printbase {
    ...
    bool free;
    int ind;
    printbase(const std::string& str, const multival& mv, const fomus_int srt, const bool prret = false, const int ind = 0):
      prret(prret), str(str), val(mv), srt(srt), free(free), ind(ind) {}
    void print(std::ostream& f, bool& isnl);
  };
  inline bool operator<(const printbase& x, const printbase& y) {return x.srt == y.srt ? (x.prret ? 1 : 0) > (y.prret ? 1 : 0) : x.srt < y.srt;}
  void printbase::print(std::ostream& f, bool& isnl) {
    if (isnl && ind) f << std::string(ind, ' ');
    f << str;
    if (prret) {
      f << '\n';
      isnl = true;
  ...
  inline bool valeq(const module_value& x, const module_value& y) {return (x.type == y.type && (x.type == module_string ? boost::iequals(x.val.s, y.val.s) : x == y));}
  ...
  void outdata::printnotes(std::ostream& out, const module_partobj p, module_measobj& m, module_noteobj& n0, const std::vector<module_markobj>& mevs) {
    ...
    while (true) {
      ...
      if (mev != mevs.end()) {
	module_value ti;
	if (n) ti = module_vtime(n); else {
	  ti.type = module_rat;
	  ti.val.r = module_makerat(std::numeric_limits<fomus_int>::max() - 1, 1);
	}
	...
	first = false;
      ...
      for (module_value* i = wh.val.l.vals, *ie = wh.val.l.vals + wh.val.l.n; i < ie; ++i) {
	...
	if (module_vtime(n) >= mv.modval && module_part(m) == p) {
	  ...
	  mm << "time" << ' ' << mv.modval; // << " ";
	  prints.push_back(regs[type_time] = new printbase(mm.str(), mv, nn, withrets[type_time], withrets[type_time] ? 0 : twidth));
	  mm.str("");
	  if (module_setting_rval(m, nbeatsid) <= (fomus_int)0) {
	    mm << "duration" << ' ' << module_vdur(m); // << " ";
	    prints.push_back(regs[type_dur] = new printbase(mm.str(), mv, nn, withrets[type_dur], withrets[type_dur] ? 0 : twidth));
	    mm.str("");
	  }
	  mm << info_valstr(m);
	  prints.push_back(regs[type_time] = new printbase(mm.str(), mv, ++nn, true, twidth));
	  m = module_nextmeas();
	}
      }
      for (int i = 0; i < type_n; ++i) {
	if (!keepregs[i]) regs[i] = 0;
	...
	case type_voice:
	  {
		...
		for (const int* i1 = regs[type_voice]->val.ints.ints, *i2 = mv.ints.ints, *i2e = mv.ints.ints + mv.ints.n; i2 < i2e; ++i1, ++i2) {
		  if (*i1 != *i2) goto VSKIP;
		}
		goto MORE;
	      }
...
#warning "*** make these prefixes user configurable ***"
	      valstr << '(';
	      for (const int* v = mv.ints.ints, *ve = mv.ints.ints + mv.ints.n; v < ve; ++v) {
		if (v > mv.ints.ints) valstr << ' ';
		valstr << *v;
	      }
	      valstr << ')';
	    }
	...
	case type_time:
	  {
	    ...
	    if (regs[type_time] && valeq(regs[type_time]->val.modval, mv.modval)
		&& !(regs[type_gtime] && module_vgracetime(n).type == module_none)) goto MORE;
	...
	case type_gtime:
	  {
	    ...
	    if (mv.modval.type == module_none || mv.modval < (fomus_int)-1000 || mv.modval > (fomus_int)1000
		|| (regs[type_gtime] && valeq(regs[type_gtime]->val.modval, mv.modval))) goto MORE;
	    valstr << "grace" << ' ' << mv.modval;
	  }
	  break;
	case type_dyn:
	  if (ism || isr) goto MORE;
	  {
	    mv.modval = module_dyn(n);
	    if (regs[type_dyn] && valeq(regs[type_dyn]->val.modval, mv.modval)) goto MORE;
	...
	case type_dur:
	  {
	    ...
	    if (x.type != module_none && (x < (fomus_int)-1000 || x > (fomus_int)1000)) {
	      valstr << "duration" << ' ' << (x < (fomus_int)0 ? '-' : '+');
	      regs[type_dur] = 0;
	    } else {
	      mv.modval = module_vdur(n);
	      if (regs[type_dur] && valeq(regs[type_dur]->val.modval, mv.modval)) goto MORE;
	...
	case type_part:
	  {
	    ...
	    if (regs[type_part] && mv.ptr != regs[type_part]->val.ptr) goto MORE;
	...
	case type_pitch:
	  if (ism || isr) goto MORE;
	  {
	    ...
	    } else mv.modval = module_vpitch(n);
	    if (regs[type_pitch] && valeq(regs[type_pitch]->val.modval, mv.modval)) goto MORE;
	    valstr << "pitch" << ' ' << mv.modval;
	  }
	  break;
	default: assert(false);
	}
	prints.push_back(regs[i] = new printbase(valstr.str(), mv, nn, withrets[i], withrets[i] ? 0 : twidth));
	MORE: ;
      }
    ...
    bool di = true;
    std::for_each(prints.begin(), prints.end(), boost::lambda::bind(&printbase::print, boost::lambda::_1, boost::lambda::var(out), boost::lambda::var(di)));
  }    
  
  int filenameid, outputid, nthreadsid;
  
  void outdata::modout_write(FOMUS fom, const char* filename) { // filename should be complete
    try {
	...
	f.open(fn.FS_FILE_STRING(), boost::filesystem::ofstream::out | boost::filesystem::ofstream::trunc);
	f << "// " << PACKAGE_STRING << '\n';
	{
	  time_t tim; 
	  if (time(&tim) != -1) {
	...
	{ // --------------------SETTINGS
	  if (whset != info_none) {
	    f << "// settings\n";
	    settingslist sets(fom, whset); // get sorted settings???  by module...
	    std::string curmod;
	    for (info_setting* i = sets.l.sets, *ie = sets.l.sets + sets.l.n; i < ie; ++i) {
	      if (i->id == filenameid || i->id == outputid || i->id == nthreadsid) continue;
	...
	{ // --------------------PARTS, INSTS, ETC..
	  ...
	  if (pi.n > 0) {
	    ...
	    for (info_objinfo *i = pi.objs, *e = pi.objs + pi.n; i < e; ++i) printobj(f, "percinst", *i);
	    f << '\n';
	    x = true;
	  }
	  ...
	  if (pi.n > 0) {
	    f << "// instruments\n";
	    for (info_objinfo *i = pi.objs, *e = pi.objs + pi.n; i < e; ++i) printobj(f, "inst", *i);
	    f << '\n';
	    x = true;
	  }
	  ...
	  if (pi.n > 0) {
	    f << "// metaparts\n";
	    for (info_objinfo *i = pi.objs, *e = pi.objs + pi.n; i < e; ++i) printobj(f, "metapart", *i);
	    f << '\n';
	    x = true;
	  }
	  ...
	  if (pi.n > 0) {
	    ...
	    for (info_objinfo *i = pi.objs, *e = pi.objs + pi.n; i < e; ++i) printobj(f, "measdef", *i);
	    f << '\n';
	    x = true;
	  }
	  if (x) f << "// " << std::string(fwidth - 3, '-') << "\n\n";
	}
...
const char* module_longname() {return "FOMUS File Output";}
const char* module_author() {return "(fomus)";}
const char* module_doc() {return "Writes `.fms' files (FOMUS's native file format).";}
void* module_newdata(FOMUS f) {
  return new fmsout::outdata(f);
}
void module_freedata(void* dat) {delete (fmsout::outdata*)dat;}
...
void module_init() {
  fmsout::whichmap.insert(fmsout::whichmap_val("none", info_none)); // return nothing
  fmsout::whichmap.insert(fmsout::whichmap_val("all", info_default)); // return all
  fmsout::whichmap.insert(fmsout::whichmap_val("config", info_config)); // return only config changed
  fmsout::whichmap.insert(fmsout::whichmap_val("score", info_global)); // return only changed in input file
  fmsout::equalsmap.insert(fmsout::equalsstr_val("equals", fmsout::equalsstr_equals));
  fmsout::equalsmap.insert(fmsout::equalsstr_val("colon", fmsout::equalsstr_colon));  
  ...
  case 0:
    {
      ...
      set->descdoc = "Indicates which global settings are saved to an `.fms' output file.  "
	"`none' means none, `score' means only those set globally in a score, "
	"`config' means those set in a score and set in the `.fomus' config file, "
	"and `all' means dump all setting values (including internal defaults).";
  ...
  case 1:
    {
      ...
      set->type = module_string;
      set->descdoc = "Whether to use `=' or `:' when outputting setting values to an `.fms' output file.  "
	"Set this value to `equals' to get `=' characters and `colon' to get `:' characters between setting names and values."
	;
  ...
  case 2:
    {
      ...
      set->type = module_int;
      set->descdoc = "Number of characters allowed per line in an `.fms' output file."
  ...
  case 3:
    {
      ...
      set->type = module_int;
      set->descdoc = "Number of spaces used for indentation in an `.fms' output file.";
  ...
  case 4:
    {
      ...
      set->descdoc = "Whether or not to group output events by part in an `.fms' output file."
	"  Setting this to `yes' creates separate sections for each part."
	"  Setting this to `no' organizes all events by their times, switching parts as necessary.";
      //set->typedoc = fmsout::writetabwidthtype; 
  ...
  case 5:
    {
      ...
      set->descdoc = "A list of event parameters to write without repetition in an `.fms' output file "
	"(i.e., values are written only when they change), making the output more concise and easier to read."
	"  Set this to the parameters that you don't want repeated in each note event."
	"  An asterisk after a type string indicates that non-repeating events should also be followed by a newline to aid visibility.";
...
void module_ready() {
  ...
  if (id < 0) {ierr = "missing required setting `note-accs'"; return;}
  mustbefirst.insert(id = module_settingid("note-microtones"));
  if (id < 0) {ierr = "missing required setting `note-microtones'"; return;}
  mustbefirst.insert(id = module_settingid("note-octaves"));
  if (id < 0) {ierr = "missing required setting `note-octaves'"; return;}
  mustbefirst.insert(id = module_settingid("note-symbols"));
  if (id < 0) {ierr = "missing required setting `note-symbols'"; return;}
  mustbefirst.insert(id = module_settingid("dur-dots"));
  if (id < 0) {ierr = "missing required setting `dur-dots'"; return;}
  mustbefirst.insert(id = module_settingid("dur-symbols"));
  if (id < 0) {ierr = "missing required setting `dur-symbols'"; return;}
  mustbefirst.insert(id = module_settingid("dur-tie"));
  if (id < 0) {ierr = "missing required setting `dur-tie'"; return;}
  mustbefirst.insert(id = module_settingid("tuplet-symbols"));
  if (id < 0) {ierr = "missing required setting `tuplet-symbols'"; return;}
  ...
  if (filenameid < 0) {ierr = "missing required setting `filename'"; return;}
  outputid = module_settingid("output");
  if (outputid < 0) {ierr = "missing required setting `output'"; return;}
  nthreadsid = module_settingid("n-threads");
  if (nthreadsid < 0) {ierr = "missing required setting `n-threads'"; return;}
}

// only fmsout.cc returns true here (outputs before processing anything)
fomus_bool modout_ispre() {return true;}
...
void modout_write(FOMUS f, void* dat, const char* filename) {((fmsout::outdata*)dat)->modout_write(f, filename);}
