...
namespace harms {
  ...
  int defid, natshowid, artshowid, strtypeid, openstrid, maxartintid, minintid, minpitid;
  ...
  fomus_rat bttos(const fomus_rat& base, const fomus_rat& t, const bool art, const fomus_rat& maxartint, const fomus_rat& minint) { // base+touched to sounding
    fomus_rat d(t - base);
    if ((art && d > maxartint) || d < minint) throw invalid();
    if (d == (fomus_int)12) return base + (fomus_int)12;
    if ((d == (fomus_int)7) || (!art && d == (fomus_int)7 + 12)) return base + ((fomus_int)12 + 7); // 12
    if ((d == (fomus_int)5) || (!art && d == (fomus_int)5 + 7 + 12)) return base + ((fomus_int)12 + 7 + 5); // 12 + 7
    if ((d == (fomus_int)4) || (!art && ((d == (fomus_int)4 + 5) || (d == (fomus_int)4 + 5 + 7) || (d == (fomus_int)4 + 5 + 7 + 12))))
      return base + ((fomus_int)12 + 7 + 5 + 4); // 12, 7 + 12, 5 + 7 + 12
    if ((d == (fomus_int)3) || (!art && d == (fomus_int)3 + 4 + 5 + 7 + 12)) return base + ((fomus_int)12 + 7 + 5 + 4 + 3); // 12 + 7 + 5 + 4
    throw invalid();
  }
  fomus_rat bstot(const fomus_rat& base, const fomus_rat& s, const fomus_rat& pt, const bool art, const fomus_rat& maxartint, const fomus_rat& minint) { // base+sounding to touched (pt = possible touched)
    fomus_rat d(s - base);
    ...
    const int *r;
    if (d == (fomus_int)12) r = h12; 
    else if (d == (fomus_int)12 + 7) r = h7; // TODO: can return multiple solutions!...  pick closest one to given "touched" value
    else if (d == (fomus_int)12 + 7 + 5) r = h5;
    else if (d == (fomus_int)12 + 7 + 5 + 4) r = h4;
    else if (d == (fomus_int)12 + 7 + 5 + 4 + 3) r = h3;
    else throw invalid();
    if ((art && (fomus_int)r[0] > maxartint) || (fomus_int)r[0] < minint) throw invalid();
    if (art || pt == (fomus_int)0) return base + module_makerat(r[0], 1); // 0 = just return default lowest one
    fomus_rat dst = {std::numeric_limits<fomus_int>::max() - 1, 1}, ret; // possible t is given, find closest
    for (; *r != 0; ++r) {
  ...
  fomus_rat tstob(const fomus_rat& t, const fomus_rat& s, const bool art, const std::set<fomus_rat>& open, const fomus_rat& maxartint, const fomus_rat& minint,
		  const fomus_rat& minpit) { // touched+sounding to base
    fomus_rat d(s - t);
    static const int h0[] = {12, 12 + 7, 12 + 7 + 5, 12 + 7 + 5 + 4, 12 + 7 + 5 + 4 + 3, 0}; // possible bases below sounding
    ...
    const int *r;
    if (d == (fomus_int)0) r = h0;
    else if (d == (fomus_int)12) r = h1; 
    else if (d == (fomus_int)12 + 7) r = h2;
    else if (d == (fomus_int)12 + 7 + 5) r = h3;
    else if (d == (fomus_int)12 + 7 + 5 + 4) r = h4;
    else throw invalid();
    for (; *r != 0; ++r) {
      fomus_rat ret(s - (fomus_int)*r);
      fomus_rat d0(t - ret);
      if (ret < minpit || (art && d0 > maxartint) || d0 < minint || (!open.empty() && open.find(ret) == open.end())) continue;
      return ret;
    }
    throw invalid();
  }
  const int tryhs[] = {12, 12 + 7, 12 + 7 + 5, 12 + 7 + 5 + 4, 12 + 7 + 5 + 4 + 3, 0}; // possible bases below sounding
  fomus_rat stob(const fomus_rat& s, const std::set<fomus_rat>& open, const fomus_rat& minpit) { // sounding to base (natural harmonics only!)
    for (const int *r = tryhs; *r != 0; ++r) {
      fomus_rat ret(s - (fomus_int)*r);
      if (ret < minpit || (!open.empty() && open.find(ret) == open.end())) continue;
      return ret;
    }
    throw invalid();
  }
  ...
  enum optsenum {
    opts_play, opts_touch, opts_sound, opts_string, opts_circle, opts_stringtext,
    opts_none
  };
  ...
  struct noteholderbase {
    ...
    void skipassign() const {module_skipassign(n);}
    void insert(const mark& mk) {ins.insert(mk);}
    void insert(const std::multiset<mark, markless>& ms) {ins.insert(ms.begin(), ms.end());}
    void insorig(const mark& mk) {orig.insert(mk);}
    void doerr(const module_noteobj n, const char* str);
    virtual void notdel() {}
    virtual bool isntdel() {return true;}
  };
  struct noteholder:public noteholderbase {
    ...
    void assign();
    void delet() {del = true;}
    void notdel() {del = false;}
    bool isntdel() {return !del;}
  };
  void noteholder::assign() {
    if (del) special_assign_delete(n);
    else {
      inplace_set_difference<std::multiset<mark, markless>::iterator, std::multiset<mark, markless>::iterator, markless>(orig, ins);
  ...
  inline void getaccs(const fomus_rat& frnote, const fomus_rat& facc, const fomus_rat& tonote, fomus_rat& acc1, fomus_rat& acc2) {
  ...
  struct newnote:public noteholderbase {
    ...
    newnote(const module_noteobj n, const fomus_rat& note, const fomus_rat& frnote, const fomus_rat& facc):noteholderbase(n), note(note) {
  ...
  void newnote::assign() {
    ...
    for (std::multiset<mark, markless>::const_iterator i(ins.begin()); i != ins.end(); ++i) {
  ...
  struct harmsdata {
    ...
    void run() {
      ...
      for (const module_value *i(op.val.l.vals), *ie(op.val.l.vals + op.val.l.n); i < ie; ++i) open.insert(GET_R(*i));
	...
	try {
	  for (const module_markobj* i(m.marks), *ie(m.marks + m.n); i != ie; ++i) {
		...
		throw conflicting();
	      ...
	      sonh = nh;
	      wh = which_nat;
	      sov = no; //getmarknum(n, *i); // returns the note if the marknum is NONE
	      nh->insorig(*i);
	      break;
	    }
	    case mark_artharm_sounding: {
	    ...
	    //   if (acc1 == (fomus_int)0) acc1 = acc10; else if ((acc1 < (fomus_int)0 && acc10 > (fomus_int)0) || (acc1 > (fomus_int)0 && acc10 < (fomus_int)0))
	    //     throw invalid();
	    //   if (acc2 == (fomus_int)0) acc2 = acc20; else if ((acc2 < (fomus_int)0 && acc20 > (fomus_int)0) || (acc2 > (fomus_int)0 && acc20 < (fomus_int)0))
	    //     throw invalid();
	    // }
	  SKIPIT: ; 
	  }
	  ...
	  if (!module_isendchord(n)) continue;
	  if (so || to || pl || st) { // user wants a harmonic, figure it out
	    ...
	    if (pl) { // it's ARTIFICIAL--assume (!st)
	      if (st) throw conflicting(); // THROWS
	      if (to && so) {
		tov = bstot(plv, sov, tov, true, maxartint, minint); // readjust tov!
	      } else if (to && !so) {
		sov = bttos(plv, tov, true, maxartint, minint);
	      } else if (so && !to) {
		tov = bstot(plv, sov, module_makerat(0, 1), true, maxartint, minint);
	      } else throw notenough();
		...
		tov = bstot(stv, sov, module_makerat(0, 1), false, maxartint, minint);
	      } else throw notenough();  
	    ...
	    } else notenough(); // not enough information
	  GOTIT: // now check it, either pl and st are true, indicating type of harmonic
	    assert(pl != st);
	    // if (st && !open.empty() && open.find(stv) == open.end())
	    //   throw invalid();
	    // if (ttos(pl ? plv : stv, tov) != sov)
	    //   throw invalid();
	    struct module_list shw(module_setting_val(n0, pl ? artshowid : natshowid).val.l);
	    std::multiset<mark, markless> ins;
	    std::for_each(notevect.begin(), notevect.end(), boost::lambda::bind(&noteholder::delet, boost::lambda::_1));
	    boost::ptr_vector<newnote> newnotes;
	    fomus_rat fra(module_fullacc(n0));
	    for (const module_value *i(shw.vals), *ie(shw.vals + shw.n); i != ie; ++i) {
	      assert(i->type == module_string);
	      switch (opts.find(i->val.s)->second) {
	      case opts_play: {
		if (plnh) plnh->notdel();
		else newnotes.push_back((newnote*)(plnh = new newnote(n0, plv, nomin, fra)));
		plnh->insert(mark(mark_artharm_base));
	      } break;
	      case opts_string: {
		if (stnh) stnh->notdel();
		else newnotes.push_back((newnote*)(stnh = new newnote(n0, stv, nomin, fra)));
		stnh->insert(mark(mark_natharm_string));
	      } break;
	      case opts_touch: {
		if (tonh) tonh->notdel();
		else newnotes.push_back((newnote*)(tonh = new newnote(n0, tov, nomin, fra)));
		tonh->insert(mark(pl ? mark_artharm_touched : mark_natharm_touched));
	      } break; // THIS IS FOR BOTH TYPES OF HARMONICS
	      case opts_sound: {
		if (sonh) sonh->notdel();
		else newnotes.push_back((newnote*)(sonh = new newnote(n0, sov, nomin, fra)));
		sonh->insert(mark(pl ? mark_artharm_sounding : mark_natharm_sounding));
	      } break;
	      case opts_circle: {
		ins.insert(mark(mark_harm));
	      } break;
	      case opts_stringtext: {
		switch (strtypes.find(module_setting_sval(n0, strtypeid))->second) {
		case tstyle_let:
		  ins.insert(mark(mark_sul, stv.num / stv.den)); // specify the string, which is in a diferent font ("/" converts to int
		  break;
		case tstyle_rom: {
		  std::set<fomus_rat>::const_iterator i(open.find(stv));
		  if (i == open.end()) break;
		  int n = 0;
		  do {++n; ++i;} while (i != open.end());
	    ...
	    if (stnh && stnh->isntdel()) stnh->insert(ins);
	    else if (plnh && plnh->isntdel()) plnh->insert(ins);
	    else if (tonh && tonh->isntdel()) tonh->insert(ins);
	    else if (sonh && sonh->isntdel()) sonh->insert(ins);
	    std::for_each(newnotes.begin(), newnotes.end(), boost::lambda::bind(&newnote::assign, boost::lambda::_1));
	    std::for_each(notevect.begin(), notevect.end(), boost::lambda::bind(&noteholder::assign, boost::lambda::_1));
	    notevect.clear();
	    so = to = pl = st = false;
	...
	std::for_each(notevect.begin(), notevect.end(), boost::lambda::bind(&noteholder::skipassign, boost::lambda::_1));
	notevect.clear();
	so = to = pl = st = false;
	sonh = tonh = plnh = stnh = 0;
	wh = which_none;
  ...
  void run_fun(FOMUS fom, void* moddata) {((harmsdata*)moddata)->run();}
  const char* err_fun(void* moddata) {return ((harmsdata*)moddata)->err();}

  const char* ashowtypes = "(string_show string_show ...), string_show = base|touched|sounding|circle";
  ...
  int valid_ashowtypes(const struct module_value val) {return module_valid_listofstrings(val, -1, -1, 1, -1, valid_ashowtypes_aux, ashowtypes);}
  const char* nshowtypes = "(string_show string_show ...), string_show = string|touched|sounding|circle|stringtext";
  int valid_nshowtypes_aux(int x, const char* val) {
    const static bool arr[] = {false, true, true, true, true, true};
    optstype::const_iterator i(opts.find(val));
    return (i != opts.end() && arr[i->second]);
  }
  int valid_nshowtypes(const struct module_value val) {return module_valid_listofstrings(val, -1, -1, 1, -1, valid_nshowtypes_aux, nshowtypes);}
  ...
  int valid_deftypes(const struct module_value val) {return module_valid_string(val, 1, -1, valid_deftypes_aux, deftypes);}
  // const char* strtypetypes = "letter|roman";
  // int valid_strtypetypes_aux(const char* val) {return (strtypes.find(val) != strtypes.end());}
  // int valid_strtypetypes(const struct module_value val) {return module_valid_string(val, 1, -1, valid_strtypetypes_aux, strtypetypes);}
  const char* openstrtype = "(note|rational0..128 note|rational0..128 ...)";
  int valid_openstrtype(const struct module_value val) {return module_valid_listofrats(val, -1, -1, module_makerat(0, 1), module_incl, module_makerat(128, 1), module_incl, 0, openstrtype);}

  const char* minrattype = "rational3..7";
  int valid_minrattype(const struct module_value val) {
    return module_valid_rat(val, module_makerat(3, 1), module_incl, module_makerat(7, 1), module_incl, 0, minrattype);
  }
  const char* maxrattype = "rational>=3";
  int valid_maxrattype(const struct module_value val) {
    return module_valid_rat(val, module_makerat(3, 1), module_incl, module_makerat(0, 1), module_nobound, 0, maxrattype);
  }
...
const char* module_longname() {return "Harmonics";}
const char* module_author() {return "(fomus)";}
const char* module_doc() {return "Facilitates and checks notation of string harmonics.";}
void* module_newdata(FOMUS f) {return new harmsdata;} //new accsdata;}
void module_freedata(void* dat) {delete (harmsdata*)dat;} //delete (accsdata*)dat;}
int module_priority() {return 0;} 
enum module_type module_type() {return module_modspecial;}
int module_itertype() {return module_bymeas | module_byvoice | module_norests;}
  ...
  case 0:
    {
      ...
      set->descdoc = "This is a list of strings specifying which notes and symbols of an artificial harmonic are to appear in the score."
  ...
  case 1:
    {
      ...
      set->type = module_list_strings;
      set->descdoc = "This is a list of strings specifying which notes and symbols of an natural harmonic are to appear in the score."
	"  Set this to determine how natural harmonics are notated."
	"  The choices are `string', `touched' and `sounding', `circle' and `stringtext'.  "
	"`circle' places a harmonic symbol in the score above the note while `stringtext' inserts a \"Sul\" text mark with the proper string name.";
  ...
  case 2:
    {
      ...
      set->descdoc = "Specifies the open strings of a string instrument or part."
	"  Set this in the definition of a string instrument so that natural and artificial harmonics may be calculated correctly." ;
  ...
  case 3:
    {
      ...
      set->type = module_rat;
      set->descdoc = "The maximum interval (in semitones) allowed between the base and touched pitches of an artificial harmonic.  "
	"Set this in the definition of a string instrument to the widest interval a player can be expected to stretch.  "
  ...
  case 4:
    {
      ...
      set->type = module_rat;
      set->descdoc = "The minimum interval (in semitones) allowed between the base and touched pitches of an artificial harmonic or the "
...
void module_ready() {
  ...
  if (strtypeid < 0) {ierr = "missing required setting `sul-style'"; return;}
  minpitid = module_settingid("min-pitch");
  if (minpitid < 0) {ierr = "missing required setting `min-pitch'"; return;}
}
