...
namespace merge {
  ...
  struct noteobjbase {
    ...
    noteobjbase(const module_noteobj n):n(n), voice(module_voice(n)), isrest(module_isrest(n)), marks(module_marks(n)),
					off(module_time(n)), dur(module_dur(n)), chordsc(module_setting_fval(n, chordscid)),
					distsc0(module_setting_fval(n, distscid)) {}
    bool isntvoice_or_isrest(const int v) const {return voice != v || isrest;}
    bool isvoice(const int v) const {return voice == v;}
  };
  struct noteobj:public noteobjbase {
    fomus_float val, distsc;
    module_noteobj mergeto;
    noteobj(const noteobjbase& x):noteobjbase(x), val(1), distsc(0), mergeto(0) {}
    fomus_float getsc() const {return val + distsc;}
  ...
  // called from vmergeable(), from newnode()---if mergeable, also sets `val', which is the base score
  bool mergeable(noteobj& n1, noteobj& n2, int& wh) { // for comparing notes in sequence
    if (!n1.isrest) { // note, rest
      if (n2.isrest) {
	if (wh == 2) return false; // conflict = no deal
	wh = 1; // note merges into rest--setting wh means it *must* go in that direction
      }
    }
    if (!n2.isrest) {
      if (n1.isrest) {
	if (wh == 1) return false;
	wh = 2;
      }
    }
    if (n1.off != n2.off || n1.dur != n2.dur || n1.marks.n != n2.marks.n) return false; // same off and dur
    for (const module_markobj *i1(n1.marks.marks), *ie1(n1.marks.marks + n1.marks.n), *i2(n2.marks.marks); i1 < ie1; ++i1, ++i2) {
      if (module_markid(*i1) != module_markid(*i2)) return false;
      const char* x = module_markstring(*i1);
      const char* y = module_markstring(*i2);
      if (std::string(x ? x : "") != std::string(y ? y : "")) return false;
      module_value x0(module_marknum(*i1));
      module_value y0(module_marknum(*i2));
      if ((x0.type == module_none) ? (y0.type != module_none) : (y0.type == module_none || x0 != y0)) return false;
    }
    if (n1.isrest) n1.val = 0; // rests have score base 0
    if (n2.isrest) n2.val = 0;
  ...
  bool vmergeable(noteobjvect& vect, const int v1, const int v2) { // vect = list of notes, v1 = (upper), v2 = (lower)
    boost::ptr_vector<noteobj>::iterator i1(vect.begin()), i2(vect.begin());
    bool isany = false;
    int wh = 0; // 1 = v1 ---> v2, 2 = v2 <--- v1, 0 = either way is a-okay
    while (true) {
      while (i1 != vect.end() && !i1->isvoice(v1)) ++i1;
      while (i2 != vect.end() && !i2->isvoice(v2)) ++i2;
      if (i1 == vect.end() && i2 == vect.end()) return isany; // if voice doesn't exist, then it isn't mergeable
      if (i1 == vect.end() || i2 == vect.end()) return false; // voices should be same length
      isany = true;
      if (!mergeable(*i1++, *i2++, wh)) return false;
    }
    assert(wh != 0);
    i1 = i2 = vect.begin();
    while (true) { // iterate in parallel through voices again and set which ones merge to which
      while (true) {
	if (i1 == vect.end()) return true;
	if (i1->isvoice(v1)) break;
	++i1;
      }
      while (!i2->isvoice(v2)) ++i2;
      if (wh == 1) i1->mergeto = i2->n; else i2->mergeto = i1->n;
      ++i1; ++i2;
    }
  }
  ...
  // node contains merges, # of notes (this is being minimized!, chords count as 1?), # of notes across (to keep track of dist--includes measure boundaries)
  struct node _NONCOPYABLE {
    ...
    int valid;
#endif    
    node(std::auto_ptr<noteobjvect>& vect0, const pairvect& switchs, const bool lastinm, const boost::ptr_list<nodemap>::iterator& it)
      :vect(vect0), switchs(switchs), // vector ot noteobjs have been prepared & scored already
       sc(accumulate_results<fomus_float>(vect->begin(), vect->end(), boost::lambda::bind(&noteobj::getsc, boost::lambda::_1))),
       dist(0), it(it) {
#ifndef NDEBUG
      valid = 12345;
#endif      
      ...
      for (boost::ptr_vector<noteobj>::const_iterator i(vect->begin()); i != vect->end(); ++i) {
	fomus_int d = ++cnt[i->voice];
	if (d > dist) dist = d;
      }
      if (lastinm) ++dist; // add one for measure boundary
    }
#ifndef NDEBUG
    bool isvalid() const {return valid == 12345;}
#endif          
    fomus_float getscore() const {return sc;}
    void assign() const {assert(isvalid()); std::for_each(vect->begin(), vect->end(), boost::lambda::bind(&noteobj::assign, boost::lambda::_1));}
  ...
  inline bool nodesamemerge(const node& x, const node& y) {
    assert(x.isvalid());
    assert(y.isvalid());
    for (pairvect::const_iterator i(x.switchs.begin()), j(y.switchs.begin()); ; ++i, ++j) {
      if (i == x.switchs.end()) return j == y.switchs.end();
      if (j == y.switchs.end() || *i != *j) return false;
    }
  }
  
  // get all combinations--only merge upwards
  void collectmerge(const int* to, const int* fr, const int* end, pairvect& vect, boost::ptr_vector<pairvect>& merges) {
    if (fr >= end) {
      if (!vect.empty()) merges.push_back(new pairvect(vect));
    } else {
  ...
  struct divpt {
    ...
    void set(const fomus_rat& pt0) {pt = pt0; end = false;}
    divpt(const fomus_rat& pt, int):pt(pt), end(true) {}
    void set(const fomus_rat& pt0, int x) {pt = pt0; end = x;}
    void set(const divpt& x) {pt = x.pt; end = x.end;}
  };
  ...
  struct data {
    ...
    boost::ptr_vector<pairvect> merges; // list of merges (from which voice to which voice)
    ...
    union search_score getscore(struct search_nodes& nodes) {
      ...
      if (nodes.n > 1) {
	fomus_int d = 0;
	fomus_int mindist = (*(node**)(nodes.nodes + nodes.n - 2))->mindist(); // it's a setting
	for (node** i = (node**)(nodes.nodes + nodes.n - 2); i >= (node**)nodes.nodes; --i) {
	  if (nodesamemerge(**i, l)) d += (*i)->dist; else break;
	  if (d >= mindist) {
	    ret.f = l.sc;
	    return ret;
	  }
	...
	collectmerge(vl.ints, vl.ints + 1, vl.ints + vl.n, vect, merges);
      ...
      merges.push_back(new pairvect(1, std::pair<int, int>(0, 0))); // "no merge"
      return merges.size();
    }
    void assignnext(const int choice) {
      DBG((ass == nodes.begin() ? "ASSIGNING FIRST!" : "ASSIGNING!") << std::endl);
	...
	boost::ptr_list<nodemap>::iterator pl(prev == api.begin ? nodes.begin() : boost::next(prev->it));
	if (pl != nodes.end()) { // nodes is a list
	  nodemap::iterator i(pl->find(choice)); // in the map--choice should be there
	...
	if (ass == nodes.end()) ass = boost::prior(nodes.end());
      }
	...
	if (!nd) goto STARTHERE;
	while (isl || module_time(nd) < nxdv.pt) {
	  nobs.push_back(new noteobjbase(nd));
	  DBG("pushed back t=" << module_time(nd) << " onto nobs ending at " << nxdv.pt << std::endl);
	STARTHERE:
	  nd = module_nextnote();
	  DBG("merge: nextnote = " << nd << std::endl);
	  if (!nd) {
	    if (nobs.empty()) return api.end;
	    break;
	  }
	}
      } while (nobs.empty());
      assert(nobs.size() != 0); // node is a group of notes in all voices that belong to a measure division (ea. node has a mapping from/to spec)
      int n = 0;
      assert(!nodes.empty()); boost::ptr_list<nodemap>::iterator ee(boost::prior(nodes.end()));
      for (boost::ptr_vector<pairvect>::const_iterator i(merges.begin()); i != merges.end(); ++i) { // complete list of merges
	std::auto_ptr<noteobjvect> x(new noteobjvect);
	for (boost::ptr_vector<noteobjbase>::const_iterator j(nobs.begin()); j != nobs.end(); ++j) x->push_back(new noteobj(*j));
	if (i->front().first > 0) { // no-merge--always allow this
	  for (pairvect::const_iterator pp(i->begin()); pp != i->end(); ++pp) { // go through sequence of merges, make sure every one is mergeable
	    if (!vmergeable(*x.get(), pp->first, pp->second)) goto SKIPINS;
	  }
	}
    ...
    bool isoutofrange(const node* node1, const node* node2) {
      ...
      fomus_int d = 0;
      for (boost::ptr_list<nodemap>::const_iterator i(node1->it), ie(node2->it); i != ie; ++i) { // yes, I want <node2, not <=node2
	assert(!i->empty());
  ...
  int valid_mergeminnotes(const struct module_value val) {return module_valid_int(val, 0, module_incl, 0, module_nobound, 0, mergeminnotestype);}
  const char* mergechordsscoretype = "real>=0";
  int valid_mergechordsscore(const struct module_value val) {
//     module_value z;
//     z.type = module_int;
//     z.val.i = 0;
    return module_valid_num(val, module_makeval((fomus_int)0), module_incl, module_makeval((fomus_int)0), module_nobound, 0, mergechordsscoretype);
  }
...
//     z.type = module_int;
//     z.val.i = 0;
    return module_valid_num(val, module_makeval((fomus_int)0), module_incl, module_makeval((fomus_int)0), module_nobound, 0, mergedistscoretype);
  }
  ...
  void search_assign(void* moddata, int choice) {((data*)moddata)->assignnext(choice);} // makes a solution assignment & reports it so that other phases of the program can continue
  union search_score search_get_score(void* moddata, struct search_nodes nodes) {return ((data*)moddata)->getscore(nodes);}
  search_node search_new_node(void* moddata, search_node prevnode, int choice) {return ((data*)moddata)->newnode((node*)prevnode, choice);} // return NULL if no valid next node or END if at end of search--`prevnode' might be BEGIN (get the first node)
  ...
  int search_is_outofrange(void* moddata, search_node node1, search_node node2) {return ((data*)moddata)->isoutofrange((node*)node1, (node*)node2);}
...
const char* module_longname() {return "Merge Voices";}
const char* module_author() {return "(fomus)";}
const char* module_doc() {return "Merges separate voices with identical rhythms.  Also adds text markings such as \"a2,\" \"I\" and \"II\" where appropriate.";}
void* module_newdata(FOMUS f) {return new data;}
void module_freedata(void* dat) {delete (data*)dat;}
int module_priority() {return 0;}
enum module_type module_type() {return module_modmerge;}
int module_itertype() {return module_bypart;}
...
int module_get_setting(int n, module_setting* set, int id) {
  ...
  case 0:
    {
      ...
      set->descdoc = "The minimum number of note events required between changes in how voices are \"merged\" "
	"(i.e., separate voices are notated as chords or as monophonic lines marked \"a2,\" etc.).  "
	"Set this to a higher value to cause merges to occur over larger spans of notes and vice versa.";
  ...
  case 1:
    {
      ...
      set->type = module_number;
      set->descdoc = "Score encouraging separate voices to be merged together into chords.  "
	"Increasing this increases the chance of separate voices merging together into chords."
	;
  ...
  case 2:
    {
      ...
      set->type = module_number;
      set->descdoc = "Score encouraging merged notes to be close together pitchwise.  The higher this value is, the greater the penalty for merging "
  ...
  case 4:
	...
	"  Set this to the name of an engine module to change the search algorithm used for merging voices.";
