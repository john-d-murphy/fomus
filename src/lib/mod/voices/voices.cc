...
namespace voices {
  ...
  // setting ids
  int vertmaxid, heapsizeid, vertmaxscid, previsgraceid, voiceolapid,
      voicecrossid, /*balanceid,*/ smoothid, adjsmoothid, exponid, octdistid,
      beatdistid, rangeid, distmodid, enginemodid, connectid, dissimid;

  struct voicenode _NONCOPYABLE {
    module_noteobj note;
    int voice;
    fomus_float vertmaxpenalty, previsgracescore, voiceolappenalty,
        voicecrosspenalty, /*balancepenalty,*/ smoothpenalty, connectpenalty,
        dissimpenalty, adjsmoothpenalty;
    fomus_float expon;
#ifndef NDEBUG
    int valid;
#endif
    ...
    voicenode(const module_noteobj note, const int voice)
        : note(note), voice(voice),
          ...
          valid(12345),
#endif
          ti(module_time(note)), teti(module_tiedendtime(note)),
          no(module_pitch(note)), isgr(module_isgrace(note)) {
      initstuff();
    }
    void initstuff() {
  ...
  struct scorenode {
    ...
    fomus_float dist;
    scorenode(const voicenode* node, const fomus_float dist)
        : node(node), dist(dist) {}
  };

  // voice overlap and cross--overlapping notes in wrong order--should be
  // penalized severely--crossing notes should be penalized gently
  inline fomus_float voiceolap(const voicenode& n1, const voicenode& n2) {
    if (n1.teti > n2.ti) { // must be overlapping...
      if ((n1.no > n2.no && n1.voice > n2.voice) ||
          (n2.no > n1.no && n2.voice > n1.voice))
        return n2.voiceolappenalty;
    }
    return 0;
  }
  inline fomus_float voicecross(const voicenode& n1, const voicenode& n2) {
    if (n1.teti <= n2.ti) { // must NOT be overlapping...
      if ((n1.no > n2.no && n1.voice > n2.voice) ||
          (n2.no > n1.no && n2.voice > n1.voice))
        return n2.voicecrosspenalty;
    }
    return 0;
  }
  inline fomus_float dissim(const voicenode& n1, const voicenode& n2) {
    assert(n1.ti <= n2.ti);
    if (n1.voice == n2.voice && n1.teti > n2.ti &&
        (n1.teti != n2.teti || n1.ti != n2.ti))
      return n2.dissimpenalty;
    return 0;
  }
  inline fomus_float smooth(const voicenode& n1, const voicenode& n2) {
    if (n1.teti < n2.ti) { // must NOT be overlapping or touching...
      return fabs(module_rattofloat(n1.no - n2.no)) *
             ((double) 1 / (double) 12) * n2.smoothpenalty;
    }
    return 0;
  }
  ...
  // a grace note is touching the current note--boost score to maintain
  // continuity
  ...
  struct voicesdata _NONCOPYABLE {
    ...
    bool isoutofrange(const search_node n1, const search_node n2) const {
      return diface.is_outofrange(diface.moddata, ((voicenode*) n1)->note,
                                  ((voicenode*) n2)->note);
    }
    ...
    search_score getscore(const search_nodes& nodes)
      ...
      if (!arr.empty()) {
        ...
        for (std::vector<scorenode>::const_iterator i(arr.begin());
          ...
          assert(i->dist > 0);
          ll += i->dist;
          mx += (previsgrace(*i->node, n2) + voiceolap(*i->node, n2) +
                 voicecross(*i->node, n2) + dissim(*i->node, n2) +
                 smooth(*i->node, n2) + adjsmooth(*i->node, n2)) *
                i->dist;
        }
        sc.f += mx / ll;
      }
    ...
    fomus_float vertmax(const std::vector<scorenode>& arr,
                        const voicenode& n2) const {
      ...
      for (std::vector<scorenode>::const_iterator i(arr.begin());
        ...
        if (et > n2.ti) { // overlapping notes only
          std::map<int, int>::iterator c(cnt.find(
              i->node->voice)); // TODO: account for all possible voices
          if (c == cnt.end())
            cnt.insert(std::map<int, int>::value_type(i->node->voice, 1));
          else
            ++c->second;
      ...
      for (std::vector<int>::const_iterator i(voices.begin());
           i != voices.end();
           ++i) { // TODO: only those present in simult. notes
        std::map<int, int>::iterator c(cnt.find(*i));
        int n = (c == cnt.end() ? 0 : c->second);
        if (n > n2.vmax)
          sc += (n - n2.vmax) * n2.vertmaxpenalty;
      }
      return sc;
    }
    search_node newnode(const search_node prevnode, const int choice) {
      ...
      if (!module_hasvoice(n, voices[choice]))
        return 0;
  ...
  void search_assign(void* moddata, int choice) {
    ((voicesdata*) moddata)->assignnext(choice);
  ...
  int search_is_outofrange(void* moddata, search_node node1,
                           search_node node2) {
    return ((voicesdata*) moddata)->isoutofrange(node1, node2);
  }

  // ------------------------------------------------------------------------------------------------------------------------

  const char* vertmaxtype = "integer>=1";
  int valid_vertmax(const struct module_value val) {
    return module_valid_int(val, 1, module_incl, 0, module_nobound, 0,
                            vertmaxtype);
  }
...
void module_fill_iface(void* moddata, void* iface) { // module
  ...
  ((search_iface*) iface)->nchoices =
      ((voicesdata*) moddata)->getlistofvoices(p);
...
const char* module_doc() {
  return "Separates notes into voices";
}
void* module_newdata(FOMUS f) {
  return new voicesdata;
}
void module_freedata(void* dat) {
  delete (voicesdata*) dat;
}
int module_priority() {
  return 0;
}
enum module_type module_type() {
  return module_modvoices;
}
int module_itertype() {
  return module_bypart | module_norests | module_firsttied | module_noperc;
}
...
int module_get_setting(int n, module_setting* set, int id) {
  ...
  case 0: {
    ...
    set->type = module_int;
    set->descdoc =
        "The maximum number of notes an instrument is able to play "
        "simultaneously in a single voice."
        "  Set this in an instrument or part to the maximum number of notes "
        "that can appear in a chord."
        "  For example, most instruments can only play one note at a time but "
        "keyboard instruments could have a value of 5 or more."
        "  String instruments might also require larger values here for chords "
        "or double stops, etc..";
  ...
  case 2: {
    ...
    set->descdoc =
        "Score when the number of overlapping notes in one voice doesn't "
        "exceed the limit for the instrument.  "
        "Increasing this increases the likelihood that chords exceeding the "
        "`vertmax' limit are split into separate voices (if possible).";
  ...
  case 3: {
    ...
    set->type = module_number;
    set->descdoc =
        "Score when the previous note in the same voice is a grace note (to "
        "insure continuity).  "
  ...
  case 4: {
    ...
    set->type = module_number;
    set->descdoc =
        "Score when overlapping notes are ordered as expected (i.e., "
        "overlapping higher notes are in upper voices and vice versa).  "
        "Increasing this value increases the chance of overlapping notes being "
        "ordered correctly.";
    set->typedoc = scoretype;
  ...
  case 5: {
    ...
    set->type = module_number;
    set->descdoc =
        "Score when notes in close proximity to each other are also close to "
        "each other in pitch.  "
  ...
  case 6: {
    ...
    set->type = module_number;
    set->descdoc =
        "Score encouraging dissimilar rhythms in separate voices."
        "  This also encourages notes containing the same rhythms to be "
        "appropriately grouped into chords.  "
        "Increasing this value decreases the chance of multiple rhythms being "
        "notated in the same voice with a lot of chords and ties.";
    set->typedoc = scoretype;
  ...
  case 7: {
    ...
    set->descdoc = "If the `cartdist' distance or similar module is selected, "
                   ...
                   "decisions regarding voice assignments and vice versa.";
    set->typedoc = scoretype;
  ...
  case 9: {
    ...
    set->type = module_number;
    set->descdoc =
        "The maximum distance allowed for voice assignment calculations."
  ...
  case 10: {
    ...
    set->descdoc =
        ...
        "making decisions regarding voice assignments.";
  ...
  case 12: {
    ...
    set->descdoc = "Engines provide different types of search functionality to "
                   ...
                   "search algorithm used for finding voice assignments.";
  ...
  case 13: {
    ...
    set->type = module_number;
    set->descdoc =
        "Score when notes crossing in separate voices are ordered as expected "
        "(i.e., higher notes are in upper voices and vice versa).  "
        "Increasing this increases the chance of crossing notes being ordered "
        "correctly.";
    set->typedoc = scoretype;
  ...
  case 14: {
    ...
    set->type = module_number;
    set->descdoc =
        "Encourages neighboring notes to be in a single voice (prevents "
        "hockets between voices).  "
  ...
  case 15: {
    ...
    set->type = module_number;
    set->descdoc =
        "Score when adjacent notes are smoothly connected (i.e., smaller "
        "intervals are preferred over larger ones).  "
        "Increasing this value improves the voice leading in each voice.";
    set->typedoc = scoretype;
