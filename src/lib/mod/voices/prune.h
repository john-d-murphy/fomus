...
namespace prune {
  ...
  inline const char* name() {return "prune";};
  inline const char* prettyname() {return "Prune";}
  inline const char* author() {return "(built-in)";}
  inline const char* doc() {return "Prunes notes that overlap in each voice.";}
  inline void* newdata(FOMUS f) {return 0;}
  inline void freedata(void* dat) {}
  inline int priority() {return 0;}
  inline enum module_type modtype() {return module_modprune;}
