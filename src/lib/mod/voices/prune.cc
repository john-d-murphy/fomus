...
namespace prune {
  ...
  struct node {
    ...
    bool isgr;
    fomus_rat pi;
    node(const module_noteobj n):n(n), end(), ok(false), holes(ranges_init()), isgr(module_isgrace(n)), pi(module_isrest(n) ? module_makerat(-1, 1) : module_pitch(n)) {
      modutil_range x;
      if (isgr) {
    ...
    ~node() {ranges_free(holes);}
    void remove(const modutil_range& r) {
      assert(r.x2 > r.x1);
      ranges_remove(holes, r);
      struct modutil_ranges x(ranges_get(holes));
      if (x.n > 0) end = GET_R((x.ranges + x.n - 1)->x2); else end = module_makerat(std::numeric_limits<fomus_int>::min() + 1, 1);
    }
    ...
    bool isdirt(struct modutil_ranges& x) const {return (x.n != 1 ||
							 (isgr
							  ? (x.ranges->x1 != module_gracetime(n) || x.ranges->x2 != module_graceendtime(n))
							  : (x.ranges->x1 != module_time(n) || x.ranges->x2 != module_endtime(n)))
							 );}
  };
  ...
  bool isspecial(const node& x, const node& y) {
    ...
    for (const module_markobj* i = m1.marks, *ie = m1.marks + m1.n; i < ie; ++i) {
      for (const module_markobj* j = m2.marks, *je = m2.marks + m2.n; j < je; ++j) {
	if (module_markisspecialpair(module_markid(*i), module_markid(*j))) return true;
      }
    }
    return false;
  }
  ...
  enum prunetype {
    prune_cutoff, prune_breakup, prune_steal, prune_xfer
  };
  ...
  void blastassigns(boost::ptr_deque<node>& notes, const bool nlst) {
	...
	for (modutil_range* i = x.ranges, *ie = x.ranges + x.n; i < ie; ++i) {
	  DBG("PRUNE: " << module_time(no.n) << " - " << module_endtime(no.n) << " to " << GET_R(i->x1) << " - " << GET_R(i->x2) << std::endl);
	  prune_assign(no.n, GET_R(i->x1), GET_R(i->x2)); // clips represent begins/ends of NEW notes
	}
	prune_assign_done(no.n);
      } else module_skipassign(no.n);
      notes.pop_front();
    }
  }
  ...
  void prune_run_fun(FOMUS fom, void* moddata) {
	...
	if ((nn->isgr != (*i)->isgr) || (*i)->end <= xx.x1) {
	  ...
	  if (ismono || (*i)->pi == nn->pi) {
	    if (!isspecial(**i, *nn)) { // if it's not a special overlapping pair...
	      switch (typ) {
	      case prune_cutoff: { // cutoff earlier note--only one that could reduce the total duration
		CUTOFF:
		...
		break;
	      ...
	      case prune_xfer: {
		modutil_range yy = {xx.x1, module_makeval((*i)->end)};
		modutil_range yy2 = {module_makeval(nn->end), module_makeval((*i)->end)};
		(*i)->remove(yy);
		if (yy2.x2 > yy2.x1) nn->insert(yy2);
		break;
	      }
	      }
	    }
	  } else if ((*i)->pi < (fomus_int)0) goto STEAL; // second is a rest
	  else if (nn->pi < (fomus_int)0) {if (typ != prune_breakup) goto BREAKUP; else goto CUTOFF;} // first is a rest
	  ++i;
	}       
      }
      eoffs.push_back(nn);
  ...
  const char* prune_err_fun(void* moddata) {return 0;}
  ...
  int valid_prune_aux(const char* val) {return ptypes.find(val) != ptypes.end();}
  int valid_prune(const struct module_value val) {return module_valid_string(val, -1, -1, valid_prune_aux, prunetype);}
  ...
  int get_setting(int n, module_setting* set, int id) {
	...
	set->descdoc = "Notes of the same pitch that overlap in the same voice must be adjusted or \"pruned\" somehow.  This can be done in several different ways."
	  "  `cutoff' specifies that the durations of earlier notes be \"cut\" to avoid conflicting with later notes."
	  "  `transfer' preserves the total duration of two conflicting notes by cutting the earlier note and extending the duration of the later note if necessary."
	  "  `split' splits notes into two parts if another conflicting note falls somewhere in the middle (i.e., an extra note might be created)."
	  "  `steal' causes earlier notes to steal durations from any conflicting notes (i.e., the later notes are \"cut\")."
	  ;
	...
	set->descdoc = "Specifies whether or not notes are clipped when necessary to produce a monophonic line.  "
	  "Set this to `yes' if you don't want any notes to overlap in the same voice."
	  ;
