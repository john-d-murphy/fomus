...
namespace octs {
  ...
  // setting ids
  int heapsizeid,
    properoctid, octchangeid, //diaintid, dianoteid, simqtid,
    exponid,
    octdistid, beatdistid, rangeid, distmodid, enginemodid,
    octupid, octdownid, upllsid, downllsid, isanoctid;
  
  struct octsnode _NONCOPYABLE {
    module_noteobj note;
    fomus_float properoctpenalty, octchangepenalty, isanoctpenalty; // diaintpenalty, dianotepenalty, simqtpenalty;
    fomus_float expon;
    int oct;
    int midpitch;
    octsnode(const module_noteobj note, /*const deque<octsnode*>::type::const_iterator& it,*/ const int oct):note(note), /*it(it),*/ oct(oct),
  ...
  struct scorenode {
    ...
    fomus_float dist;
    scorenode(const octsnode* node, const fomus_float dist):node(node), dist(dist) {}
  };
  inline fomus_float properoct(const octsnode& n2) {
    ...
    if (n > u) return ((n - u) / 2) * n2.properoctpenalty;
    //} else {
    int dw = n2.midpitch - (module_setting_ival(n2.note, downllsid) * 2 + 5);
    if (n < dw) return ((dw - n) / 2) * n2.properoctpenalty;
    //}
    return 0;
  }
  inline fomus_float isanoct(const octsnode& n2) {return n2.oct ? n2.isanoctpenalty : 0;}
  inline fomus_float octchange(const std::vector<scorenode>::const_iterator& n1, const octsnode& n11, const octsnode& n2) { // n2 used for calculating distance
  ...
  struct octsdata _NONCOPYABLE {
    ...
    ~octsdata() {if (diface.moddata) diface.free_moddata(diface.moddata);}
    bool isoutofrange(const search_node n1, const search_node n2) const {return diface.is_outofrange(diface.moddata, ((octsnode*)n1)->note, ((octsnode*)n2)->note);}
    // no rests, only notes
	...
	if (d <= diface.data.rangemax) arr.push_back(scorenode(*i, pow(n2.expon, d)));
      ...
      assert(isanoct(n2) >= 0);
      sc.f = properoct(n2) + isanoct(n2); //vertmax(arr, n2) * arr.size(); // multiply by arr size so matches with accumulated calculations in loop
	...
	: ((((octsnode*)prevnode)->note != getn)
      ...
      DBG("octup=" << module_setting_ival(n, octupid) << " octdown=" << module_setting_ival(n, octdownid) << std::endl);
      if (choice - 2 > module_setting_ival(n, octupid) || choice - 2 < -module_setting_ival(n, octdownid)) return 0;
      int uo = module_octsign(n);
      DBG("uo = " << uo << std::endl);
      if (uo != 0 && uo != choice - 2) return 0;
      DBG("NEWNODE t=" << module_time(n) << " oct=" << choice - 2 << std::endl);
      return new octsnode(n, /*boost::prior(vect.end()),*/ choice - 2);
    }
    void assignnext(const int choice) {
      DBG("ASSIGNING OCT " << module_time(module_peeknextnote(ass)) << " --> " << choice - 2 << std::endl);
  ...
  extern "C" {
    ...
    //void search_free_moddata(void* moddata); // free up moddata structure and choices vector when finished
  ...
  void search_assign(void* moddata, int choice) {((octsdata*)moddata)->assignnext(choice);} // makes a solution assignment & reports it so that other phases of the program can continue
  union search_score search_get_score(void* moddata, struct search_nodes nodes) {return ((octsdata*)moddata)->getscore(nodes);}
  search_node search_new_node(void* moddata, search_node prevnode, int choice) {return ((octsdata*)moddata)->newnode(prevnode, choice);} // return NULL if no valid next node or END if at end of search--`prevnode' might be BEGIN (get the first node)
  void search_free_node(void* moddata, search_node node) {delete (octsnode*)node;}
  //void search_free_moddata(void* moddata) {delete (octsdata*)moddata;} // free up moddata structure and choices vector when finished
  ...
  int search_is_outofrange(void* moddata, search_node node1, search_node node2) {return ((octsdata*)moddata)->isoutofrange(node1, node2);} // node2 has the proper diface
  ...
  int valid_octsdistmult(const struct module_value val) {
    return module_valid_num(val, module_makeval((fomus_int)0), module_excl, module_makeval((fomus_int)0), module_nobound, 0, octsdistmulttype);
  }
  ...
  int valid_octsupdown(const struct module_value val) {return module_valid_int(val, 0, module_incl, 2, module_incl, 0, octsupdowntype);}
...
const char* module_longname() {return "Octave Signs";}
const char* module_author() {return "(fomus)";}
const char* module_doc() {return "Places octave change signs in the score at appropriate points.";}
void* module_newdata(FOMUS f) {return new octsdata;}
void module_freedata(void* dat) {delete (octsdata*)dat;}
const char* module_err(void* dat) {return 0;}
int module_priority() {return 0;}
enum module_type module_type() {return module_modocts;}
int module_itertype() {return module_bypart | module_bystaff | module_norests /*| module_firsttied*/;}
...
int module_get_setting(int n, module_setting* set, int id) {
  ...
  case 2:
	...
	"  Set this to the name of an engine module to change the search algorithm used for finding octave changes.";
  ...
  case 3:
    {
      ...
      set->type = module_number;
      set->descdoc = "Score when a note is inside the staff (or within a certain number of ledger lines)."
	"  Decreasing this value increases the likelyhood of inserting an octave sign.";
  ...
  case 4:
	...
	"Increasing this value decreases the chance of switching often between octave change signs.";
      set->typedoc = scoretype; 
  ...
  case 5:
	...
	"  A larger distance translates to a smaller weight in decisions regarding octave change signs and vice versa.";
      set->typedoc = scoretype; 
  ...
  case 8:
	...
	"  Set this to change the algorithm used for calculating distance when making decisions regarding the use of octave signs."
	;
  ...
  case 9:
    {
      ...
      set->type = module_number;
      set->descdoc = "Value indicating which octave signs are allowed above a staff.  "
	"Set this in a staff definition to indicate which octave changes are allowed for an instrument.  "
	"0 means no octave changes allowed, 1 means a one-octave (8va) change sign is allowed and 2 means a two-octave (15va) change sign is allowed.";
  ...
  case 10:
    {
      ...
      set->type = module_number;
      set->descdoc = "Value indicating which octave signs are allowed below a staff.  "
	"Set this in a staff definition to indicate which octave changes are allowed for an instrument.  "
	"0 means no octave changes allowed, 1 means a one-octave (8vb) change sign is allowed and 2 means a two-octave (15vb) change sign is allowed.";
      set->typedoc = octsupdowntype; // uses the same string/valid fun
  ...
  case 11:
	...
	"Increasing this descreases the change of octave change signs occurring.";
      set->typedoc = scoretype; 
...
void module_ready() {
  ...
  if (upllsid < 0) {ierr = "missing required setting `ledgers-up'"; return;}
  downllsid = module_settingid("ledgers-down");
  if (downllsid < 0) {ierr = "missing required setting `ledgers-down'"; return;}
}
