...
namespace phrdyns {
  ...
  int maxrestdurid, beginid, endid, beginmarksid, endmarksid, dynrangeid,
      dynsymrangeid, dodynsid;
  ...
  void dodyn(const module_noteobj n, const module_markslist& ml) {
    ...
    if (!module_setting_ival(n, dodynsid))
      return;
    ...
    if (!skp)
      marks_assign_add(n, dym, 0, module_makenullval());
  }

  bool hasamark(const module_noteobj n, const module_markslist& ml,
                const int setid) {
    // module_markslist ml(module_singlemarks(n));
    ...
    for (const module_value *i(li.val.l.vals), *ie(li.val.l.vals + li.val.l.n);
      ...
      for (const module_markobj *m(ml.marks), *me(ml.marks + ml.n); m < me;
           ++m) {
        int i = module_markid(*m);
        if (boost::algorithm::ilexicographical_compare(mn, module_marktostr(i)))
          return true;
        int d = module_markbaseid(i);
        if (d != i &&
            boost::algorithm::ilexicographical_compare(mn, module_marktostr(d)))
          return true;
      }
    }
    return false;
  ...
  void run_fun(FOMUS f, void* moddata) {
    ...
    while (true) {
      ...
      module_noteobj n0 = n;
      if (lastoff < (fomus_int) 0 ||
          ti - lastoff >
              module_setting_rval(
                  n, maxrestdurid)) { // module_setting_rval(n, repeatid)
        if (ln) {
          module_markslist lml(module_singlemarks(ln));
          if (module_setting_ival(ln, endid) || hasamark(ln, lml, endmarksid))
            dodyn(ln, lml);
        }
        module_markslist ml(module_singlemarks(n));
        if (module_setting_ival(n, beginid) || hasamark(n, ml, beginmarksid)) {
          dodyn(n, ml);
          n = 0;
        }
      }
  ...
  int valid_markslisttype(const struct module_value val) {
    return module_valid_listofstrings(val, -1, -1, -1, -1,
                                      valid_markslisttypeaux, markslisttype);
  }
} // namespace phrdyns
...
const char* module_longname() {
  return "Phrase Dynamics";
}
const char* module_author() {
  return "(fomus)";
}
const char* module_doc() {
  return "Converts dynamic values to score symbols at the beginnings and ends "
         "of phrases.";
}
...
int module_get_setting(int n, struct module_setting* set, int id) {
  ...
  case 1: {
    ...
    set->descdoc =
        "Whether or not a dynamic symbol should appear at the beginning of a "
        "\"dynamics phrase.\"  "
        "A dynamics phrase is calculated using `dynphrase-maxrestdur', "
        "`dynphrase-beginmarks' and `dynphrase-endmarks' "
        "for the purpose of placing dynamic marks at logical locations in the "
        "score.";
  ...
  case 2: {
    ...
    set->type = module_bool;
    set->descdoc =
        "Whether or not a dynamic symbol should appear at the end of a "
        "\"dynamics phrase.\"  "
        "A dynamics phrase is calculated using `dynphrase-maxrestdur', "
        "`dynphrase-beginmarks' and `dynphrase-endmarks' "
        "for the purpose of placing dynamic marks at logical locations in the "
  ...
  case 3: {
    ...
    set->descdoc =
        "Marks that denote the beginning of a phrase calculated for the "
        "purpose of attaching dynamic symbols to the beginnings and ends of "
        "them.  "
        "Set this to influence how dynamics phrases are calculated.  "
        "Any mark whose id appears in this list is considered to be the "
  ...
  case 4: {
    ...
    set->type = module_list_strings;
    set->descdoc =
        "Marks that denote the end of a phrase calculated for the purpose of "
        "attaching dynamic symbols to the beginnings and endings of them.  "
        "Set this to influence how dynamics phrases are calculated.  "
        "Any mark whose id appears in this list is considered to be the end of "
        "a phrase.";
    set->typedoc = markslisttype;
...
void module_ready() {
  ...
  if (dynrangeid < 0) {
    ierr = "missing required setting `dyn-range'";
    return;
  }
  dynsymrangeid = module_settingid("dynsym-range");
  if (dynsymrangeid < 0) {
    ierr = "missing required setting `dynsym-range'";
    return;
  }

  dodynsid = module_settingid("dyns");
  if (dodynsid < 0) {
    ierr = "missing required setting `dyns'";
    return;
  }
}
