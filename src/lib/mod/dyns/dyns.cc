...
namespace dyns {
  ...
  struct ass {
    ...
    bool nowedge;
    ass(const module_noteobj n, const fomus_float d, const bool dodyn):n(n), d(d), dodyn(dodyn), point(-1), nowedge(false) {}
    void assign() const {
      if (!dodyn || mks.empty()) {
	module_skipassign(n);
      } else {
	for (std::vector<module_markids>::const_iterator i(mks.begin()); i != mks.end(); ++i) marks_assign_add(n, *i, 0, module_makenullval());
	marks_assign_done(n);
      }
    }
  };
  ...
  void run_fun(FOMUS f, void* moddata) {
	...
	for (const module_markobj* i(mks.marks), *ie(mks.marks + mks.n); i < ie; ++i) {
	  ...
	  if (dy != dynids.end()) {
	    ...
	    dynstate = lvl; // 76
	    goto SKIP;
	  } else if (moredynids.find(id) != moredynids.end()) { // ids that don't signify permanent change
	    deq.back().nowedge = true;
	    fidr = false; // dont' reset "first" yet! dynstate cuodl be invalid
	    goto SKIP;
	  } else if (evenmoreids.find(id) != evenmoreids.end()) { // something that would cancel a wedge
	    deq.back().nowedge = true;
	    dynstate = lvl;
	    goto SKIP;
	  }
	...
	if (first) {
	  ...
	  if (j0 > i - sti && j0 < (i + 1) + sti) goto SKIP; // don't change the dyn bin#
    ...
    SKIP:
      ...
      int dir = 0;
      for (boost::ptr_deque<ass>::iterator i(deq.begin()); i != deq.end(); ++i) {
	if (i->point >= 0) {
	  if (pts.empty()) {pts.push_back(i); continue;}
	  if ((dir > 0 && i->point > pts.back()->point) || (dir < 0 && i->point < pts.back()->point)
	      || (dir == 0 && i->point == pts.back()->point)) { // continuing in same direction, point is an int
	    pts.back() = i;
	  } else {
	    dir = (i->point > pts.back()->point ? 1 : (i->point < pts.back()->point ? -1 : 0));
	    pts.push_back(i);
	  }
	}
      }
      while (pts.size() >= (n ? 3 : 2)) { // need three points (except at end) -- use only first two
	...
	if (w > 0) { // 1/9/12 was w >= 0
	  ...
	  fomus_float v1 = scale(be->d, mi, ma, mis, mas), v2 = scale(l->d, mi, ma, mis, mas);
	  ...
	  for (boost::ptr_deque<ass>::iterator i(be); i <= l; ++i) {
	    ...
	    if (i->nowedge || t - et > module_setting_rval(i->n, maxrestid)) goto NOWEDGE;
	    et = module_endtime(i->n);
	    if (t > t1 && t < t2) {
	      ++sz;
	      dif += fabs(scale(i->d, mi, ma, mis, mas) - scale(t, t1, t2, v1, v2));
	    }
	  }
	  if (sz > 0 && dif / sz <= w) {
	    assert(be < l);
	    for (boost::ptr_deque<ass>::iterator i(boost::next(be)); i != l; ++i) i->mks.clear();
	    if (be->d < l->d) {
    ...
    while (!deq.empty()) {
      deq.front().assign(); deq.pop_front();
    }   
  }
  
  const char* err_fun(void* moddata) {return 0;}

  const char* dynsymrangetype = "(string_dynsym string_dynsym), string_dynsym = pppppp|ppppp|pppp|ppp|pp|p|mp|mf|f|ff|fff|ffff";
  int valid_dynsymrange_aux(int n, const char* str) {return symvals.find(str) != symvals.end();}
  int valid_dynsymrange(const struct module_value val) {return module_valid_listofstrings(val, 2, 2, -1, -1, valid_dynsymrange_aux, dynsymrangetype);}

  const char* dynsticktype = "number0..1/2";
  int valid_dynstick(const struct module_value val) {
    return module_valid_num(val, module_makeval((fomus_int)0), module_incl, module_makeval(module_makerat(1, 2)), module_incl, 0, dynsticktype);
  }

  const char* restmaxtype = "rational>0";
  int valid_restmax(const struct module_value val) {
    return module_valid_rat(val, module_makerat(0, 1), module_excl, module_makerat(0, 1), module_nobound, 0, restmaxtype);
  }

  const char* dynrangetype = "(number number)";
  int valid_dynrange(const struct module_value val) {
    return module_valid_listofnums(val, 2, 2, module_makeval((fomus_int)0), module_nobound, module_makeval((fomus_int)0), module_nobound, 0, dynrangetype);
  }
...
const char* module_longname() {return "Dynamics";}
const char* module_author() {return "(fomus)";}
const char* module_doc() {return "Converts dynamic values to appropriate score symbols.";}
void* module_newdata(FOMUS f) {return 0;} //new accsdata;}
...
int module_get_setting(int n, struct module_setting* set, int id) {
  ...
  case 0:
	...
	"Set this to a list of two numbers representing the minimum and maximum dynamic values expected in note events."
	"  Dynamic values are scaled/translated to dynamic text markings by means of the setting `dynsym-range'.";
  ...
  case 1:
    {
      ...
      set->descdoc = "The range of dynamic symbols that a dynamic input value translates to.  "
	"Set this to a list of two dynamic text symbols representing the minimum and maximum dynamic levels allowed in the score."
  ...
  case 2:
    {
      ...
      set->descdoc = "Affects how dynamic input values are translated into dynamic text symbols.  "
	"If `dyn-sticky' is set to 0, dynamic values in the range `mindyn' to `maxdyn' are scaled and translated directly to "
	"dynamic marking symbols (the input values are effectively quantized to the given range of symbols).  "
	"A potential problem is that if values often change close to the borders between neighboring dynamic symbols, then the score could "
	"end up containing dynamic symbols that fluctuate constantly.  "
	"Increasing `dyn-stick' to a value like 1/3 or 1/2 effectively increases the amount the dynamic level must change before "
	"FOMUS switches to another dynamic symbol.  "
	"For example, 1/3 means the dynamic level must move at least one-third of the way into a neighboring dynamic symbol's range before that "
	"symbol is displayed.";
  ...
  case 3:
    {
      ...
      set->type = module_number;
      set->descdoc = "The likelihood of a crescendo/diminuendo wedge being created between two dynamic markings when translating dynamic input values into text symbols.  "
	"Increase this value to increase the likelihood of inserting a crescendo/diminuendo wedge between dynamic markings."
	"  For a span of notes to qualify for a wedge, the dynamic levels between them must gradually increase or decrease."
	"  This setting designates the amount of error allowed when comparing the levels to a straight line between two beginning and ending values."
  ...
  case 4:
    {
      ...
      set->descdoc = "Whether or not to translate dynamic input values into dynamic text markings.  "
	"Set this to `yes' if you want dynamic values that are entered along with other note event parameters to be converted to dynamic text symbols.";
  ...
  case 5:
    {
      ...
      set->type = module_number;
      set->descdoc = "This is the maximum total rest duration that a wedge is allowed to pass through when creating crescendo/diminuendo wedges from dynamic input values.  "
	"Higher values decrease the possibility of wedges being inserted in passages containing rests.  "
