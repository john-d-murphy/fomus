  ...
  void run_fun(FOMUS f, void* moddata) {
    ...
    GOTIT1: // dy is updated--no symbol here
      if (ti - lastoff >
        ...
        //   if (rep && thedyns.find(i) != thedyns.end()) marks_assign_remove(n,
        //   i, 0, fomus_make_nullval());
        // }
        // if (!skp) {
        if (rep) {
        ...
        if (stopinsdy >= (fomus_int) 0 && ti > stopinsdy)
          insdy = -1; // outside of chord now
        else {
          for (const module_markobj *m(ml.marks), *me(ml.marks + ml.n); m < me;
               ++m) {
            int i = module_markid(*m);
...
const char* module_longname() {
  return "Repeated Dynamics";
}
const char* module_author() {
  return "(fomus)";
}
const char* module_doc() {
  return "Repeats dynamics after a certain rest interval for improved "
         "readability.";
}
...
int module_get_setting(int n, struct module_setting* set, int id) {
  ...
  case 0: {
    ...
    set->type = module_rat;
    set->descdoc = "The maximum rest duration allowed before a dynamic symbol "
                   "is redundantly repeated.  "
                   "Use this to adjust how FOMUS repeats dynamic symbols for "
  ...
  case 1: {
    ...
    set->descdoc = "Whether or not to repeat dynamic symbols after a certain "
                   "rest interval.  "
