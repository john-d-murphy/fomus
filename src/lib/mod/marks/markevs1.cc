...
namespace markevs1 {
  ...
  void markevs1run_fun(FOMUS fom, void* moddata) {
    while (true) { // this has to be the last module, since it adds/deletes notes!
	...
	if (mdet >= 2 || (mdet && det)) {
	  ...
	  if (vs.n > 0) {
	    ...
	    case markpos_above: vo += 2000; break;
	    case markpos_below: vo += 3000; break;
	    default: vo += 1000;
	    }
	    markevs_assign_add(prt, vo, // pedals and similar marks w/ markpos_below go on lowest staff w/ voice = 3000+
			       (module_markisspannerend(module_markbaseid(id)) ? eti : ti), id, module_markstring(*a), module_marknum(*a));
	  }
	}
      }
    }
  ...
  const char* markevs1err_fun(void* moddata) {return 0;}
  ...
  int get_setting(int n, module_setting* set, int id) {
    ...
    // 	set->descdoc = //"Most marks (e.g., articulation) must be attached to note events.  "
    // 	  "Some marks such as crescendo wedges often need to appear detached from notes events.  "
