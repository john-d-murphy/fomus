...
namespace vsmarks {
  ...
  struct mark {
    ...
    const char* str;
    module_value val;
    // bool cmp; // this mark is for comparison purposes only, all "cmp" marks
    // don't compare the val/str
    mark(const int ty) : ty(ty), grp(module_markspangroup(ty)), str(0) {
      val.type = module_none;
    }
    mark(const int ty, const char* str, const module_value& val)
        : ty(ty), grp(module_markspangroup(ty)), str(str),
          val(val) /*, cmp(false)*/ {}
    mark(const int ty, const int grp, const char* str, const module_value& val)
        : ty(ty), grp(grp), str(str), val(val) /*, cmp(false)*/ {}
    mark(const int ty, const int grp) : ty(ty), grp(grp), str(0) /*cmp(true)*/ {
      val.type = module_none;
    }
    void assass(const module_noteobj n) const {
      DBG("assigning mark " << ty << ' ' << (str ? str : "NIL") << ' ' << val
                            << " at " << module_time(n) << std::endl);
      marks_assign_add(n, ty, str, val);
    }
    void assrem(const module_noteobj n) const {
      DBG("removing mark " << ty << ' ' << (str ? str : "NIL") << ' ' << val
                           << " at " << module_time(n) << std::endl);
    ...
    mark getminus1() const {
      return mark(module_markbaseid(ty) - 1, grp, str, val);
    }
#ifndef NDEBUGOUT
    void dump() const {
  ...
  inline bool operator<(const mark& x, const mark& y) {
    ...
    if (x.grp != y.grp || (x.grp == 0 && y.grp == 0))
      return x.ty < y.ty; // if in same group, maybe treat as =
    // if (x.cmp || y.cmp) return false;
    // if (x.val.type != module_none && y.val.type != module_none) return x.val
    // < y.val; if ((x.val.type == module_none) != (y.val.type == module_none))
  ...
  struct markless : std::binary_function<const mark&, const mark&,
                                         bool> { // markless only pays attention
                                                 // to val, not str
...
private:
    ...
    std::multiset<mark, markless2>
        ins; // stored markobjs are the SOURCE of the begin/end mark (which
             // isn't created yet)
public:
    ftimewends off, endoff;
    bool isrest;
    node(const module_noteobj n)
        : n(n), off(n), endoff(n, 0),
          isrest(module_isrest(n) && !module_ismarkrest(n)) {
      DBG("module_ismarkrest(n) = " << module_ismarkrest(n) << std::endl);
    }
    void assign() {
      DBG("orig.size() = " << orig.size() << " ins.size() = " << ins.size()
                           << std::endl);
      inplace_set_difference<std::multiset<mark>::iterator,
                             std::multiset<mark>::iterator, markless2>(orig,
                                                                       ins);
...
#ifndef NDEBUGOUT
    void dump() const {
      DBG("(node t=" << module_time(n) << " ol=" << orig.size()
                     << " il=" << ins.size() << ")" << std::endl);
    }
#endif
  };
  inline bool operator<(const node& x, const ftimewends& y) {
    return x.off < y;
  }
  inline bool operator<(const ftimewends& x, const node& y) {
    return x < y.off;
  }

  struct nodebwds : std::binary_function<const node*, const node*, bool> {
    inline bool operator()(const node* x, const node* y) {
      return x->endoff < y->endoff;
    }
  };
  inline bool operator<(const node* x, const ftimewends& y) {
    return x->endoff < y;
  ...
  node* beginnode(const ftimewends& off, boost::ptr_vector<node>& vect,
                  const bool cantouch, const bool coer) {
    boost::ptr_vector<node>::iterator v(
        cantouch ? std::lower_bound(vect.begin(), vect.end(), off)
    ...
    for (ftimewends o(bv->off); v != vect.end() && v->off <= o; ++v) {
      if ((coer || !v->isrest) && v->endoff < bv->endoff)
        bv = v;
    }
    return &*bv;
  }
  node* endnode(const ftimewends& off, std::vector<node*>& vect,
                const bool cantouch, const bool coer) {
    std::vector<node*>::iterator v(
        cantouch // get one past
            ? std::upper_bound(vect.begin(), vect.end(), off)
  ...
  std::ostream& dumpmarks(std::ostream& ou, const module_markslist& sp) {
    DBG("MARKS=");
    for (const module_markobj *j = sp.marks, *je = sp.marks + sp.n; j != je;
         ++j) {
      DBG(module_markid(*j) << " ");
    }
    return ou;
  ...
  void doends(node* ipr0, nodemap& spans, nodemap& es,
              const boost::ptr_vector<node>::iterator& i,
              const module_markslist& se, const bool pass10,
              boost::ptr_vector<node>& vect, const bool isvoice,
              const std::set<mark, markless>& begids,
              std::set<module_markobj>& endids) {
    for (const module_markobj *j = se.marks, *je = se.marks + se.n; j != je;
         ++j) {
      int mkid = module_markid(*j);
      if (!(isvoice ? module_markisvoice(mkid) : module_markisstaff(mkid)))
        continue;
      bool coer = module_markcanendonrests(mkid, i->n);
      node* ipr = (coer ? &*i : ipr0); // current valid end
      mark mk(
          mkid, module_markstring(*j),
          module_marknum(*j)); // it's an end--subtract 1 from id to get start
...
#endif
        nodemap::iterator f(spans.find(
            mk)); // finds it if it's in the same group (spans = all the begins)
        // if (f == spans.end()) f = spans.find(mark(mk.ty, mk.grp)); // search
        // in map of begins
        if (f != spans.end()) { // another begin of same type, have a match
          if (module_markcanspanone(mk.ty, i->n)
                  ? (f->second->off <= ipr->off &&
                     f->second->endoff <= ipr->endoff)
                  : (f->second->off < ipr->off &&
                     f->second->endoff < ipr->endoff)) { // if it's valid
            f->second->insert(f->first); // insert the begin into the first node
            ipr->insert(mk.getplus1(
                f->first)); // insert proper end into the second node, also set
                            // mk to inserted id, plus1 returns end mark w/o any
                            // argument values
          } else if (f->second->off == ipr->off &&
                     f->second->endoff == ipr->endoff &&
                     module_markcanreduce(
                         mk.ty)) { // if NOT later offset or able to begin/end
                                   // on same note (NOT valid)
            f->second->insert(
                mk.getminus1()); // convert to single mark (beginid - 1)
                                 // beginnind mark has the arguments
          }
          spans.erase(
              f); // either case: `begin' is gone or used up, so get rid of it
        } else {  // have an `end' without a `begin'
          // if pass = 1 && markcanspanone, look for begin in same event (?)--if
          // there is, set flag so mark is treated as pass1 during pass 2
          if (pass1 && module_markcanspanone(mk.ty, i->n) &&
              begids.find(mk) != begids.end()) {
            endids.insert(*j);
            continue;
          }
          ...
          node* b = beginnode(
              bi == es.end() ? module_makerat(-1, 1) : bi->second->off, vect,
              pass1,
              coer); // first nonrest node w/ off just at or after bi->off
          if (b && ipr) {
            if (module_markcanspanone(mk.ty, i->n)
                    ? (b->off <= ipr->off && b->endoff <= ipr->endoff)
                    : (b->off < ipr->off &&
                       b->endoff < ipr->endoff)) { // got room for a match
              assert(!b->isrest);
              b->insert(mk); // b is guaranteed to be a note
              ipr->insert(mk.getplus1());
            } else if (b->off == ipr->off && b->endoff == ipr->endoff &&
                       module_markcanreduce(mk.ty)) { // if same offset...
              b->insert(mk.getminus1()); // convert to single mark (beginid - 1)
                                         // beginnind mark has the arguments
            }
          }
        }
        es.erase(mk);
        es.insert(nodemap::value_type(
            mk, ipr)); // end in es is replaced no matter what
      }
    }
  }

  // a begin mark
  void dobegs(const mark& mk, nodemap::iterator& f, nodemap& es,
              const ftimewends& ioff, std::vector<node*>& vect, const bool coer,
              const module_noteobj nn) {
    node* e = endnode(ioff, vect, module_markcantouch(mk.ty, nn),
                      coer); // last node w/ offset just before or at ioff
    if (e) {
      DBG("f->second->off = " << f->second->off.time << std::endl);
      if (module_markcanspanone(mk.ty, nn)
              ? (f->second->off <= e->off && f->second->endoff <= e->endoff)
              : (f->second->off < e->off &&
                 f->second->endoff < e->endoff)) { // room for an end
      ...
      es.erase(mk);
      es.insert(
          nodemap::value_type(mk, e)); // inserted an end, so put that in here
    }
  }
  ...
  inline void vsmarks_run_fun(FOMUS f, const bool isvoice) {
    ...
    for (boost::ptr_vector<node>::iterator i(vect.begin()); i != vect.end();
         ++i)
      vectb.push_back(&*i);
    ...
    for (boost::ptr_vector<node>::iterator i(vect.begin()); i != vect.end();
      ...
      if (!i->isrest) {
        ...
        if (lipr && lipr->endoff > ien)
          ien = lipr->endoff; // ien = max endoff of notes up to ipr
      ...
      // std::transform(sp.marks, sp.marks + sp.n, std::inserter(sp00),
      // boost::lambda::bind(module_markid, boost::lambda::_1));
      for (const module_markobj *j = sp.marks, *je = sp.marks + sp.n; j != je;
           ++j)
        sp00.insert(mark(module_markid(*j)));
      ...
      while (inx0 != vect.end() && inx0->isrest)
        ++inx0; // inx0 = current or next one that isn't a rest
      std::set<mark, markless> spans0;
      if (i->isrest || (lipr && i->off > ien)) { // rest or skip
        for (nodemap::iterator f(spans.begin()); f != spans.end();) {
          if (module_markcanspanrests(f->first.ty, i->n)) {
            ++f;
            continue;
          } // if can't span a rest, must end it and start a new span
      ...
      if (inx0 != vect.end()) {
        for (std::set<mark, markless>::const_iterator it(spans0.begin());
             it != spans0.end(); ++it) {
    ...
    for (nodemap::iterator f(spans.begin()); f != spans.end(); ++f) {
      dobegs(f->first, f, es,
             module_makerat(std::numeric_limits<fomus_int>::max(), 1), vectb,
             false /*, true*/, vect.back().n);
    }
    for (boost::ptr_vector<node>::iterator i(vect.begin()); i != vect.end();
         ++i)
      i->assign();
  ...
  const char* vsmarks_err_fun(void* moddata) {
    return 0;
  }
...
namespace vmarks {
  ...
  void vmarks_run_fun(FOMUS f, void* moddata) {
    vsmarks::vsmarks_run_fun(f, true);
  }
...
namespace smarks {
  ...
  void smarks_run_fun(FOMUS f, void* moddata) {
    vsmarks::vsmarks_run_fun(f, false);
  }
  void fill_iface(void* moddata, void* iface) {
    ((dumb_iface*) iface)->moddata = 0;
