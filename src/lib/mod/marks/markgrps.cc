...
namespace markgrs {
  ...
  int icompare(const std::string& x, const std::string& y) {
    ...
    while (true) {
      ...
      if (j == y.end())
        return 1;
      if (!boost::algorithm::is_iequal()(*i, *j))
        return (boost::algorithm::is_iless()(*i, *j) ? -1 : 1);
      ++i;
      ++j;
    }
  }
  ...
  struct markopts {
    ...
    int id;
    markopts() : id(0) {}
    // bool operator==(const markopts& x) {return abs == x.abs &&
    // std::equal(mks, x.mks);}
    bool operator<(const markopts& x) const;
    void activate() {
      st = abs;
      id = module_strtomark(st.c_str());
      if (id < 0)
        id = mark_text;
    }
    void resfo() {
    ...
    bool check(const std::string& a, const module_markobj m,
               boost::ptr_multiset<mark, markless>& orig) {
      ...
      if ((!abs.empty() && boost::algorithm::iequals(abs, a)) ||
          mks.find(a) != mks.end()) { // user change! leave it and change state
        st = a;
        fo = true;
        return true;
      }
    ...
    // bool remove(const std::string& a, const module_markobj m,
    // boost::ptr_multiset<mark, markless>& orig) {
    //   DBG("MARKGRPS +++" << std::endl);
    //   if ((!abs.empty() && boost::algorithm::iequals(abs, a)) || mks.find(a)
    //   != mks.end()) {
    // 	orig.insert(new mark(m));
    // 	DBG("MARKGRPS ---" << std::endl);
    // 	return true;
  ...
  bool markopts::operator<(const markopts& x) const {
    if (mks.size() < x.mks.size())
      return true; // smaller groups first (also faster sort)
    if (mks.size() > x.mks.size())
      return false;
    ...
    if (c > 0)
      return false;
    for (std::set<std::string>::iterator i1(mks.begin()), i2(x.mks.begin());
         i1 != mks.end(); ++i1) {
  ...
  void markgrs_run_fun(FOMUS f, void* moddata) {
    ...
    while (true) {
      ...
      for (module_value *i(li.val.l.vals), *ie(li.val.l.vals + li.val.l.n);
           i != ie; ++i)
        inc.insert(i->val.s);
      std::auto_ptr<boost::ptr_set<markopts>> opts0(
          new boost::ptr_set<markopts>);
      module_value v(module_setting_val(module_nextpart(), pairsid));
      assert(v.type == module_list);
      for (module_value *i(v.val.l.vals), *ie(v.val.l.vals + v.val.l.n);
           i != ie; ++i) {
        bool skip = true;
      ...
      std::for_each(opts->begin(), opts->end(),
                    boost::lambda::bind(&markopts::resfo, boost::lambda::_1));
      ...
      for (const module_markobj *i(l.marks), *ie(l.marks + l.n); i != ie; ++i) {
        ...
        if (s[0]) {
          ...
          // if (istl) {
          //   DBG("MARKGRPS ***" << std::endl);
          //   for_each_until(opts->begin(), opts->end(),
          //   boost::lambda::bind(&markopts::remove, boost::lambda::_1,
          //   boost::lambda::constant_ref(a), *i, boost::lambda::var(orig)));
          // } else {
          for_each_until(opts->begin(), opts->end(),
                         boost::lambda::bind(&markopts::check,
                                             boost::lambda::_1,
                                             boost::lambda::constant_ref(a), *i,
                                             boost::lambda::var(orig)));
          // }
        }
      SKIPIT:;
      }
      if (!module_istiedleft(n))
        std::for_each(
            opts->begin(), opts->end(),
            boost::lambda::bind(
                &markopts::found, boost::lambda::_1,
                boost::lambda::var(
                    ins))); // ones that weren't found are set to "absent"
      inplace_set_difference<boost::ptr_multiset<mark, markless>::iterator,
                             boost::ptr_multiset<mark, markless>::iterator,
                             markless>(orig, ins);
  ...
  const char* pairstype =
      "((string_mark string_mark ...) (string_mark string_mark ...) ...)";
  int valid_groups_aux(int n, const struct module_value val) {
    return module_valid_listofstrings(val, -1, -1, 1, -1, 0, 0);
  }
  int valid_groups(const struct module_value val) {
    return module_valid_listofvals(val, -1, -1, valid_groups_aux, pairstype);
  }

  const char* listtype = "(string_mark string_mark ...)";
  int valid_list(const struct module_value val) {
    return module_valid_listofstrings(val, -1, -1, 1, -1, 0, listtype);
  }
...
int module_get_setting(int n, module_setting* set, int id) {
  ...
  case 0: {
    ...
    set->descdoc =
        "Groups of marks that cancel each other when they appear in one voice. "
        " Examples are dynamic markings and \"on/off\" mark pairs such as "
        "`pizz' and `arco'.  The strings in each inner list should match "
        "either a mark id (a single mark, not a spanner) or the string of a "
        "text mark.  "
        "Strings prefixed with a `-' indicate the mark that should appear in "
        "the absence of the other marks (i.e., the mark that indicates a "
        "return to a \"normal\" state)."
        "  (pizz -arco) would make sense for a string instrument, for example. "
        " You would then use `pizz' marks on all pizzicato notes and "
  ...
  case 1: {
    ...
    set->descdoc = "A list of IDs that indicate which mark groups defined in "
                   "`mark-group-defs' are to take effect.  "
                   "An ID in this case is simply one of the marks or strings "
                   "that are in a group (e.g., \"mp\" specifies the group "
                   "containing all of the dynamic symbols including \"mp\").  "
                   "Use this setting to switch mark groups on and off at any "
                   "place in the score.";
...
const char* module_doc() {
  return "Processes marks that belong in groups (e.g., dynamic markings).";
}
