...
namespace special {
  ...
  void doadds(module_obj a, const module_obj b, const bool bisgr) {
    ...
    if (!bisgr || module_time(a) < module_time(b) || module_gracetime(a) < module_gracetime(b)) {
  ...
  void run_fun(FOMUS fom, void* moddata) {
    ...
    while (true) {
      ...
      if (module_isgrace(n) && module_gracetime(n) >= (fomus_int)0 && module_gracetime(n) <= (fomus_int)1000) { // ***GRACE NOTE
	if (!ingr) { // grace note and not in grace slur yet...
	  if (module_setting_ival(n, doitid)) ingr = n;
	} else if (!nx) { // grace note and "in grace slur" and at the end
	  doadds(ingr, n, true);
	  return;
	} //else if (module_isrest(nx)) { // grace note and "in grace slur" and grace rest coming up
	// 	  doadds(ingr, n);
	// 	  ingr = 0;
	// 	  continue;
	// 	}
...
const char* module_longname() {return "Grace Note Slurs";}
const char* module_author() {return "(fomus)";}
const char* module_doc() {return "Automatically attaches slurs to grace notes.";}
...
int module_get_setting(int n, struct module_setting* set, int id) {
  ...
  case 0:
	...
	"Set this to `yes' if you want all grace notes to have slurs over them.";
      //set->typedoc = qtstype; 
