...
namespace markevs2 {
  ...
  enum markincl { // leftmost is most inclusive, rightmost is least inclusive
    mark_musttouch,
    mark_mustolap,
    mark_mustbeincltouch //, mark_mustbeincl // default should be mustolap
  };
  ...
  struct offgroff {
    ...
    offgroff(const module_markobj m)
        : off(module_vtime(m)),
          groff(module_isgrace(m) ? module_vgracetime(m)
                                  : module_makenullval()) {}
    offgroff(const module_markobj m, int)
        : off(module_vendtime(m)),
          groff(module_isgrace(m) ? module_vgracetime(m)
                                  : module_makenullval()) {}
  };
  inline bool operator<(const offgroff& x, const offgroff& y) {
    if (x.off == y.off) {
      return x.groff.type == module_none
                 ? false
  ...
  struct markassnode {
    ...
    int valid;
#endif
    markassnode(const module_noteobj n)
        : n(n), ass(0)
#ifndef NDEBUG
          ,
          valid(12345)
    ...
    void insert(const module_markobj m) {
      assert(isvalid());
      ins.insert(mark(m));
    }
    void assign() {
      ...
      inplace_set_difference<std::multiset<mark>::iterator,
                             std::multiset<mark>::iterator, markless>(orig,
                                                                      ins);
  ...
  struct marknode {
    ...
    int valid;
#endif
    marknode(const module_markobj m)
        : m(m), // vn1(0), //, vn2(0),
          lincl(itypes.find(module_setting_sval(m, leftid))->second),
          rincl(itypes.find(module_setting_sval(m, rightid))->second), st(m),
          et(m, 0), got(false), detall(module_setting_ival(m, detachid))
#ifndef NDEBUG
          ,
          valid(12345)
#endif
    ...
    bool stick(const std::list<markassnode>::iterator& x, const offgroff& o1,
               const offgroff& o2, bool& done, const int v);
    void gotit();
  };
  bool marknode::stick(const std::list<markassnode>::iterator& x,
                       const offgroff& o1, const offgroff& o2, bool& done,
                       const int v) {
    assert(isvalid());
    if (o2 < st)
      return true;
    ...
    case mark_musttouch:
      if (o1 <= et)
        ...
        return false;
    case mark_mustolap:
      if (st >= et ? o1 <= et : o1 < et)
        break;
      else
        return false;
    case mark_mustbeincltouch:
      if (o2 <= et)
        break;
      else
        return false;
    ...
    if (got) {
      ...
      assert(vn2->isvalid());
      {
        ...
        for (const module_markobj *a(ma.marks), *ae(ma.marks + ma.n); a != ae;
          ...
          if (detall && module_markisdetachable(id))
            continue;
  ...
  struct marknodeless
      : public std::binary_function<const marknode&, const marknode&, bool> {
    bool operator()(const marknode& x, const marknode& y) const {
      return x.st < y.st;
    }
  };
  ...
  void markevs2run_fun(FOMUS fom, void* moddata) {
    ...
    while (true) {
      ...
      for (boost::ptr_set<marknode, marknodeless>::iterator i(mks.begin());
           i != mks.end();) {
        bool done = false;
        if (i->stick(x, o1, o2, done, v))
          break; // returns true if o2 is before beginning of mark obj
        if (done) {
          i->gotit();
          mks.erase(i++);
        } else
          ++i; // done is true if the right one was assigned
      }
    ...
    for (boost::ptr_set<marknode>::iterator i(mks.begin()); i != mks.end(); ++i)
      i->gotit();
    mks.clear();
    while (!ass.empty()) {
      ass.front().assign();
  ...
  int valid_markevsaux(const char* val) {
    return itypes.find(val) != itypes.end();
  }
  int valid_markev(const struct module_value val) {
    return module_valid_string(val, -1, -1, valid_markevsaux, markevtype);
  }
  ...
  int get_setting(int n, module_setting* set, int id) {
    ...
    case 0: {
      ...
      set->descdoc =
          "Determines along with `right' how the time of a mark event matches "
