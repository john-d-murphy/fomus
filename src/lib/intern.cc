...
#ifndef NDEBUG
#define INTREADLOCK(zzz)                                                       \
  lcheck zzz000;                                                               \
  shlock xxx(CASTEVENT(zzz)->interngetmut())
#define INTUPREADLOCK(zzz)                                                     \
  lcheck zzz000;                                                               \
  uplock xxx(CASTEVENT(zzz)->interngetmut())
#define INTTOWRITELOCK                                                         \
  boost::upgrade_to_unique_lock<boost::shared_mutex> yyy(xxx)
#define INTWRITELOCK(zzz)                                                      \
  lcheck zzz000;                                                               \
  boost::unique_lock<boost::shared_mutex> yyy(CASTEVENT(zzz)->interngetmut())
#else
...
#define INTWRITELOCK(zzz)                                                      \
  ...
  void postmeasdoit(FOMUS fom, void* moddata) { // BY PART
    while (
        true) { // eat the notes--this is last in substage section --it's okay
  ...
  void inbetweenmarksdoit(FOMUS fom, void* moddata) { // by part/voice
    ...
    if (ln) {
      while (true) {
        ...
        if (!n)
          break;
        if (CASTNOTEEVBASE(n)->getfulltime() >
            CASTNOTEEVBASE(ln)->getfulltime()) {
          ...
          for (const module_markobj *m(ml.marks), *me(ml.marks + ml.n); m < me;
            ...
            case mark_gliss_before: {
              {
                INTWRITELOCK(n);
                CASTNOTEEVBASE(n)->assignmarkrem(mark_gliss_before, 0,
                                                 numb(module_none));
              }
              {
                INTWRITELOCK(ln);
                CASTNOTEEVBASE(ln)->assignmarkins(mark_gliss_after, 0,
                                                  numb(module_none));
              }
              ass = true;
              break;
            ...
            case mark_port_before: {
              {
                INTWRITELOCK(n);
                CASTNOTEEVBASE(n)->assignmarkrem(mark_port_before, 0,
                                                 numb(module_none));
              }
              {
                INTWRITELOCK(ln);
                CASTNOTEEVBASE(ln)->assignmarkins(mark_port_after, 0,
                                                  numb(module_none));
              }
              ass = true;
              break;
            ...
            case mark_breath_before: {
              {
                INTWRITELOCK(n);
                CASTNOTEEVBASE(n)->assignmarkrem(mark_breath_before, 0,
                                                 numb(module_none));
              }
              {
                INTWRITELOCK(ln);
                CASTNOTEEVBASE(ln)->assignmarkins(mark_breath_after, 0,
                                                  numb(module_none));
              }
              ass = true;
              break;
            ...
            case mark_break_before: {
              {
                INTWRITELOCK(n);
                CASTNOTEEVBASE(n)->assignmarkrem(mark_break_before, 0,
                                                 numb(module_none));
              }
              {
                INTWRITELOCK(ln);
                CASTNOTEEVBASE(ln)->assignmarkins(mark_break_after, 0,
                                                  numb(module_none));
              }
              ass = true;
              break;
  ...
  void sysbreakdoit(FOMUS fom, void* moddata) { // by all
    ...
    while (true) {
      ...
      while (n && CASTNOTEEVBASE(n)->getmeasobj() == (modobjbase*) me) {
        module_markslist ml(CASTNOTEEVBASE(n)->getsinglemarks());
        for (const module_markobj *m(ml.marks), *me0(ml.marks + ml.n); m < me0;
             ++m) {
          if (CASTMARKOBJ(*m)->getmarkid() == mark_break_after) {
            CASTNOTEEVBASE(n)->assignmarkrem(mark_break_after, 0,
                                             numb(module_none));
            no.insert(n);
    ...
    while (true) {
      ...
      while (n && CASTNOTEEVBASE(n)->getmeasobj() == me) {
        if (fi) {
          CASTNOTEEVBASE(n)->assignmarkins(mark_break_after, 0,
                                           numb(module_none));
  ...
  void posttquantdoit(FOMUS fom, void* moddata) { // BY ALL
    ...
    while (true) {
      ...
      if (!p)
        break;
      CASTPARTORMPART(p)->trimmeasures(
          numtofrat(tr)); // get rid of extra measures
    }
  }
  ...
  void posttquantinvdoit(FOMUS fom, void* moddata) { // BY ALL
    ...
    //   p = stageobj->api_peeknextpart((modobjbase*)p);
    //   if (!p) break;
    //   CASTPARTORMPART(p)->trimmeasures(numtofrat(tr)); // get rid of extra
    //   measures
    // }
  }

  void postpquantdoit(FOMUS fom,
                      void* moddata) { // BY MEAS, NORESTS, NOPERCS, FIRSTONLY
    while (true) {
      module_noteobj n = stageobj->api_nextnote();
      if (!n)
        break;
  ...
  void poststavesdoit_aux(boost::ptr_map<const int, std::vector<int>>& srvs,
                          std::list<noteev*>& objs) {
    for (boost::ptr_map<const int, std::vector<int>>::iterator si(srvs.begin());
         si != srvs.end(); ++si) {
  ...
  void
  poststavesdoit(FOMUS fom,
                 void* moddata) { // BY MEAS, NORESTS, NOPERCS, FIRSTONLY--rests
    ...
    } // do not disable mutex checking!  next thread might access events in this
      // thread
    ...
    while (true) {
      n = stageobj->api_peeknextnote((modobjbase*) n);
      if (!n) {
        poststavesdoit_aux(srvs, objs);
        break;
      }
      if (CASTNOTEEVBASE(n)->checkstaves0())
      ...
      if (i == srvs.end())
        srvs.insert(st, new std::vector<int>(1, CASTNOTEEV(n)->getclefid()));
      else
        i->second->push_back(CASTNOTEEV(n)->getclefid());
    }
  }
  ...
  void fillnotes1doit(FOMUS fom, void* moddata) { // BY PART, BYVOICE
    ...
    while (true) {
      ...
      module_noteobj nn0 =
          stageobj->api_peeknextnote((modobjbase*) n); // next note
      module_noteobj nn = nn0;
      while (nn && CASTNOTEEVBASE(nn)->getfulltime() == thisoff) {
      ...
      offgroff noff(nn ? CASTNOTEEVBASE(nn)->getfulltime() : lmeo);
      {
        ...
        if (pt & (point_left | point_grleft)) {
          CASTNOTEEVBASE(n)->dopointl(
              off, tmp, LOCKARG); // an autodur note reduced to a "point"
        } else if (pt & (point_right | point_grright)) {
          CASTNOTEEVBASE(n)->dopointr(noff, tmp, LOCKARG);
        }
        offgroff eo(CASTNOTEEVBASE(n)
                        ->getendtime_nomut()); // should always have one now!
        if (eo > off0)
          off0 = eo;
      }
      if (!nn0 ||
          CASTNOTEEVBASE(nn0)->getfulltime() > CASTNOTEEVBASE(n)->getfulltime())
        off = off0;
      CASTNOTEEVBASE(n)->post_apisetvalue();
    }
  ...
  void fillnotes2doit(FOMUS fom, void* moddata) {
    std::list<noteevbase*> tmp;
    partormpart_str& p = *CASTPARTORMPART(stageobj->api_nextpart());
    offgroff lmeo(CASTMEASURE(stageobj->api_nextmeas())
                      ->getendtime() /*p.getlastmeasendoff()*/);
    while (true) {
      module_noteobj n = stageobj->api_nextnote();
      if (!n)
        break;
      {
        ...
        if (x & (point_auto | point_grauto)) {
          ...
          do {
            nn = stageobj->api_peeknextnote((modobjbase*) nn);
            DBG("maybe it's this one: "
                << (nn ? CASTNOTEEVBASE(nn)->getfulltime() : lmeo).off << ","
                << (nn ? CASTNOTEEVBASE(nn)->getfulltime() : lmeo).groff
                << std::endl);
          } while (nn && (x == point_grauto
                              ? CASTNOTEEVBASE(nn)->getfulltime() <= thisoff
                              : CASTNOTEEVBASE(nn)->gettime() <= thisoff.off));
          assert(!nn || (CASTNOTEEVBASE(nn)->getfulltime() > thisoff));
          DBG("doing dopointr at "
              << thisoff.off << "," << thisoff.groff << " up to "
              << (nn ? CASTNOTEEVBASE(nn)->getfulltime() : lmeo).off << ","
              << (nn ? CASTNOTEEVBASE(nn)->getfulltime() : lmeo).groff << " "
  ...
  void postprunedoit(
      FOMUS fom,
      void* moddata) { // BY MEAS, NORESTS, NOPERCS, FIRSTONLY--rests are dealt
                       // with later (when measure divs are created)
    while (true) {
      module_noteobj n = stageobj->api_nextnote();
      if (!n)
        break;
  ...
  inline void dobarlines(measure& m1, const measure& m2) {
    ...
    module_barlines l = barlines.find(m1.get_sval(BARLINER_ID))->second,
                    r = barlines.find(m2.get_sval(BARLINEL_ID))->second;
    if (l < r)
      std::swap(l, r); // make sure l is bigger
    DBG("left is `" << m1.get_sval(BARLINEL_ID) << "' and right is `"
                    << m2.get_sval(BARLINER_ID) << "'" << std::endl);
    DBG("left is " << l << " and right is " << r << std::endl);
    if (l == barline_final && r == barline_initial)
      l = barline_initfinal;
    else if (l == barline_repeatright && r == barline_repeatleft)
      l = barline_repeatleftright;
    m1.setbarline(l);
  }
  ...
  void barlinesdoit(FOMUS fom, void* moddata) { // by PART, VOICE
    ...
    while (true) {
      ...
      if (!m) {
        assert(lm);
        if (CASTFOMUSDATA(fom)->get_ival(FINALBAR_ID))
          CASTMEASURE(lm)->setbarline(barline_final);
        if (lnw)
          dobarliner2(*CASTMEASURE(lm));
        break;
      }
      ...
      if (lnw && nw) {
        ...
        if (lnw) {
          assert(lm);
          dobarliner(*CASTMEASURE(lm));
        }
        if (nw) {
          assert(lm);
          assert(m);
          dobarlinel(*CASTMEASURE(lm), *CASTMEASURE(m));
        }
      }
      lm = m;
      lnw = nw;
  ...
  enum splittremsenum { spltrems_none, spltrems_sing, spltrems_doub };

  void splittremsaux(std::vector<noteevbase*>& nos, const numb& spl,
                     const splittremsenum xx, const int era, const bool inv) {
    DBG("START AUX" << std::endl);
    if (xx != spltrems_none) {
      bool t1 = false, t2 = false, t0 = false;
      for (std::vector<noteevbase*>::iterator i(nos.begin()); i != nos.end();
           ++i) {
        if (!CASTNOTEEVBASE(*i)->getisnote())
          continue;
        ...
        for (const module_markobj *m(ml.marks), *me(ml.marks + ml.n); m < me;
          ...
          case mark_trem:
            ...
            case spltrems_sing: {
              INTWRITELOCK(*i);
              CASTNOTEEV(*i)->trem_fix(
                  spl,
                  WRLOCKARG); // spl is incase tremolo doesn't divide properly
              t0 = true;
              goto STLOOP;
            }
            case spltrems_doub: {
              if (era == 2) {       // only first tremchord (maybe w/ split)
                if (spl.isnull()) { // no split
                  if (inv)
                    CASTNOTEEV(*i)->trem_dur0(module_none);
                } else if (inv)
                  CASTNOTEEV(*i)->trem_incoff0(spl);
                else
                  CASTNOTEEV(*i)->trem_halfdur0(
                      spl); // inv must also switch trem marks w/ trem2 marks
              } else if (inv) {
                CASTNOTEEV(*i)->trem_incoff(spl);
                if (!era)
                  CASTNOTEEV(*i)->switchtrems(mark_trem, mark_trem2);
              } else
                CASTNOTEEV(*i)->trem_halfdur(
                    spl); // off = 18, 1/8 beams don't fit
              t1 = true;
              goto STLOOP;
            }
            default:;
          ...
          case mark_trem2: {
            if (era == 2) {
              if (spl.isnull()) {
                ...
                CASTNOTEEV(*i)->trem_halfdur0(spl);
              else
                CASTNOTEEV(*i)->trem_incoff0(spl);
            } else if (inv) {
              CASTNOTEEV(*i)->trem_halfdur(spl);
              if (!era)
                CASTNOTEEV(*i)->switchtrems(mark_trem2, mark_trem);
            } else
              CASTNOTEEV(*i)->trem_incoff(spl);
            t2 = true;
            goto STLOOP;
  ...
  void splittremsdoit(FOMUS fom, void* moddata) { // by PART, VOICE
    ...
    while (true) { // this can update measure info independent of other stages
      module_noteobj n = stageobj->api_nextnote();
      if (!n)
        break; // this is a _destr stage, so nothing is waiting on this thread
      ...
      if (notyet) {
        ...
        for (const module_markobj *i(ml.marks), *ie(ml.marks + ml.n); i < ie;
          ...
          case mark_trem2: {
            ...
            if (!x.isint() || abs_int(x.val.i) < 8 || abs_int(x.val.i) > 32 ||
                !isexpof2(abs_int(x.val.i))) {
              CERR << "invalid tremolo number";
              integerr("tremolos");
            }
            fomus_rat dt1((fint) 1 /
                          ((x.geti() < 0 ? -x.geti() : x.geti()) *
                           mlt)); // duration of 1st + 2nd part of tremolo
            if (x < (fint) 0) {
              fomus_rat dt2(dt1 *
                            (fint) 2); // duration of 1st + 2nd part of tremolo
              xx = spltrems_doub;
              if (dt2 <= du) {
                numb x0(mod(
                    du, dt2)); // used to be dt1, but lilypond doesn't notate it
                               // unless both part of alt. trem divide evenly
                if (x0 > (fint) 0)
                  spl = du - x0; // else spl.null();
                if (dt2 >= (spl.isnull() ? du : spl))
                  era = 1;
                if (/*spl.isnull() &&*/ x.geti() > -32 && mod(du, dt2) == dt1)
                  minv = true;
              } else {
  ...
  bool tremtiesaux(const std::vector<module_noteobj>& x,
                   const std::vector<module_noteobj>& y,
                   bool ch1is /*, const offgroff& ti*/) {
    // if (y.empty()) return false;
    assert(y.empty() || tremties.find(CASTNOTEEV(y.front())->get_sval(
                            TREMTIE_ID)) != tremties.end());
    bool ch2is;
    if (!y.empty() &&
        tremties.find(CASTNOTEEV(y.front())->get_sval(TREMTIE_ID))->second ==
            tiestyle_none) {
  ...
  SKIP:
    ...
    if (ch1is || ch2is /*&& x.front()->getfullendtime() >= ti*/) {
      assert(ch1is ? !x.empty() : true);
      for (std::vector<module_noteobj>::const_iterator i(x.begin());
           i != x.end(); ++i)
        CASTNOTEEV(*i)->destroytiedr();
      // for (std::vector<module_noteobj>::const_iterator i(y.begin()); i !=
      // y.end(); ++i) CASTNOTEEV(*i)->destroytiedl();
    }
    return ch2is;
  }

  inline void tremtiesdone(std::vector<module_noteobj>& x) {
    for (std::vector<module_noteobj>::iterator i(x.begin()); i != x.end(); ++i)
      CASTNOTEEV(*i)->post_apisetvalue();
  }
  void tremtiesdoit(FOMUS fom, void* moddata) { // by PART, VOICE, NOTES ONLY
    std::auto_ptr<std::vector<module_noteobj>> ch1(
        new std::vector<module_noteobj>),
        ch2(new std::vector<module_noteobj>);
    bool ch1is = false;
    offgroff cti((fint) -1);
    while (true) {
      module_noteobj n = stageobj->api_nextnote();
  ...
  void fixlyrsdoit(FOMUS fom, void* moddata) { // by PART, VOICE
    ...
    while (true) {
      module_noteobj n = stageobj->api_nextnote();
      if (!n) {
        for (std::vector<noteevbase*>::iterator i(q.begin()); i != q.end(); ++i)
          (*i)->post_apisetvalue();
        return;
      }
      module_markslist ml(CASTNOTEEVBASE(n)->getsinglemarks());
      if (CASTNOTEEVBASE(n)->getfulltime() > of) {
        ...
        for (const module_markobj *i(ml.marks), *ie(ml.marks + ml.n); i < ie;
          ...
          if (CASTMARKOBJ(*i)->getmarkid() == mark_vocal_text) {
            if (boost::algorithm::starts_with(CASTMARKOBJ(*i)->str, "--"))
              ddr = true;
            if (boost::algorithm::ends_with(CASTMARKOBJ(*i)->str, "--"))
              ddr0 = true;
            mks0->push_back(CASTMARKOBJ(*i));
          }
        }
        if (ddr) {
          for (std::vector<markobj*>::iterator i(mks->begin()); i != mks->end();
               ++i)
            (*i)->insureddr();
          for (std::vector<markobj*>::iterator i(mks0->begin());
               i != mks0->end(); ++i)
            (*i)->insureddl();
        }
        mks = mks0;
  ...
  inline bool debugmarksinorder(const module_markslist& x) {
    if (x.n >= 2) {
      for (const module_markobj *m(x.marks), *me(x.marks + x.n - 1); m < me;
           ++m) {
        if (CASTMARKOBJ(*m)->getmarkid() > (CASTMARKOBJ(*(m + 1)))->getmarkid())
          return false;
      }
    }
    return true;
        ...
        break;
      {
        ...
        for (const module_markobj* m(e.marks); m < me; ++m) {
          ...
          if (moddata ? markdefs[id].getisstaff() : markdefs[id].getisvoice()) {
            ...
            if (markdefs[id].getcantouch_nomut(*CASTNOTEEVBASE(n)) &&
                cso) { // ambiguous end/begin order
              ...
              std::multiset<int>::const_iterator i(st.find(id));
              if (i == st.end()) {
                CERR << "invalid end mark";
                integerr("mark spanners");
              }
              st.erase(i);
              rec.insert(id); // ends founds w/ matches
            }
        ...
        for (const module_markobj* m(b.marks); m < mbe; ++m) {
          ...
          if (moddata ? markdefs[id].getisstaff() : markdefs[id].getisvoice()) {
            if (st.find(id) !=
                    st.end() // already another of same one missing an end
                || (!markdefs[id].getcantouch_nomut(
                        *CASTNOTEEVBASE(n)) // can't touch (so can't be a new
                                            // beginning on same note)
                    && rec.find(id) != rec.end())) { // corresponding end w/
                                                     // beginning already found
              CERR << "invalid begin mark";
              integerr("mark spanners");
            }
            st0.insert(id); // defer adding them to st, in case there is more
                            // than one begin
          }
        }
        ...
        for (const module_markobj* m(e.marks); m < me; ++m) {
          ...
          if (rec.find(id) != rec.end())
            continue; // end mark has already been matched
          if (moddata ? markdefs[id].getisstaff() : markdefs[id].getisvoice()) {
            bool cso = markdefs[id].getcanspanone_nomut(*CASTNOTEEVBASE(n));
            if (markdefs[id].getcantouch_nomut(*CASTNOTEEVBASE(n)) &&
                cso) { // ambiguous end/begin order
              ...
              if (i != st.end())
                st.erase(i);
            } else if (cso) { // finish up what was ignored above, now that
                              // starts were iterated through...
              std::multiset<int>::const_iterator i(st.find(id));
              if (i == st.end()) {
                CERR << "invalid end mark";
                integerr("mark spanners");
  ...
  inline bool isconflict(
      const markobj& x,
      const module_markobj y) { // x is always end mark, y is always begin mark
    assert(x.getwhich() == marks_end);
    assert(CASTMARKOBJ(y)->getwhich() == marks_begin);
    // return (x.getisconflict() ? x.getmarkid() - 3 : x.getmarkid() - 1) ==
    // (CASTMARKOBJ(y)->getisconflict() ? CASTMARKOBJ(y)->getmarkid() - 2 :
    // CASTMARKOBJ(y)->getmarkid());
  ...
  void finalmarksdoit(FOMUS fom, void* moddata) { // BY PART, BY VOICE
    ...
    while (true) {
      ...
      for (const module_markobj* m(e.marks); m < me; ++m) {
        ...
        if (moddata ? markdefs[id].getisstaff()
                    : markdefs[id].getisvoice()) { // only if voice/staff
          ...
          if (i != mks.end()) {
            ...
            CASTMARKOBJ(*m)->sort =
                (hassome(b.marks, be,
                         boost::lambda::bind(
                             isconflict,
                             boost::lambda::constant_ref(*CASTMARKOBJ(*m)),
                             boost::lambda::_1))
                     ? 0
                     : 2);
            DBG("(a) MARK @ " << numb(CASTNOTEEVBASE(n)->gettime())
                              << " ENDMARK order = " << CASTMARKOBJ(*m)->sort
                              << std::endl);
      ...
      for (const module_markobj* m(e.marks); m < me; ++m) {
        int id = CASTMARKOBJ(*m)->getmarkid() - 1; // the begin mark
        if ((moddata ? markdefs[id].getisstaff() : markdefs[id].getisvoice()) &&
            skp.find(id) == skp.end()) { // only if voice/staff and not skipping
          std::set<int>::iterator i(mks.find(id));
          if (i != mks.end()) {
            mks.erase(id);
            CASTMARKOBJ(*m)->sort = 2;
            DBG("(b) MARK @ " << numb(CASTNOTEEVBASE(n)->gettime())
                              << " ENDMARK order = " << CASTMARKOBJ(*m)->sort
                              << std::endl);
          }
        }
  ...
  void spreadmarksdoit(FOMUS fom, void* moddata) { // BY MEAS, BY VOICE
    ...
    while (true) {
      ...
      if (!n)
        break;
      DBG("spreading " << numb(CASTNOTEEVBASE(n)->gettime()) << " voice "
                       << CASTNOTEEVBASE(n)->getvoice() << std::endl);
      CASTNOTEEVBASE(n)->spreadgetmarks(mrks); // note is emptied
      module_noteobj nx = stageobj->api_peeknextnote((modobjbase*) n);
      if (!n0)
        n0 = n;
      if (!nx ||
          CASTNOTEEVBASE(n)->getfulltime() !=
              CASTNOTEEVBASE(nx)->getfulltime()) { // next note isn't in chord
        while (true) {
          DBG("!!!" << std::endl);
  ...
  void postbeamsdoit(FOMUS fom, void* moddata) { // BYMEAS, VOICE
    ...
    while (true) {
        ...
        break;
      {
        ...
        if (bl >= 0 && CASTNOTEEVBASE(n)->getfulltime_nomut() == t)
          CASTNOTEEVBASE(n)->assignbeams2(bl, br, LOCKARG);
        else {
          if (CASTNOTEEVBASE(n)->getisnote() &&
              !CASTNOTEEVBASE(n)->getisperc_nomut()) {
            CASTNOTEEV(n)->getblbr(bl, br);
            t = CASTNOTEEVBASE(n)->getfulltime_nomut();
          } else
            bl = -1;
        }
        if (CASTNOTEEVBASE(n)->isbeambeg() && CASTNOTEEVBASE(n)->isinnerbeg())
          b = n;
        else if (b && CASTNOTEEVBASE(n)->isbeamend() &&
                 CASTNOTEEVBASE(n)->isinnerend()) {
          UNLOCKUPREAD;
          module_noteobj j = b;
          do {
          ...
          b = 0;
        } else if (!CASTNOTEEVBASE(n)->isbeammid())
          b = 0;
      }
      CASTNOTEEVBASE(n)->post_apisetvalue();
    }
  ...
  void fillholesdoit(FOMUS fom, void* moddata) { // BY MEAS, BY VOICE
    ...
    DBG("the measure is: " << &m << " @ t=" << (const numb&) m.gettime()
                           << " - t=" << (const numb&) m.getendtime()
                           << std::endl);
    ...
    AGAIN:
      ...
      if (CASTNOTEEVBASE(n)->getisgrace()) {
        ...
        do {
          ...
          int_skipassign(n);
          n = stageobj->api_nextnote();
        } while (n && CASTNOTEEVBASE(n)->getisgrace() &&
                 CASTNOTEEVBASE(n)->gettime() <= ti);
        assert(!nov.empty());
        assert(e > std::numeric_limits<fomus_int>::min() + 1);
        ranges grholes(range(CASTNOTEEVBASE(nov.front())->getgracetime(), e));
        for (std::vector<module_noteobj>::const_iterator i(nov.begin());
             i != nov.end(); ++i) {
          grholes.remove_range(range(CASTNOTEEVBASE(*i)->getgracetime(),
                                     CASTNOTEEVBASE(*i)->getgraceendtime()));
        }
        for (ranges::const_iterator i(grholes.begin()); i != grholes.end(); ++i)
          m.fillgrhole(ti, i->x1, i->x2, v);
        if (!n)
          break;
        else
          goto AGAIN;
    ...
    DISABLEMUTCHECK;
    assert(v);
    for (ranges::const_iterator i(holes.begin()); i != holes.end(); ++i)
      m.fillhole(i->x1, i->x2, v);
  }
  ...
  struct groupstr {
    ...
    groupstr(const int ind, const parts_grouptype type,
             const scorepartlist::iterator p1, const scorepartlist::iterator p2)
        : ind1(ind), type(type), p1(p1), p2(p2) {}
  ...
  struct splless : public std::binary_function<const scorepartlist_it&,
                                               const scorepartlist_it&, bool> {
    bool operator()(const scorepartlist_it& x,
                    const scorepartlist_it& y) const {
      return x->get() < y->get();
    }
  };
  ...
  void fomusdata::postparts() {
    ...
    std::list<groupstr> grps;
    {
      ...
      for (scorepartlist_it i(scoreparts.begin()); i != scoreparts.end(); ++i) {
        part& p = (*i)->getpart();
        for (std::map<int, parts_grouptype>::const_iterator
                 j(p.getbgroups().begin()),
             je(p.getbgroups().end());
             j != je; ++j) {
          std::map<int, groupstr*>::iterator t(grps0.find(j->first));
          if (t == grps0.end()) {
            grps.push_back(
                groupstr((*i)->index(), j->second, i, scoreparts.end()));
            grps0.insert(
                std::map<int, groupstr*>::value_type(j->first, &grps.back()));
          } else {
            if (t->second->p2 != scoreparts.end()) {
              CERR << "invalid part group";
              integerr("parts");
            }
    ...
    for (std::list<groupstr>::const_iterator i(grps.begin()); i != grps.end();
         ++i) {
      if (i->p2 == scoreparts.end()) {
        CERR << "invalid part group";
        integerr("parts");
      }
      std::map<scorepartlist_it, int, splless>::iterator l1 =
          grplvls.find(i->p1);
      int lvl;
      if (l1 == grplvls.end()) {
        grplvls
            .insert(
                std::map<scorepartlist_it, int, splless>::value_type(i->p1, 1))
            .first;
        lvl = 1;
      } else {
        lvl = ++l1->second;
      ...
      do {
        ...
        std::map<scorepartlist_it, int, splless>::iterator l2 =
            grplvls.find(i0);
        int lvl0;
        if (l2 == grplvls.end()) {
          grplvls
              .insert(
                  std::map<scorepartlist_it, int, splless>::value_type(i0, 1))
              .first;
          lvl0 = 1;
        } else {
          lvl0 = ++l2->second;
        ...
        if (lvl != lvl0) {
          CERR << "overlapping parts group";
          integerr("parts");
        }
      } while (i0 != i->p2);
      (*i->p1)->insbgroups(lvl, i->type);
      (*i0)->insegroups(lvl);
      DBG("LAYER " << lvl << " BRACKET AROUND: " << (*i->p1)->getid() << " and "
                   << (*i0)->getid() << std::endl);
    }
  }
  ...
  void
  postspecialdoit(FOMUS fom,
                  void* moddata) { // bymeas--this must be last in stage!!!!
  ...
  void markhelpersdoit(FOMUS fom, void* moddata) { // by meas
    ...
    const listelmap &map1(pmp.get_varbase(MARKTEXTS_ID).getmapval()),
    ...
    while (true) {
      ...
      bool rca = false;
      for (const module_markobj *m(ma.marks), *me(ma.marks + ma.n); m != me;
           ++m) {
        int id = CASTMARKOBJ(*m)->getmarkid();
        switch (markdefs[id].getbaseid()) {
        case mark_sul: {
          assert(sulstyles.find(CASTNOTEEVBASE(n)->get_sval(SULSTYLE_ID)) !=
                 sulstyles.end());
          assert(CASTMARKOBJ(*m)->val.type() == module_int);
          static const char lett[7] = {'C', 'D', 'E', 'F', 'G', 'A', 'B'};
          switch (sulstyles.find(CASTNOTEEVBASE(n)->get_sval(SULSTYLE_ID))
                      ->second) {
          case ss_letter:
            CASTMARKOBJ(*m)->str =
                lett[diatonicnotes[CASTMARKOBJ(*m)->val.geti() % 12]];
            break;
          case ss_sulletter:
            CASTMARKOBJ(*m)->str =
                sul + lett[diatonicnotes[CASTMARKOBJ(*m)->val.geti() % 12]];
            break;
          case ss_roman:
            CASTMARKOBJ(*m)->str = toromanstr(CASTMARKOBJ(*m)->val.geti());
            break;
          case ss_sulroman:
            CASTMARKOBJ(*m)->str =
                sul + toromanstr(CASTMARKOBJ(*m)->val.geti());
            break;
          }
        } break;
        case mark_pizz:
        ...
        case mark_ord: {
          ...
          if (i == map1.end())
            i = map2.find(na);
          CASTMARKOBJ(*m)->str =
              (i != map2.end() ? listel_getstring(i->second) : qu);
        } break;
        case mark_text:
        ...
        case mark_stafftext_begin: {
          ...
          if (i != smap.end())
            CASTMARKOBJ(*m)->str = listel_getstring(i->second);
        } break;
        }
        if (npgr && markdefs[id].ispgroupmark()) {
          CASTNOTEEVBASE(n)->assignmarkrem(id, CASTMARKOBJ(*m)->str.c_str(),
                                           CASTMARKOBJ(*m)->val);
          rca = true;
        }
      }
  ...
  struct mhelpstr {
    ...
    void ins(const int i, const std::string& str) { /*INTWRITELOCK(n);*/
      CASTNOTEEVBASE(n)->assignmarkins(i, str, numb(module_none));
      m = true;
    }
    void rem(const int i) {
      CASTNOTEEVBASE(n)->assignmarkrem(i, 0, numb(module_none));
      m = true;
    }
  };
  void contmarksaux1(const std::map<int, std::string>& s1,
                     const std::map<int, std::string>& s2,
                     std::vector<mhelpstr>& objs2) {
    for (std::map<int, std::string>::const_iterator i(s2.begin());
         i != s2.end(); ++i) {
      if (i->first < 0)
        continue;
      int id = i->first - 2; // id of begin mark
      if (s1.find(i->first) == s1.end() &&
          s1.find(-id) == s1.end()) { // insert a begin mark
        std::for_each(
            objs2.begin(), objs2.end(),
            boost::lambda::bind(&mhelpstr::ins, boost::lambda::_1, id,
                                boost::lambda::constant_ref(i->second)));
      }
    }
  }
  void contmarksaux2(const std::map<int, std::string>& s1,
                     const std::map<int, std::string>& s2,
                     std::vector<mhelpstr>& objs1) {
    for (std::map<int, std::string>::const_iterator i(s1.begin());
         i != s1.end(); ++i) {
      if (i->first < 0)
        continue;
      int id = i->first - 1; // id of end mark
      if (s2.find(i->first) == s2.end() &&
          s2.find(-id) == s2.end()) { // insert an end mark
        std::for_each(
            objs1.begin(), objs1.end(),
            boost::lambda::bind(&mhelpstr::ins, boost::lambda::_1, id,
                                boost::lambda::constant_ref(i->second)));
      }
    }
    std::for_each(objs1.begin(), objs1.end(),
                  boost::lambda::bind(&mhelpstr::ass, boost::lambda::_1));
  }

  void contmarksdoit(FOMUS fom, void* moddata) {
    DBG("contmarksdoit" << std::endl);
    std::auto_ptr<std::map<int, std::string>> s1(
        new std::map<int, std::string>),
        s2(new std::map<int, std::string>);
    offgroff o((fomus_int) 0);
    std::auto_ptr<std::vector<mhelpstr>> objs1(new std::vector<mhelpstr>),
        objs2(new std::vector<mhelpstr>);
    while (true) {
      module_noteobj n = stageobj->api_nextnote();
      if (!n) {
      ...
      for (const module_markobj *m(ma.marks), *me(ma.marks + ma.n); m != me;
           ++m) {
        int id = CASTMARKOBJ(*m)->getmarkid();
        if (markdefs[id].iscont() &&
            (moddata ? markdefs[id].getisstaff() : markdefs[id].getisvoice())) {
          s2->insert(
              std::map<int, std::string>::value_type(id, CASTMARKOBJ(*m)->str));
          objs2->back().rem(id);
        }
      }
      {
        ...
        for (const module_markobj *m(ma.marks), *me(ma.marks + ma.n); m != me;
          ...
          if (moddata ? markdefs[id].getisstaff() : markdefs[id].getisvoice())
            s2->insert(
                std::map<int, std::string>::value_type(-id, std::string()));
        }
      }
      {
