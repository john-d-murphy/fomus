...
namespace fomus {
...
#else
      to.insert(
          printmap_val(numtorat(listel_getnumb(el.find(str)->second)), str));
#endif
    }
  }
  bool listofmods::isvalid(const enum module_type ty) {
    ...
    mval.reset();
    initmodval();
    if (!module_valid_listofstrings(mval, -1, -1, -1, -1, 0, typestr.c_str()))
      return false;
    for (module_value *i = mval.val.l.vals,
      ...
      if (m == modsbyname.end()) {
        ...
        CERR << "module `" << i->val.s << "' is not of type `"
             << modtypetostr(ty) << '\'';
        return false;
      }
    }
  ...
  void percinstrs_var::initmodval() {
    ...
    for_each2(map.begin(), map.end(), mval.val.l.vals,
              boost::lambda::bind(
                  &percinstr_str::getmodval,
                  boost::lambda::bind(
  ...
  varbase* percinstrs_var::getnewprepend(const globpercsvarvect& val,
                                         const filepos& p) {
  ...
  std::string percinstrs_var::getvalstr(const fomusdata* fd,
                                        const char* st) const {
    std::ostringstream o;
    o << '(';
    if (!map.empty()) {
    ...
    for_each2(map.begin(), map.end(), mval.val.l.vals,
              boost::lambda::bind(
                  ...
                  boost::lambda::bind(
                      ...
                      boost::lambda::bind(&globinstsvarvect_val::second,
                                          boost::lambda::_1)),
                  boost::lambda::_2, false));
  }

  varbase* instrs_var::getnewprepend(const globinstsvarvect& val,
                                     const filepos& p) {
  ...
  void var_keysigs::redo(const fomusdata* fd) {
    assert(varisvalid());
    el.clear();
    for (boost::ptr_map<const std::string,
  ...
  bool var_keysigs::keysig_isinvalid(const fomusdata* fd, int& n,
                                     const char*& s,
  ...
  bool var_keysigs::module_valid_keysigs(const fomusdata* fd) {
    assert(varisvalid());
    if (!module_valid_listaux(mval, -1, -1, gettypedoc()))
      return false;
    int n = -1;
    const char* s;
    if (hassome(mval.val.l.vals, mval.val.l.vals + mval.val.l.n,
                boost::lambda::bind(&var_keysigs::keysig_isinvalid, this, fd,
                                    boost::lambda::var(n),
  ...
  void redosig(const boost::ptr_vector<userkeysigent>& user, listelvect& el,
               ...
               const printmap& micpr,
               const printmap& octpr) { // yes, sig is copied!
    for (boost::ptr_vector<userkeysigent>::const_iterator i(user.begin());
         i != user.end(); ++i) {
      assert(i->isvalid());
  ...
  BOOST_SPIRIT_OPAQUE_RULE_PARSER(
      notematchhasoct,
      (8, (((const boostspirit::symbols<numb>&), notesym),
           ((const boostspirit::symbols<numb>&), accsym),
           ((const boostspirit::symbols<numb>&), micsym),
           ((const boostspirit::symbols<numb>&), octsym), ((numb&), nov),
           ((numb&), acv), ((numb&), miv), ((numb&), ocv))),
      -,
      (notesym[setnumval<numb>(nov)] >> !accsym[setnumval<numb>(acv)] >>
       !micsym[setnumval<numb>(miv)] >> !octsym[setnumval<numb>(ocv)] >>
       boostspirit::end_p) |
          boostspirit::eps_p[setconstval<numb>(nov, module_none)])

  inline const char* endof(const char* x) {
    while (*x)
      ++x;
    return x;
  }
  bool doparsekeysig(const fomusdata* fd, const module_value& lst,
    ...
    for (const module_value *i = lst.val.l.vals,
      ...
      if (o.numerator() !=
        ...
        rat x0(o + m);
        if (x0.denominator() != 1 || isblack(x0.numerator()))
  ...
  BOOST_SPIRIT_OPAQUE_RULE_PARSER(
      ...
      (8, (((const boostspirit::symbols<numb>&), notesym),
           ((const boostspirit::symbols<numb>&), accsym),
           ((const boostspirit::symbols<numb>&), micsym),
           ((const boostspirit::symbols<numb>&), octsym), ((numb&), prval),
           ((bool&), gup), ((numb&), val), ((module_noteparts&), nps))),
      -,
      (notesym[setnumval<numb>(val)][setnummodval<numb>(nps.note)] >>
       ...
       (octsym[addnumval(val)][setnummodval<numb>(nps.oct)] |
        boostspirit::eps_p[nearestpitch(val, prval, gup)]) >>
       boostspirit::end_p) |
          boostspirit::eps_p[setconstval<numb>(val, module_none)])
  numb doparsenote(fomusdata* fd, const char* no, const bool noneonerr,
                   module_noteparts* xtra) {
    assert(fd); // doparsenote should only get called when user is entering a
                // string and the setting is a module_notesym type, or when a
                // pitch is entered
    numb val0(module_none);
    assert(val0.type() == module_none);
    assert(val0.isnull());
    module_noteparts xtra0;
    parserule rl(relnotematch(fd->getnotepa(), fd->getaccpa(), fd->getmicpa(),
                              fd->getoctpa(), fd->prevnote, fd->prevnotegup,
                              val0, xtra ? *xtra : xtra0));
  ...
  BOOST_SPIRIT_OPAQUE_RULE_PARSER(
      accmatch,
      (4, (((const boostspirit::symbols<numb>&), accsym),
           ((const boostspirit::symbols<numb>&), micsym), ((numb&), val),
           ((module_noteparts&), nps))),
      -,
      (!accsym[addnumvalac(val)][setnummodval<numb>(nps.acc1)] >>
       !micsym[addnumvalac(val)][setnummodval<numb>(nps.acc2)] >>
       boostspirit::end_p) |
          boostspirit::eps_p[setconstval<numb>(val, module_none)])
  numb doparseacc(const fomusdata* fd, const char* no, const bool noneonerr,
                  module_noteparts* xtra) {
    assert(fd); // doparsenote should only get called when user is entering a
                // string and the setting is a module_notesym type, or when a
                // pitch is entered
    ...
    assert(val0.isnull());
    module_noteparts xtra0;
    parserule rl(accmatch((fd ? fd->getaccpa() : acc_parse),
                          (fd ? fd->getmicpa() : mic_parse), val0,
                          xtra ? *xtra : xtra0));
    parse_it p(no, endof(no));
    parse(p, parse_it(), rl);
  ...
  BOOST_SPIRIT_OPAQUE_RULE_PARSER(
      durmatchaux,
      (5, (((const boostspirit::symbols<numb>&), durdot),
           ((const boostspirit::symbols<numb>&), dursyms),
           ((const boostspirit::symbols<numb>&), durtie),
           ((const boostspirit::symbols<numb>&), tupsyms), ((numb&), val))),
      -,
      dursyms[setnumval<numb>(val)] >> !durdot[multnumval(val)] >>
          *tupsyms[multnumval(val)])
  BOOST_SPIRIT_OPAQUE_RULE_PARSER(
      durmatch,
      (6, (((const boostspirit::symbols<numb>&), durdot),
           ((const boostspirit::symbols<numb>&), dursyms),
           ((const boostspirit::symbols<numb>&), durtie),
           ((const boostspirit::symbols<numb>&), tupsyms), ((numb&), val),
           ((numb&), val2))),
       ...
       *(durtie >> durmatchaux(durdot, dursyms, durtie, tupsyms,
                               val2)[addconstnumval(val, val2)]) >>
       boostspirit::end_p) |
          boostspirit::eps_p[setconstval<numb>(val, module_none)])

  numb doparsedur(fomusdata* fd, const char* no, const bool noneonerr) {
    numb val0(module_none), vals;
    parserule rl(durmatch(fd->getdurdot(), fd->getdursyms(), fd->getdurtie(),
                          fd->gettupsyms(), val0, vals));
    parse_it p(no, endof(no));
    parse(p, parse_it(), rl);
    if (val0.isnull() && !noneonerr && fd) {
  ...
  void initaccrules() {
    ...
    accrules.insert(
        accrulestype::value_type("note-naturals", accrule_notenats));
    barlines.insert(
        std::map<std::string, module_barlines>::value_type("", barline_normal));
    barlines.insert(std::map<std::string, module_barlines>::value_type(
        "!", barline_normal));
    barlines.insert(std::map<std::string, module_barlines>::value_type(
        "!!", barline_double));
    barlines.insert(std::map<std::string, module_barlines>::value_type(
    ...
    tremties.insert(std::map<std::string, enum_tiestyles, isiless>::value_type(
        "none", tiestyle_none));
    tremties.insert(std::map<std::string, enum_tiestyles, isiless>::value_type(
        "tied", tiestyle_tied));
    tremties.insert(std::map<std::string, enum_tiestyles, isiless>::value_type(
        "dashed", tiestyle_dashed));
    tremties.insert(std::map<std::string, enum_tiestyles, isiless>::value_type(
        "dotted", tiestyle_dotted));
  }

  inline void thrownewmod(const char* name, const std::string& modname) {
    CERR << "invalid default value for setting `" << name << "' in module `"
         << modname << '\'' << std::endl;
    throw errbase();
  }
  ...
  void modvaltolistelvect(const modbase& mod, const module_setting& set,
                          listelvect& el) {
    for (struct module_value *i = set.val.val.l.vals,
                             *ie = set.val.val.l.vals + set.val.val.l.n;
         i != ie; ++i) {
      switch (i->type) {
      case module_bool:
      case module_int:
      ...
      case module_list: {
        ...
        for (struct module_value *j = i->val.l.vals,
  ...
  void modvaltolistelmap(const modbase& mod, const module_setting& set,
    ...
    for (struct module_value *i = set.val.val.l.vals,
      ...
      case module_rat:
        el.insert(
            listelmap_val(i->val.s, numb(rat(j->val.r.num, j->val.r.den))));
        break;
      case module_string:
  ...
  void newmodnumvar(const modbase& mod, const module_setting& set) {
    ...
    if (set.val.type == module_int) {
      vars.push_back(boost::shared_ptr<varbase>(
          v = new var_modnum(set.val.val.i, mod, set)));
    } else if (set.val.type == module_float) {
      vars.push_back(boost::shared_ptr<varbase>(
          v = new var_modnum(set.val.val.f, mod, set)));
    } else if (set.val.type == module_rat) {
      vars.push_back(boost::shared_ptr<varbase>(
          v = new var_modnum(rat(set.val.val.r.num, set.val.val.r.den), mod,
                             set)));
    } else {
      thrownewmod(set.name, mod.getsname());
    }
  ...
  void newmodboolvar(const modbase& mod, const module_setting& set) {
    ...
    if (set.val.type == module_int) {
      vars.push_back(boost::shared_ptr<varbase>(
          v = new var_modbool(set.val.val.i, mod, set)));
    } else if (set.val.type == module_float) {
      vars.push_back(boost::shared_ptr<varbase>(
          v = new var_modbool(set.val.val.f, mod, set)));
    } else if (set.val.type == module_rat) {
      vars.push_back(boost::shared_ptr<varbase>(
          v = new var_modbool(rat(set.val.val.r.num, set.val.val.r.den), mod,
                              set)));
    } else {
      thrownewmod(set.name, mod.getsname());
    }
  ...
  void newmodnotevar(const modbase& mod, const module_setting& set) {
    ...
    if (set.val.type == module_int) {
      vars.push_back(boost::shared_ptr<varbase>(
          v = new var_modnote(set.val.val.i, mod, set)));
    } else if (set.val.type == module_float) {
      vars.push_back(boost::shared_ptr<varbase>(
          v = new var_modnote(set.val.val.f, mod, set)));
    } else if (set.val.type == module_rat) {
      vars.push_back(boost::shared_ptr<varbase>(
          v = new var_modnote(rat(set.val.val.r.num, set.val.val.r.den), mod,
                              set)));
    } else {
      thrownewmod(set.name, mod.getsname());
    }
  ...
  void newmodlistofnumsvar(const modbase& mod, const module_setting& set) {
    if (set.val.type != module_list)
      thrownewmod(set.name, mod.getsname());
    varbase* v;
    vars.push_back(
        boost::shared_ptr<varbase>(v = new var_modlistofnums(mod, set)));
    varslookup.insert(varsmap_val(v->getname(), v));
  }
  void newmodlistofstringsvar(const modbase& mod, const module_setting& set) {
    ...
    varbase* v;
    vars.push_back(
        boost::shared_ptr<varbase>(v = new var_modlistofstrings(mod, set)));
    varslookup.insert(varsmap_val(v->getname(), v));
  }
  ...
  void newmodmaptonumsvar(const modbase& mod, const module_setting& set) {
    ...
    varbase* v;
    vars.push_back(
        boost::shared_ptr<varbase>(v = new var_modmaptonums(mod, set)));
    varslookup.insert(varsmap_val(v->getname(), v));
  }
  void newmodmaptostringsvar(const modbase& mod, const module_setting& set) {
    ...
    varbase* v;
    vars.push_back(
        boost::shared_ptr<varbase>(v = new var_modmaptostrings(mod, set)));
    varslookup.insert(varsmap_val(v->getname(), v));
  }
  void newmodlistofnumlistsvar(const modbase& mod, const module_setting& set) {
    ...
    varbase* v;
    vars.push_back(
        boost::shared_ptr<varbase>(v = new var_modlistofnumlists(mod, set)));
    varslookup.insert(varsmap_val(v->getname(), v));
  }
  void newmodlistofstringlistsvar(const modbase& mod,
                                  const module_setting& set) {
    if (set.val.type != module_list)
      thrownewmod(set.name, mod.getsname());
    varbase* v;
    vars.push_back(
        boost::shared_ptr<varbase>(v = new var_modlistofstringlists(mod, set)));
    varslookup.insert(varsmap_val(v->getname(), v));
  }
  void newmodmaptonumlistsvar(const modbase& mod, const module_setting& set) {
    ...
    varbase* v;
    vars.push_back(
        boost::shared_ptr<varbase>(v = new var_modmaptonumlists(mod, set)));
    varslookup.insert(varsmap_val(v->getname(), v));
  }
  void newmodmaptostringlistsvar(const modbase& mod,
                                 const module_setting& set) {
    if (set.val.type != module_list)
      thrownewmod(set.name, mod.getsname());
    varbase* v;
    vars.push_back(
        boost::shared_ptr<varbase>(v = new var_modmaptostringlists(mod, set)));
    varslookup.insert(varsmap_val(v->getname(), v));
  }

  void addmodvar(const modbase& mod, const module_setting& s) {
    if (s.name == NULL) {
      CERR << "missing setting name in module `" << mod.getsname() << '\''
           << std::endl;
      throw errbase();
    }
    switch (s.type) {
    ...
    default:
#endif
      CERR << "bad type for setting `" << s.name << "' in module `"
           << mod.getsname() << '\'' << std::endl;
    }
  }
...
public:
  ...
  void
  listel_tomodval::operator()(const boost::shared_ptr<listelvect>& x) const {
    val.type = module_list;
    struct module_list& l(val.val.l);
    l.vals = newmodvals(l.n = x->size());
    struct module_value* v = l.vals;
    for (listelvect_constit i(x->begin()); i != x->end(); ++i, ++v)
      apply_visitor(listel_tomodval(*v), *i);
  }
  ...
  void listvarbase::initmodval() {
    ...
    struct module_value* v = l.vals;
    for (listelvect_constit i(el.begin()); i != el.end(); ++i, ++v)
      apply_visitor(listel_tomodval(*v), *i);
  }
  ...
  void initvars() {
    ...
    {
      var_notesymbols* x;
      vars.push_back(boost::shared_ptr<varbase>(x = new var_notesymbols));
      x->activate(xx);
    }
    {
      var_noteaccs* x;
      vars.push_back(boost::shared_ptr<varbase>(x = new var_noteaccs));
      x->activate(xx);
    }
    {
      var_notemicrotones* x;
      vars.push_back(boost::shared_ptr<varbase>(x = new var_notemicrotones));
      x->activate(xx);
    }
    {
      var_noteoctaves* x;
      vars.push_back(boost::shared_ptr<varbase>(x = new var_noteoctaves));
      x->activate(xx);
    }
    ...
    {
      var_dursymbols* x;
      vars.push_back(boost::shared_ptr<varbase>(x = new var_dursymbols));
      x->activate(xx);
    }
    {
      var_durdot* x;
      vars.push_back(boost::shared_ptr<varbase>(x = new var_durdot));
      x->activate(xx);
    }
    {
      var_durtie* x;
      vars.push_back(boost::shared_ptr<varbase>(x = new var_durtie));
      x->activate(xx);
    }
    {
      var_tupsyms* x;
      vars.push_back(boost::shared_ptr<varbase>(x = new var_tupsyms));
      x->activate(xx);
    }
    {
      var_dumpingmsg* x;
      vars.push_back(boost::shared_ptr<varbase>(x = new var_dumpingmsg));
      x->activate(xx);
    }
    ...
    for (varsvect_constit i(vars.begin()); i != vars.end(); ++i)
      varslookup.insert(varsmap_val((*i)->getname(), i->get()));
  }
  ...
  struct catcherr {
    ...
    catcherr(parserule& recov, const parserule& setrule, bool& err)
        : recov(recov), setrule(setrule), err(err) {}
  ...
  boostspirit::error_status<> catcherr::operator()(
  ...
  BOOST_SPIRIT_OPAQUE_RULE_PARSER(rest,
                                  (2, (((confscratch&), xx),
                                       ((parserule&), cont))),
                                  -, cont >> commatch)
  ...
  BOOST_SPIRIT_OPAQUE_RULE_PARSER(
      oksymrule,
      (3, (((const boostspirit::symbols<parserule*>), conts),
           ((parserule&), cont), ((confscratch&), xx))),
      -,
      recerrpos(xx.pos.file, xx.pos.line, xx.pos.col, xx.pos.modif) >>
          symmatcherr(conts, cont, "+:=", xx.str, xx.pos, ferr) >>
          pluseqlmatch(xx.isplus) >> rest(xx, cont) >> commatch)
  BOOST_SPIRIT_OPAQUE_RULE_PARSER(
      recovsymrule,
      (5,
       (((const boostspirit::symbols<parserule*>), conts), ((parserule&), cont),
        ((parserule&), curloop), ((parserule&), okrule), ((confscratch&), xx))),
      -,
      (
#ifndef NDEBUG
          ...
          >> commatch >>
          !(recerrpos(xx.pos.file, xx.pos.line, xx.pos.col, xx.pos.modif) >>
            symmatch(conts, cont, "+:=") >> pluseqlmatch(xx.isplus) >>
            rest(xx, cont) >> commatch)[setcurloop(curloop, okrule)]))
  BOOST_SPIRIT_OPAQUE_RULE_PARSER(
      mainrule,
      (5, (((boostspirit::guard<filepos*>&), theguard), ((parserule&), curloop),
           ((parserule&), recovrule),
           ((const boost::filesystem::path&), filename), ((bool&), err))),
      -,
      !boostspirit::str_p("\xef\xbb\xbf") >> commatch >>
          *(theguard(if_p(boostspirit::end_p)[boostspirit::nothing_p]
                         .else_p[curloop])[catcherr(curloop, recovrule, err)]))

  inline bool confvarssort(const varbase& x, const varbase& y) {
    if (x.getline() != y.getline())
      return x.getline() < y.getline();
    else
      return x.getcol() < y.getcol();
  }
  inline bool confvarssortp(const varbase* x, const varbase* y) {
    return confvarssort(*x, *y);
  }
  void doloadconf(const boost::filesystem::path& fn) {
    ...
    try {
      ...
      f.open(fn, boost::filesystem::ifstream::in |
      ...
      p.set_tabchars(1);
      p.set_position(
          boostspirit::file_position_base<std::string>(fn.FS_FILE_STRING()));
      boostspirit::symbols<parserule*>
          conts; // plconts is settings that are followed by '+='
      ...
      std::for_each(vars.begin(), vars.end(),
                    (boost::lambda::bind(
                         ...
                         boost::lambda::bind(&boost::shared_ptr<varbase>::get,
                                             boost::lambda::_1),
                         boost::lambda::var(conts), &symrules[0]),
                     boost::lambda::bind(
                         &varbase::addconfrule,
                         boost::lambda::bind(&boost::shared_ptr<varbase>::get,
                                             boost::lambda::_1),
                         &symrules[0], boost::lambda::var(xx))));
      parserule cont;
  ...
  void loadconf() {
    try {
      ...
      CERR << "error accessing `" << fomusconfig.FS_FILE_STRING()
  ...
  struct unnestmapkey : public boost::static_visitor<const std::string&> {
    const std::string& operator()(const numb& i) const {
      throw typeerr();
    }
    ...
    const std::string&
    operator()(const boost::shared_ptr<listelvect>& str) const {
      throw typeerr();
    }
  };

  listelmap varbase::nestedlisttomap(const listelvect& vect,
...
#warning "check how throw typeerr is used--replace with errbase"
  ...
  std::string ordmapvartonums::getvalstr(const fomusdata* fd,
                                         const char* st) const {
    std::ostringstream s;
    s << '(';
    if (!el.empty()) {
      ...
      assert(i != el.end());
      s << stringify(i->first, ":=,") << ' ' << i->second;
      for (listelvect_constit o(boost::next(ord.begin())); o != ord.end();
           ++o) {
        listelmap_constit i(el.find(listel_getstring(*o)));
        assert(i != el.end());
        s << ", " << stringify(i->first, ":=,") << ' ' << i->second;
...
public:
    ...
    bool operator()(const boost::shared_ptr<listelvect>& x) const {
      return true;
    }
  };
  inline bool isavect(const listel& el) {
    return apply_visitor(isavectaux(), el);
  }
  std::ostream& operator<<(std::ostream& s, const listelvect& el) {
    ...
    if (!el.empty()) {
      s << el.front();
      std::for_each(boost::next(el.begin()), el.end(),
                    s << boost::lambda::constant(' ') << boost::lambda::_1);
    }
    if (forcep)
      s << ')';
    return s;
  ...
  bool localallowed(
      const enum module_setting_loc in,
      const enum module_setting_loc setloc) { // is setloc allowed in `in'?
    switch (setloc) {
  ...
  const instr_str& fomusdata::getdefinstr(const std::string& id) const {
    ...
    if (i == default_insts.end()) {
      const globinstsvarvect& mp(
          ((instrs_var*) getvarbase(INSTRS_ID))->getmap());
  ...
  int foundpreset(const char* filename, lt_ptr data) {
    try {
      ...
      boost::filesystem::path pa(
          FS_CHANGE_EXTENSION(boost::filesystem::path(filename), ".fpr"));
  ...
  void initpresets() {
    ...
    std::string pa(std::string(bp ? bp : (wsys ? wsys : "")) +
                   CMD_MACRO(PRESETS_PATH));
#else
    std::string pa(bp ? bp : CMD_MACRO(PRESETS_PATH));
#endif
    char* p = getenv("FOMUS_PRESETS_PATH");
    if (p != NULL)
      pa = std::string(p) + LT_PATHSEP_CHAR + pa;
      ...
      if (ta.fd) {
        modsvect_it i0(
            std::find_if(mods.begin(), mods.end(),
                         boost::lambda::bind(&modbase::modin_hasext,
                                             boost::lambda::_1, "fms")));
        if (i0 == mods.end()) {
          CERR << "cannot load preset, `.fms' file input module not found"
               << std::endl;
          throw errbase();
        }
        const modbase& mo = *i0;
        moddata d(mo, mo.getdata(ta.fd));
        if (mo.loadfile(ta.fd, d.get(),
                        presets.find(listel_getstring(*i))
                            ->second.FS_FILE_STRING()
                            .c_str(),
                        true))
          throw errbase();
      } else
        doloadconf(presets.find(listel_getstring(*i))->second);
    }
  }
  ...
  const char* var_inputmod::gettypedoc() const {
    ...
    if (typstr.empty()) {
      std::ostringstream ou;
      ou << "(string_ext string_modname, string_ext string_modname, ...), "
            "string_modname = ";
      bool fi = true;
      assert(!mods.empty());
      for (modsvect_constit i(mods.begin()); i != mods.end(); ++i) {
  ...
  const char* var_chooseclef::gettypedoc() const {
    ...
    if (typstr.empty()) {
      std::ostringstream s;
      s << "string_clef | (string_clef string_clef ...), string_clef = "
        << clefstypestr;
      typstr = s.str();
    }
    return typstr.c_str();
  ...
  const char* var_presets::gettypedoc() const {
    ...
    if (typstr.empty()) {
      std::ostringstream s;
      s << "string_prefix | (string_prefix string_prefix ...), string_prefix = "
        << presetstype;
      typstr = s.str();
    }
    return typstr.c_str();
  ...
  void
  redokeysigs(const symtabs& ta) { // var_keysigs, KEYSIG_ID contains allsig
    if (ta.fd) {
      assert(ta.fd->isvalid());
      DBG("in redokeysigs the address of var_keysigs is "
          << &(const var_keysigs&) ta.fd->get_varbase(KEYSIG_ID));
      assert(((const var_keysigs&) ta.fd->get_varbase(KEYSIG_ID)).varisvalid());
      assert(ta.fd->fu());
      var_keysigs* k;
      ta.fd->set_varbase(
          KEYSIG_ID, k = new var_keysigs(
                         (const var_keysigs&) ta.fd->get_varbase(KEYSIG_ID)));
      k->redo(ta.fd);
      var_keysig* x;
      ta.fd->set_varbase(
          KEYSIGDEF_ID,
          x = new var_keysig(
              (const var_keysig&) ta.fd->get_varbase(KEYSIGDEF_ID)));
      x->redo(ta.fd);
    } else {
#ifndef NDEBUG
  ...
  varbase* ordmapvartonums::getnewprepend(const listelvect& val,
                                          const filepos& p) {
    listelvect z;
    for (listelvect_constit i(ord.begin()); i != ord.end(); ++i) {
      z.push_back(*i);
