...
extern "C" {
  ...
  // must call before anything, can be called repeatedly, frees and invalidates all instances, if fomus_err() reports an error then can't continue  
  ...
  LIBFOMUS_EXPORT void fomus_rval(FOMUS f, int par, int act, fomus_int num, fomus_int den);
  LIBFOMUS_EXPORT void fomus_mval(FOMUS f, int par, int act, fomus_int val, fomus_int num, fomus_int den);
  ...
  // save instance as `.fms' file, always destroys instance (copy it first if necessary)
  LIBFOMUS_EXPORT void fomus_save(FOMUS f, const char* filename);
  
  // set output callback functions, either/both of these can be NULL, `newline' set to 1 means include newline in output
  LIBFOMUS_EXPORT void fomus_set_outputs(fomus_output out, fomus_output err, int newline);
