...
enum info_sort { info_ascending, info_descending };
...
LIBFOMUS_EXPORT const struct info_modlist
info_list_modules(struct info_modfilterlist* filter, struct info_modsearch* sim,
                  struct info_sortlist* sortlst, int limit);
LIBFOMUS_EXPORT const struct info_setlist
info_list_settings(FOMUS fom, struct info_setfilterlist* filter,
                   struct info_setsearch* sim, struct info_sortlist* sortlst,
                   int limit);
LIBFOMUS_EXPORT const struct info_marklist
info_list_marks(FOMUS fom, struct info_markfilterlist* filter,
                struct info_marksearch* sim, struct info_sortlist* sortlst,
                int limit);
// backend extensions;
...
info_settingloc_to_extstr(enum module_setting_loc loc);
...
LIBFOMUS_EXPORT const char*
info_valstr(module_obj obj); // returns "" if default, "id" if has id, or full
                             // def if no id--use module_free_string to free it

LIBFOMUS_EXPORT struct info_objinfo info_getlastentry(
    FOMUS f); // if one of the above has been entered, this gets the last entry
              // (so frontend can redisplay it, etc..)

// if f is NULL, you get the default value (only need to do this from within
// validdeps function)
