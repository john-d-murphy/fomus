...
const char* getval(const module_value& x, const std::string& id) {
  ...
  for (const module_value *i(x.val.l.vals), *ie(x.val.l.vals + x.val.l.n);
       i < ie; i += 2) {
  ...
  for (std::string::const_iterator i(str.begin()); i != str.end();) {
    if ((unsigned char) *i < 0x80) {
      ...
      case 50089:
        ...
        break;
      default:
        std::cerr << "makedocs: stringify missing UTF-8 translation for " << u
                  << ' ' << std::string(i0, i) << std::endl;
        exit(EXIT_FAILURE);
      }
    }
  }
...
const char* thecats[][3] = {
    {"basic", "Basic",
     "Settings affecting FOMUS's overall operation and performance, plus a few "
     "global score parameters."},
    {"syntax", "Syntax",
     "Settings specifying the syntax of certain items in `.fms' input files."},
    {"libs", "Libraries",
     "Settings that belong in your `.fomus' initialization file and function "
     "as libraries."},
    {"instsparts", "Instrument Definitions/Parts",
     "Settings that have more or less to do with defining instruments and/or "
     "parts."},
    {"quant", "Quantization",
     "Settings that determine how time, durations and pitches are quantized."},
    {"meas", "Measures",
     "Settings affecting measures and items related to measures."},
    {"voices", "Voices",
     "Settings affecting how notes are distributed into voices."},
    {"staves", "Staves",
     "Settings affecting how notes are distributed onto staves."},
    {"notes", "Notes",
     "Settings that allow you to override certain decisions regarding notes."},
    {"grnotes", "Grace Notes", "Settings affecting grace notes."},
    {"marks", "Marks",
     "Settings affecting the behavior of marks and mark objects."},
    {"dyns", "Dynamics",
     "Settings affecting dynamic markings and the translation of dynamic "
     "values into text markings."},
    {"accs", "Accidentals", "Settings affecting accidental spellings."},
    {"octs", "Octave Signs", "Settings affecting octave sign placement."},
    {"tuplets", "Tuplets",
     "Settings affecting how tuplets are created, plus settings to override "
     "tuplet placement."},
    {"beams", "Beams", "Settings affecting placement of beams."},
    {"special", "Special Notations",
     "Settings for special notations such as string harmonics, trills and "
     "tremolos."},
    {"rhythmic", "Rhythmic Spelling",
     "Settings affecting how notes and rests are split and tied in a measure."},
    ...
    {"lilyout", "LilyPond Output",
     "Settings affecting output to LilyPond input files."},
    {"xmlout", "MusicXML Output",
     "Settings affecting output to MusicXML files."},
    {"midiin", "MIDI Input", "Settings affecting output to MIDI files."},
    {"midiout", "MIDI Output", "Settings affecting input from MIDI files."},
    {"mods", "Modules",
     "Settings for selecting and combining modules, which contain all of "
     "FOMUS's decision-making algorithms."},
...
int main(int ac, char** av) {
  ...
  if (cmd == "sets1") {
    ...
    for (int i = 0; thecats[i][0] != 0; ++i)
      val.insert(thecats[i][0]);
    boost::ptr_map<const std::string, std::set<std::string>>
        cats; // category, vect-of-sets
    ...
    f.close();
    info_setfilter fi = {0, 0,        0, module_nomodtype, 0, module_noloc,
                         3, info_none};
    ...
    for (int i0 = 0; thecats[i0][0] != 0; ++i0) {
      boost::ptr_map<const std::string, std::set<std::string>>::const_iterator
          i(cats.find(thecats[i0][0]));
      assert(i != cats.end());
      ...
      for (int ul = 0; ul <= 3; ++ul) {
        bool fi = true;
        for (struct info_setting *i(lst.sets), *ie(lst.sets + lst.n); i < ie;
             ++i) {
          if (i->uselevel == ul && sets.find(i->name) != sets.end()) {
            if (fi) {
              static const char* ulvls[4] = {"Beginner", "Intermediate",
                                             "Advanced", "Guru"};
              std::cout << "@item " << stringify(ulvls[ul])
                        << "\n"; // different kind of list?
              fi = false;
            }
            std::cout << "@ref{set" << stripnonalpha(i->name) << ",,"
                      << stringify(i->name) << "}  ";
          }
        }
        if (!fi)
    ...
    for (struct info_setting *i(lst.sets), *ie(lst.sets + lst.n); i < ie; ++i) {
      ...
      std::cout << "@anchor{set" << stripnonalpha(i->name) << "}\n@item "
                ...
                << stringify(i->typedoc) << "}\n\ndefault value: @code{"
                << stringify(def) << "}\n\nlocation: @code{"
                << stringify(info_settingloc_to_extstr(i->loc)) << "}\n\n"
                << stringify(i->descdoc) << "\n";
    }
    std::cout << "@end table\n";
  } else if (cmd == "marks") {
    ...
    info_setfilter fi = {
        0, 0, 0, module_nomodtype, "inst-defs", module_noloc, 3, info_none};
    info_setfilterlist fl = {1, &fi};
    const struct info_setlist lst(info_list_settings(0, &fl, 0, 0, 0));
    assert(lst.n == 1);
    ...
    for (module_value *i(v.val.l.vals), *ie(v.val.l.vals + v.val.l.n); i < ie;
         ++i) {
      assert(getval(*i, "id"));
    ...
    std::cout << "@itemize @bullet\n";
    info_setfilter fi = {
        0, 0, 0, module_nomodtype, "percinst-defs", module_noloc, 3, info_none};
    info_setfilterlist fl = {1, &fi};
    const struct info_setlist lst(info_list_settings(0, &fl, 0, 0, 0));
    assert(lst.n == 1);
    struct module_value& v = lst.sets->val;
